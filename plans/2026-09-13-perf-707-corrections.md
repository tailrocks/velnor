...
Base: origin/main tip 91defe66. Branch: r0-707-corr. Worktree: /tmp/velnor-707c.
...
at the host budget. Unknown holder dimensions (legacy claims,
unobservable at claim time) contribute nothing; an all-unknown dimension
falls back to static policy. The releaser sums the *remaining* holders'
recordings, so its own limits never touch their ceiling. `builder_holder_count`
is replaced by `builder_holders_for_sizing`; legacy claim files parse
via `#[serde(default)]` as unknown.
