...
#[cfg(test)] mod parent_second_review {
 use super::prefix_step_block_with_if;
 #[test] fn rejects_plain_scalar_continuation() {
  let input="      - name: continued\n        if: ready\n          && other\n        run: true\n";
  assert!(prefix_step_block_with_if(input,Some("guard")).is_err());
 }
 #[test] fn quoted_unary_negation_is_valid_expression() {
  let input="      - name: negated\n        if: \"!cancelled()\"\n        run: true\n";
  let out=prefix_step_block_with_if(input,Some("guard")).unwrap();
  assert!(out.contains("(guard) && (!cancelled())"),"{out}");
 }
}
