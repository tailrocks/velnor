...
### Current baseline — 2026-08-27T18:43:50Z

- Repository: branch `fix/watchdog-registration-deadline`, HEAD
  `beb7222cb6ce0cd934123a57ded6f50b3f406c6c`, clean worktree, remote
  `origin=https://github.com/tailrocks/velnor.git`; branch is 227 commits ahead
...
- Current-SHA local evidence: `rtk mise run check` passed, exit 0, captured at
  `2026-08-27T18:43:50Z`: actionlint, cargo deny, cargo fmt, fleet generation,
  clippy `-D warnings`, and workspace nextest `1485/1485` passed with zero
  skipped tests. Focused `velnor-runner` nextest passed `1091/1091`; commit
  `beb7222` fails closed on corrupt or unreadable daemon-slot runner identity,
  preserving local recovery state.
