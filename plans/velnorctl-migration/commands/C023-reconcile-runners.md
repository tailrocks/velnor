# Command Task C023: Implement `velnorctl reconcile runners`

> **Executor instructions**: Implement only `velnorctl reconcile runners`. Do not fold
...
Plan and explicitly repair runner registry/local identity divergence.
...
Implement parser, typed arguments, handler, rendering, errors, tests, help, and completion metadata for only `velnorctl reconcile runners` in `crates/velnorctl/src/commands/reconcile_runners.rs` and `crates/velnorctl/tests/reconcile_runners.rs`.
...
1. Add exact typed Clap shape for `velnorctl reconcile runners`; closed values use `ValueEnum`. Reject unknown or sibling-command arguments.
...
4. Add parser, service-mock, transport, golden-output, exit-code, and no-secret tests named with filter `command_c023`.
5. Update command reference, generated completion/man metadata, and migration matrix. Do not retain an old alias.

**Verify**: `rtk cargo nextest run -p velnorctl --locked command_c023` passes; then `rtk mise run check` exits 0.
...
Seed each validation-owned divergence around fresh fixture run; prove dry-run exact, execution safe/idempotent, busy/foreign registrations preserved.
...
- [ ] `velnorctl reconcile runners --help` exits 0 and documents exact accepted syntax.
