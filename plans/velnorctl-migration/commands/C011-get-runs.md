# Command Task C011: Implement `velnorctl get runs`

> **Executor instructions**: Implement only `velnorctl get runs`. Do not fold
...
Expose versioned run resources through generic `get`, distinct from GitHub-style `run list`.
...
Implement parser, typed arguments, handler, rendering, errors, tests, help, and completion metadata for only `velnorctl get runs` in `crates/velnorctl/src/commands/get_runs.rs` and `crates/velnorctl/tests/get_runs.rs`.
...
- Show GitHub status/conclusion/attempt/ref/event plus Velnor-correlated jobs and source tag.
- Support selectors, field selectors, since, limit, watch, context, and repo.
- GitHub remains authoritative; local history may enrich but never overwrite.

## Steps

1. Add exact typed Clap shape for `velnorctl get runs`; closed values use `ValueEnum`. Reject unknown or sibling-command arguments.
...
4. Add parser, service-mock, transport, golden-output, exit-code, and no-secret tests named with filter `command_c011`.
5. Update command reference, generated completion/man metadata, and migration matrix. Do not retain an old alias.

**Verify**: `rtk cargo nextest run -p velnorctl --locked command_c011` passes; then `rtk mise run check` exits 0.
...
Dispatch fresh both-lane success and controlled failure; prove GitHub-only and Velnor-enriched run resources have correct source/conclusion.
...
- [ ] `velnorctl get runs --help` exits 0 and documents exact accepted syntax.
