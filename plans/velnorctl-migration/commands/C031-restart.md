# Command Task C031: Implement `velnorctl restart instance/<name>`

> **Executor instructions**: Implement only `velnorctl restart instance/<name>`. Do not fold
...
Restart safely with draining default.
...
Implement parser, typed arguments, handler, rendering, errors, tests, help, and completion metadata for only `velnorctl restart instance/<name>` in `crates/velnorctl/src/commands/restart.rs` and `crates/velnorctl/tests/restart.rs`.
...
1. Add exact typed Clap shape for `velnorctl restart instance/<name>`; closed values use `ValueEnum`. Reject unknown or sibling-command arguments.
...
4. Add parser, service-mock, transport, golden-output, exit-code, and no-secret tests named with filter `command_c031`.
5. Update command reference, generated completion/man metadata, and migration matrix. Do not retain an old alias.

**Verify**: `rtk cargo nextest run -p velnorctl --locked command_c031` passes; then `rtk mise run check` exits 0.
...
- [ ] `velnorctl restart --help` exits 0 and documents exact accepted syntax.
