# Command Task C028: Implement `velnorctl uncordon instance/<name>`

> **Executor instructions**: Implement only `velnorctl uncordon instance/<name>`. Do not fold
...
Clear Cordoned state and allow ready slots to acquire work.
...
Implement parser, typed arguments, handler, rendering, errors, tests, help, and completion metadata for only `velnorctl uncordon instance/<name>` in `crates/velnorctl/src/commands/uncordon.rs` and `crates/velnorctl/tests/uncordon.rs`.
...
1. Add exact typed Clap shape for `velnorctl uncordon instance/<name>`; closed values use `ValueEnum`. Reject unknown or sibling-command arguments.
...
4. Add parser, service-mock, transport, golden-output, exit-code, and no-secret tests named with filter `command_c028`.
5. Update command reference, generated completion/man metadata, and migration matrix. Do not retain an old alias.

**Verify**: `rtk cargo nextest run -p velnorctl --locked command_c028` passes; then `rtk mise run check` exits 0.
...
- [ ] `velnorctl uncordon --help` exits 0 and documents exact accepted syntax.
