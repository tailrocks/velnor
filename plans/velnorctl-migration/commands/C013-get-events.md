# Command Task C013: Implement `velnorctl get events`

> **Executor instructions**: Implement only `velnorctl get events`. Do not fold
...
Expose events through generic resource listing semantics.
...
Implement parser, typed arguments, handler, rendering, errors, tests, help, and completion metadata for only `velnorctl get events` in `crates/velnorctl/src/commands/get_events.rs` and `crates/velnorctl/tests/get_events.rs`.
...
- Support selectors, field selectors, since, limit, watch, and machine formats.
- Return versioned Event resources with object reference, type, reason, message, source, and transition time.
- Keep specialized type/reason/for filters in top-level `events` command.

## Steps

1. Add exact typed Clap shape for `velnorctl get events`; closed values use `ValueEnum`. Reject unknown or sibling-command arguments.
...
4. Add parser, service-mock, transport, golden-output, exit-code, and no-secret tests named with filter `command_c013`.
5. Update command reference, generated completion/man metadata, and migration matrix. Do not retain an old alias.

**Verify**: `rtk cargo nextest run -p velnorctl --locked command_c013` passes; then `rtk mise run check` exits 0.
...
Run fresh hold/cancel fixture scenario and prove generic event resources cover acquisition through teardown in order.
...
- [ ] `velnorctl get events --help` exits 0 and documents exact accepted syntax.
