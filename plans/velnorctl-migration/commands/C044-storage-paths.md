# Command Task C044: Implement `velnorctl storage paths`

> **Executor instructions**: Implement only `velnorctl storage paths`. Keep sibling
...
List canonical durable, regenerable, runtime, log, work, and class paths with ownership policy.
...
Implement only `velnorctl storage paths`: typed parser/arguments, thin handler, versioned output, errors, help/completion metadata, and tests in `crates/velnorctl/src/commands/storage_paths.rs` and `crates/velnorctl/tests/storage_paths.rs`.
...
1. Add exact typed Clap shape for `velnorctl storage paths`; reject unknown and sibling arguments.
...
4. Add parser, mock-service, transport, output, exit, and redaction tests under filter `command_c044`.
5. Update command reference, completion/man metadata, and old-command migration mapping without aliases.

**Verify**: `rtk cargo nextest run -p velnorctl --locked command_c044` passes; `rtk mise run check` exits 0.
...
Start isolated fixture instance, verify every path maps inside configured roots and job/cache activity appears only in declared classes.
...
- [ ] `velnorctl storage paths --help` exits 0 with exact syntax.
