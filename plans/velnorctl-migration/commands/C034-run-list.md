# Command Task C034: Implement `velnorctl run list`

> **Executor instructions**: Implement only `velnorctl run list`. Keep sibling
...
- **Effort**: M
- **Risk**: MED
- **Depends on**: Plans 068, 069, 074
- **Category**: command migration
- **Planned at**: commit `35d5bb7`, 2026-08-24

## Why this matters

List GitHub workflow runs with Velnor placement and infrastructure enrichment.
...
Implement only `velnorctl run list`: typed parser/arguments, thin handler, versioned output, errors, help/completion metadata, and tests in `crates/velnorctl/src/commands/run_list.rs` and `crates/velnorctl/tests/run_list.rs`.
...
1. Add exact typed Clap shape for `velnorctl run list`; reject unknown and sibling arguments.
...
4. Add parser, mock-service, transport, output, exit, and redaction tests under filter `command_c034`.
5. Update command reference, completion/man metadata, and old-command migration mapping without aliases.

**Verify**: `rtk cargo nextest run -p velnorctl --locked command_c034` passes; `rtk mise run check` exits 0.
...
Dispatch fresh success/failure/both-lane scenarios; prove filters, pagination, placement, and conclusion/source accuracy.
...
- [ ] `velnorctl run list --help` exits 0 with exact syntax.
