# Command Task C003: Implement `velnorctl explain <resource-or-field>`

> **Executor instructions**: Implement only `velnorctl explain <resource-or-field>`. Do not fold
...
Explain versioned resource schemas, fields, conditions, sources, and target syntax.
...
Implement parser, typed arguments, handler, rendering, errors, tests, help, and completion metadata for only `velnorctl explain <resource-or-field>` in `crates/velnorctl/src/commands/explain.rs` and `crates/velnorctl/tests/explain.rs`.
...
- Resolve resource and dotted field names from installed schema metadata.
- Distinguish this schema help from `capability explain`; never query or mutate live jobs.
- Return non-zero for unknown/ambiguous fields and include accepted resource names.

## Steps

1. Add exact typed Clap shape for `velnorctl explain <resource-or-field>`; closed values use `ValueEnum`. Reject unknown or sibling-command arguments.
...
4. Add parser, service-mock, transport, golden-output, exit-code, and no-secret tests named with filter `command_c003`.
5. Update command reference, generated completion/man metadata, and migration matrix. Do not retain an old alias.

**Verify**: `rtk cargo nextest run -p velnorctl --locked command_c003` passes; then `rtk mise run check` exits 0.
...
During fresh hold scenario, explain slot phase, job condition, runner identity, and source fields, then verify live resources conform.
...
- [ ] `velnorctl explain --help` exits 0 and documents exact accepted syntax.
