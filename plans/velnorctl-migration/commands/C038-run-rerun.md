# Command Task C038: Implement `velnorctl run rerun <run-id>`

> **Executor instructions**: Implement only `velnorctl run rerun <run-id>`. Keep sibling
...
Request GitHub rerun for all or failed jobs.
...
Implement only `velnorctl run rerun <run-id>`: typed parser/arguments, thin handler, versioned output, errors, help/completion metadata, and tests in `crates/velnorctl/src/commands/run_rerun.rs` and `crates/velnorctl/tests/run_rerun.rs`.
...
1. Add exact typed Clap shape for `velnorctl run rerun <run-id>`; reject unknown and sibling arguments.
...
4. Add parser, mock-service, transport, output, exit, and redaction tests under filter `command_c038`.
5. Update command reference, completion/man metadata, and old-command migration mapping without aliases.

**Verify**: `rtk cargo nextest run -p velnorctl --locked command_c038` passes; `rtk mise run check` exits 0.
...
Dispatch controlled failure, rerun failed, prove attempt increments and only expected jobs execute; also rerun full successful run.
...
- [ ] `velnorctl run rerun --help` exits 0 with exact syntax.
