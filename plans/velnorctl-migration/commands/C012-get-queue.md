# Command Task C012: Implement `velnorctl get queue`

> **Executor instructions**: Implement only `velnorctl get queue`. Do not fold
...
- **Effort**: M
- **Risk**: MED
- **Depends on**: Plans 065–069, 073–074
- **Category**: command migration
- **Planned at**: commit `35d5bb7`, 2026-08-24

## Why this matters

Show GitHub-sourced queued jobs waiting for matching labels.
...
Implement parser, typed arguments, handler, rendering, errors, tests, help, and completion metadata for only `velnorctl get queue` in `crates/velnorctl/src/commands/get_queue.rs` and `crates/velnorctl/tests/get_queue.rs`.
...
- Support label/repo/context filters and queue age.
- Show run, repository, job, queued duration, required labels, and `SOURCE=GITHUB` or `MERGED`.
- Never present broker visibility as global queue ownership.

## Steps

1. Add exact typed Clap shape for `velnorctl get queue`; closed values use `ValueEnum`. Reject unknown or sibling-command arguments.
...
4. Add parser, service-mock, transport, golden-output, exit-code, and no-secret tests named with filter `command_c012`.
5. Update command reference, generated completion/man metadata, and migration matrix. Do not retain an old alias.

**Verify**: `rtk cargo nextest run -p velnorctl --locked command_c012` passes; then `rtk mise run check` exits 0.
...
Cordon fixture instance, dispatch fresh hold/queued work, prove queued job and labels via GitHub source, then uncordon and observe departure.
...
- [ ] `velnorctl get queue --help` exits 0 and documents exact accepted syntax.
