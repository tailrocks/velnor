...
| 039 | Org-level JIT + multi-repo fleet ops | P0 | M | — | V0.1 | IN PROGRESS: exact selected `velnor-trusted` groups are live for Tailrocks (11 repos), ChainArgos (2), and jackin-project (1); the Tailrocks org pool is live and signed-apt 0.1.108 is published. Sentry apt upgrade plus sequential ChainArgos and jackin-project migration/smoke remain |
...
| 053 | tailrocks/parallax | 2 | L | 051 (pattern), 042 (attest) | IN PROGRESS — direct-main delivery is at `705af2ca`; combined Storage `29880330009` exposed restart-before-child-reap on GitHub while every other lane passed. Graceful shutdown now owns termination and proves all engine ports released without expanding policy ratchets. Automatic CI `29881560389` passed every product gate but correctly rejected the non-linear force-push event range; exact-head dispatch `29882121621` is active before clean combined Storage replacement. Darwin-release decision remains |
