...
|------|------|-------|--------|-----------|--------|
| 047 | ChainArgos/java-monorepo | 1 | M | — | IN PROGRESS — PR #1753 head `f510ee0` is pushed with nextest-only probes and current action pins; stale runs were cancelled. Fresh V-B/V-C and explicit human merge approval remain |
| 048 | ChainArgos/blockchain-nodes | 1 | S/M | — | IN PROGRESS — PR #651 head `63a2a7c` includes the shared package fixes, latest pins, and nextest-only instructions; local amd64 proof and 4/4 nextest pass. Fresh V-B/V-C and explicit human merge approval remain |
| 049 | jackin-project/jackin (stacks on PR #810) | 1 | L | 047 (pattern) | IN PROGRESS — PR #810 head `eff7c33` is pushed; standalone audit has zero errors, active commands are nextest-only, actionlint passes, and focused suites pass. Fleet proof and approval of `docs/capability-proposal-attest-build-provenance-v4.md` remain; one pre-existing Docker-fake integration failure is recorded for root-cause repair |
...
| 051 | tailrocks/holla | 2 | S | — | IN PROGRESS — PR #37 head `e4bef1e` is pushed and clean; fresh org-fleet V-B/V-C and merge remain |
| 052 | tailrocks/ruxel | 2 | S | — | IN PROGRESS — PR #3 head `6e9c02a` is pushed and clean; ignored test gates use nextest and are discoverable. Fresh org-fleet V-B/V-C and merge remain |
| 053 | tailrocks/parallax | 2 | L | 051 (pattern), 042 (attest) | IN PROGRESS — direct-main delivery through `a5ebba6` includes deterministic race coverage, nextest/trybuild example coverage, and current pins; local gates pass. Fresh Ubuntu V-B/V-C and the Darwin-release decision remain |
| 054 | tailrocks/termrock | 3 | M | 042 (Pages) | IN PROGRESS — direct-main delivery through `92c7afb` passes the full local gate with 337 nextest tests and current pins. Fresh V-B/V-C remains |
| 055 | schemalane + pg-bigdecimal + tracing-request-level (Class D trio) | 3 | M | 040 (schemalane services) | IN PROGRESS — PRs #3/#2/#2 are pushed at `5c839db`/`2cd6c1b`/`8782114`; nextest and actionlint evidence is green. Fresh V-B/V-C and merge remain |
| 056 | tailrocks/parallax-telemetry-playground | 3 | S | — | IN PROGRESS — policy-compliant direct-main delivery through `c64cf19` replaces superseded PR #8 and carries nextest-only testing plus current pins; fresh V-B/V-C remains |
...
| 060 | Estate-wide nextest-only Rust testing | 4 | M | 041, 046 | IN PROGRESS — active program heads are migrated and scanned; former executable doctests have nextest-discoverable integration/trybuild coverage, and ignored gates preserve selection/output semantics. Fixture PR #4 and estate PR delivery plus V-A/V-B/V-C remain |
