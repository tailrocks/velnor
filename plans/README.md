...
|------|------|-------|--------|-----------|--------|
| 047 | ChainArgos/java-monorepo | 1 | M | — | IN PROGRESS — PR #1753 head `1c136c4` is pushed with nextest-only probes, current action pins, and canonical concurrency; stale runs were cancelled. Fresh V-B/V-C and explicit human merge approval remain |
...
| 052 | tailrocks/ruxel | 2 | S | — | IN PROGRESS — PR #3 head `fd12173` is pushed and clean; ignored test gates use nextest and are discoverable. Fresh org-fleet V-B/V-C and merge remain |
...
| 057 | tailrocks/tablerock | 3 | S | — | IN PROGRESS — trunk `36b83dd` retains the repaired strict-lint/Redis fixture baseline and adds its native macOS checkpoint; 768/768 nextest, strict clippy, actionlint, and the shape-aware estate audit pass. Native run `29804459618` is active; V-B/V-C waits the org-fleet migration. |
