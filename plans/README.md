...
| 053 | tailrocks/parallax | 2 | L | 051 (pattern), 042 (attest) | IN PROGRESS — direct-main `705af2ca` structurally fixes restart-before-child-reap; exact-head CI `29882121621` and combined Storage `29883285817` are green on both lanes. `24aade0` aligns the required aggregator's exact display name. §12.4 is answered from measured evidence: only the two native Apple package producers remain outside lane comparison. Exact 0.1.109 proof and evidence reconciliation remain |
...
| 057 | tailrocks/tablerock | 3 | S | — | IN PROGRESS — trunk `03bc78e8` includes direct AppKit control ownership, final CSV/grid reachability, the removed lane-specific source-touch workaround, and subsequent roadmap deliveries. CI `29884740470` exposed the known stale-target defect while running Velnor 0.1.58 and was cancelled as invalid; GitHub-hosted Native `29884740464` remains active. Exact 0.1.109 V-B/V-C proof remains |
