...
| 053 | tailrocks/parallax | 2 | L | 051 (pattern), 042 (attest) | IN PROGRESS — direct-main `705af2ca` structurally fixes restart-before-child-reap; exact-head CI `29882121621` and combined Storage `29883285817` are green on both lanes. `24aade0` aligns the required aggregator's exact display name. Its replacement Velnor proof correctly waits for the published 0.1.109 fleet upgrade; evidence reconciliation and the Darwin-release decision remain |
