...
#### Rev 14 delta @ `5a72ade1` (2026-09-16)
...
| Local proof — PASS on this tree | V | HEAD `5a72ade1` ≠ pin `eaeb3e10`: `velnor-workflow policy --workflow-root . --head-sha 5a72ade1… --base-sha $(origin/main) --base-revision 4790f7cc… --ruleset-contexts DCO,ci-required,Policy` → pinned generator built from the local clone (30 s), **11 rules, 0 failed**. With the ruleset **as it is live today** (`protect-main` = `DCO`, `ci-required`): 10 PASS, `required-checks` FAIL `the live ruleset does not require the policy entrypoint context Policy; the gate is advisory until the ruleset requires it` — a true finding, see bootstrap step 4 |
...
| Base validator on this tree (transition story) | V | `4790f7cc` binary built locally, `VELNOR_WORKFLOW_POLICY_REVISION=4790f7cc… velnor-workflow policy --workflow-root .` → `rejected 7 finding(s)`: `ci-policy.yml: pull_request_target is forbidden` + 6 × `self-hosted jobs require a default-branch trusted-event gate` (its gate rule is a literal string match against its own render). The base check **cannot** pass on this PR; it passes only once `main` carries the new validator |
| Bootstrap (one-time, admin) | V | (1) rebase/merge this series onto the PR branch and **re-run the pin bump** if any generator commit's SHA changes (`[generator] revision` = last generator commit; `--plain --force`; `--check`); (2) merge PR 872 into `main` with a **merge commit or fast-forward that preserves the commit SHAs** (`git merge --no-ff` pushed by a ruleset-bypass actor); the repository currently allows **squash only** (`allow_squash_merge: true`, merge/rebase disabled) — a squash rewrites the SHAs, the declared pin is no longer an ancestor of `main`, `pin-reachable` fails on every later PR and the local-clone pin build cannot find the commit. Enable "Allow merge commits" for this merge, or push the merge directly; (3) the base check `Policy` shows FAIL on the bootstrap PR — expected (row above); bypass it once; (4) add `Policy` to the `protect-main` ruleset's required status checks (today it requires only `DCO`, `ci-required`; `Velnor workflow policy` is the workflow name, `Policy` the check context) — until then `required-checks` fails advisory on every PR; (5) prove the new validator on `main`: `gh workflow run ci-policy.yml --ref main` (`workflow_dispatch` audits `main` against itself: `HEAD_SHA = BASE_SHA = github.sha`, pin inherited → `pin-reachable`/`pin-monotonic` PASS) or open any PR — its `Policy` run is now the new validator |
...
| Pins @ `eaeb3e10` (D19) | V | `.github-gen/velnor-workflow.toml:9`; `ci-policy.yml:51,71,93`; single pin commit `5a72ade1` at the end of the series, touching no Rust source; `--check` exit 0 with HEAD ≠ pin |
...
| Ruleset merge-method constraint | U | D19's self-referential pin requires the pinned commit to be in `main`'s history; the repository's squash-only setting is incompatible with it (see bootstrap step 2). Decision for the maintainers: allow merge commits for `main`, or the pin scheme must be replaced by one that survives squashes (e.g. pin by tree hash of `crates/velnor-workflow`) — not done here |
