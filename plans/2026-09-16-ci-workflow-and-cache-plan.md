...
Date: 2026-09-16 (rev 16: D18 read-through overlay replaced by a copy seed of the `pr` Cargo store, PR writes persist and are host-shared; rev 14: pin-decoupled policy validator + config-only trust-gated emission + `velnor-host-docker` online; rev 13: DCO sign-off restored; rev 12: first full green `ci-required` PR rollup @ `8cc755fd` run 35039991442; rev 11: input-parameterized O(1) kind reusables + template-memory ceiling + actionlint in Policy + revision-proven D19 guard (`--revision`) + local `setup-velnor-workflow` for the owner + runner manifest self-pin removed, pin `17b441c1` @ `8f41f2dd`; rev 10: trust-gated aggregate skip @ `81104ba8`; rev 9: Velnor prepare-cargo prefetch + clippy @ `28b528d8` (superseded by rev 11); rev 8: prefetch package name @ `284f1091`; rev 7: docker collapsed skip @ `8e1ae640`; rev 6: kind file sharding @ `6c51eceb` (superseded by rev 11); rev 5 collapsed verify runtime + pin `8decfeeb` @ `9c1211d7`; rev 4 D19 install fix @ `ad76f4a`; rev 3 `c273707d`).  
...
#### Rev 16 delta — D18 read-through overlay replaced by a copy seed (2026-09-16)
...
| Deleted | V | `storage::StoreOverlay` \| `ScratchMountpoints` \| `overlay_mount_options`; `container::CARGO_STORE_LAYERS` (now `CARGO_STORE_SUBTREES`, plain binds) \| `store_overlays` \| `create_store_overlay*` \| `inspect_store_overlay_scratch_args` \| `store_overlay_scratch_*` \| `job_volume_labels`; `executor::create_store_overlays`; `execution::store_overlay_support` \| `StoreOverlaySupport` \| `STORE_OVERLAY_PROBE_LABEL` \| `probe_store_overlay` \| `probe_daemon_path`; `trust_class::ReadThroughSupport` \| `StoreReadThrough` \| `read_through_*`; `docker_lease::list_daemon_owned_volume_format_args` \| `reclaim_daemon_orphan_volumes`; `docker::client::daemon_orphan_volume_names`; `velnorctl docker report` `capabilities.readThroughStoreOverlay` \| `DiagnosticDockerRunner`; `velnor-runner preflight` `Read-through store overlay (D18)` line; `lib.rs` `CommandRunner` re-export; all their tests |
| Replacement | V | `storage::seed_cargo_store(from, to, budget)`: for every regular file under trusted `registry/cache` \| `registry/index` \| `git/db` absent in `pr`, copy to `.velnor-seed-<pid>-<seq>` beside the destination, `link(2)` into place (no-replace rename), unlink temp; symlinks skipped; `git/db/<repo>` seeded whole (objects before refs); newest-first under `StoreBudgetPolicy::cargo_seed_headroom_bytes(pr store bytes)` = compiler budget − current `pr` Cargo store; skipped units logged. Called in `runner.rs` `execute_script_job_inner` before `github_job_container_spec` for `AdmittedTrust::cargo_seed_scope() == Some("trusted")` jobs only. PR jobs bind the `pr` store read-write via the same `cargo_store_mount_operands` path as trusted jobs |
| Logs | V | daemon: `seeded pr cargo store from trusted: N files, M bytes, T ms`; job **Set up job**: `Persistent stores: cargo pr scope (seeded from trusted: N files)` \| `Persistent stores: cargo trusted scope` |
| Tests | V | `storage::tests::seed_copies_only_missing_files_and_never_overwrites` \| `seed_writes_a_same_directory_temp_and_leaves_none_behind` \| `seed_skips_symlinks_and_never_follows_them` \| `modifying_a_seeded_pr_file_leaves_the_trusted_file_unchanged` (bytes + inode + nlink) \| `seed_budget_takes_newest_units_first_and_reports_what_it_skipped`; `capacity::tests::cargo_seed_headroom_is_the_compiler_budget_less_the_store`; `container::tests::pr_scope_jobs_bind_the_persistent_pr_cargo_store_read_write`; `trust_class::tests::same_repo_pull_request_on_trusted_pool_writes_pr_store_seeded_from_trusted` |
| Docs | V | `content/docs/guides/macos-host.mdx` §"Stores and the `pr` seed" (cold-trusted-store caveat removed: the first `Prepare Cargo` on a host warms every later unit); `config/fleet/RUNBOOK.md` §"Trust-scope `pr` seed (D18)" |
