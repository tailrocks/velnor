...
- `actionlint` checks workflow syntax; `deny` runs advisory checks. `check` is the
  aggregate gate for these local mise tasks and also runs the offline CI/fleet
  policy tasks — this is the mise-task aggregate, not the runtime
  expected-work aggregate that scores CI results (see [change-aware
  planning](/concepts/change-aware-planning)). Fleet policy
