...
> Navigation: [← Security and data](/operations/security-and-data) · [Index](/) · [Next: Contributing →](/guides/contributing)

## Current: build, test, lint, format

The workspace has 9 members and uses Cargo resolver 3, Rust edition 2024, and a
shared dependency table. The pinned toolchain is Rust 1.98.1 with `rustfmt` and
...
Zig, plus the pinned Bun and prek used by local commit checks. The lockfile is
mandatory, and CI/jobs use locked artifacts. Prepare each clone or agent checkout:
...
Bootstrap installs a repository-local hook shared by linked worktrees. Normal
commits validate their complete staged snapshot with formatting first and Clippy
second, including configuration-only commits. Unstaged fixes, helper changes,
and untracked dependencies cannot supply missing staged inputs. Checks preserve
the original staged, unstaged, and untracked content. Apply formatting separately
with `mise run fmt-fix`, then review and stage it. Rerun bootstrap after hook
tooling changes; existing foreign hooks or configured hook paths require explicit
integration and are never overwritten. Run these current gates:
...
- `actionlint` checks workflow syntax; `deny` checks advisories, bans, licenses,
  and sources. `check` runs formatting, Clippy, then tests in order and preserves
  the remaining validation gates, including isolated-hook regressions and the
  offline CI/fleet policy tasks. Fleet policy
  generation is a privileged publisher operation: set
  `VELNOR_FLEET_POLICY_OUT_DIR` to the provisioned absolute root-owned directory.
  `audit-ci`, `fleet-plan`, and the runtime read that same directory when the
  variable is set; without it, repository checks read the committed
  `config/fleet/policies/` snapshots. Org-scoped daemons on runner hosts fall back to
  the packaged copy of those snapshots (`/usr/share/velnor/fleet/policies`), so
  a missing publisher directory cannot leave a fleet without a desired policy.
...
```sh
mbx zigbuild -p velnor-runner -p velnorctl -p velnor-tools \
...
- `velnor-control` owns daemon-side application services and directly consumes
  the shared model. `velnor-client` owns the transport contract
...
- `velnorctl` owns operator composition and the Axum adapter; it currently uses
  ...
  owns the daemon/runtime binaries and consumes the control and model crates.
  `velnor-tools` owns maintainer policy/verification tooling; the
