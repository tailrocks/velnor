...
- `velnor-cas` owns digest-verified content-addressed storage; the crash-safe
  action journal is limited to the action and shared models. Docker Rust build
  outputs are owned by the image-pinned Mr Boxington store rather than another
  Velnor compiler-cache crate.
...
- `velnorctl` owns operator composition and the Axum adapter; it currently uses
  ...
  owns the daemon/runtime binaries and consumes control, action, and model
