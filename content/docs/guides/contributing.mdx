...
```mermaid
flowchart TD
  A[Admission] --> B[Execution]
```
````

The documentation site renders these blocks as diagrams with strict SVG
sanitization. Keep labels readable, use diagrams to clarify relationships,
and retain surrounding prose so the page remains understandable without the
rendered graphic.
