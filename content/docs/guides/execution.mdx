...
user secrets is refused before execution unless the pool is `trusted`
(`VELNOR_TRUST_SCOPE=trusted`) and the job's own trust class — derived from
its event — is trusted: fork and unknown jobs are refused user secrets on
every pool. See [Trust scope](/reference/trust-scope) for the full capability
table and the store partitioning it controls.
...
Jobs run unbounded: no `--cpus`/`--memory` flag, no slice CPU/RAM ceiling,
and no compiler partition is ever imposed — builders see the whole
machine. Host capacity is governed by the single host-wide `max_jobs=N`
permit ledger. Full policy:
[Storage and resource budgets](/operations/storage-and-resources).
