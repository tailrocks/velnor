...
the scheduler and source of truth. Native slots apply Velnor's action,
capability, and trust admission, execute eligible jobs, stream progress, and
report the result. The Scale Set adapter classifies offers, then delegates
acquired jobs to the official Actions runner.
...
sequenceDiagram
    ...
    participant V as Native Velnor runner
...
Native admission happens before checkout, downloads, container creation,
cache or artifact work, leases, or injection of job execution credentials. It
checks the trust policy, capability manifest, action graph, job shape, and
selected backend. Rejection is fail-closed.
...
## Native admission and supported work

This section covers the Native lane's action-closure admission. The Scale Set
adapter classifies offers from event and owner/repository fields, then hands
acquired jobs to the official Actions runner; it does not resolve the Native
action closure. Offers with unparseable event names, missing owner/repository
fields, `pull_request*` events, or `workflow_run` are classified as unknown.
Unknown Scale Set offers are demoted to `Observed` and are never admitted or
acquired. Other parseable event names with nonempty owner and repository fields
are classified as trusted. See
the [Scale Set job flow](/concepts/architecture#scale-set-job-flow) for the
offer-classifier limits.

The Native lane does not run arbitrary Actions workflows. It admits a **closed
...
`velnor-workflow` generates the workflow files and local reusable-workflow calls
in the repository. Velnor does not fetch a workflow provider at runtime. The
remaining fetched action surfaces use pinned commit SHAs and closed input
surfaces: `jackin-project/jackin-role-action`, `fsfe/reuse-action` (a Docker
action), and `jdx/mr-boxington-action` (a Node action). The admitted
server-expanded reusable workflow is the pinned `jackin-project/jackin-role-action`
publish workflow.
...
These limits apply to the Native lane and its built-in action allowlist. The
Scale Set lane uses offer classification and delegates acquired jobs to the
official Actions runner; this Native action closure does not govern it.
...
- **macOS and Windows.** Execution is Linux-only and macOS labels are rejected.
  A Mac can still host Linux container jobs through its Docker VM; see the
  [macOS on-demand host](/guides/macos-host) guide.
...
These checkout, cache, and artifact paths describe Native Velnor adapters. The
Scale Set official runner handles those job steps through its own Actions
session.
...
not used as a job token. For native jobs, GitHub supplies job credentials in
the acquired payload; Velnor injects the applicable `GITHUB_TOKEN`, Actions
runtime token, OIDC data, cache credentials, and Results Service data only
after admission. In the Scale Set lane, the official runner receives job
credentials through its GitHub session.

Native trust scope defaults to `untrusted` and fails closed. A native job
carrying GitHub user secrets is refused before execution unless the pool is
`trusted` (`VELNOR_TRUST_SCOPE=trusted`) and the job's own trust class —
derived from its event — is trusted: fork and unknown jobs are refused user
secrets on every pool. See [Trust scope](/reference/trust-scope) for the full
capability table and the store partitioning it controls.

For Native jobs, secret variables and mask hints become runtime masks; masked
values are excluded from environment URLs and sanitized job dumps. Firecracker
snapshots must contain no job credentials, and guest readiness rejects guests
that already contain them.

## Backend selection

Select exactly one backend for Native jobs in
`/etc/velnor/execution.toml` (or the configured execution file):
...
#### Dual-lane cache transport

Generated CI runs the same verification units on two lanes. Cache **keys**
are lane-agnostic; transport, budget, and retention differ.

```text
┌─────────────────────────────────────────────────────────────────────────┐
│  Same unit keys (rustup, mold, cargo, mbx, docker seed) per verification │
│  Different transport + budget + observability per lane                   │
└─────────────────────────────────────────────────────────────────────────┘

  GitHub lane                          Velnor lane
  ───────────                          ───────────
  Transport: actions/cache API         Transport: host bind mounts
  Budget: 8 GiB (internal class table) Budget: 50 GiB caches class + mbx
  Platform cap: 10 GB (7-day LRU)       Platform cap: host disk + slot GC
  mbx: backend github, objects         mbx: backend local, per-slot store
  PR: restore-only (no trusted save)   PR: host store writes (trust-scoped)
  Outcomes: exact | prefix | cold |     Outcomes: keyed evidence or unknown
            unknown                   Declarations: host_warm_layers
```

Each unit job emits a schema-version-2 `VELNOR_CI_REPORT` line and a step summary through
`report-velnor-ci-outcomes`. On the GitHub lane, `cache_outcomes.{rustup,mold,
cargo,mbx,docker_seed}` classify `actions/cache/restore` results with
`cache-primary-key` versus `cache-matched-key` (`exact`, `prefix`, or `cold`).
The pinned MBX action exposes an exact-hit boolean and primary key, but no
matched key; a false result is therefore `unknown` unless a producer supplies
explicit matched-key evidence. Compiler-cache summaries remain in the
separate `compiler.mbx_outcomes` field. On the Velnor lane,
`cache_declarations.host_warm_layers` records the runner's persistent-path
declaration; it is kept separate from keyed cache proof. Only explicit cold
layers log `::warning::`, and telemetry never fails the job.

Trusted producers (`push` to the default branch, `schedule`, and trusted
`workflow_dispatch`) save GitHub-lane snapshots; pull requests and fork PRs
are restore-only on GitHub. Fork pull requests that require the Velnor lane are
rejected at admission until an explicit GitHub-only policy is chosen.

Unit jobs may declare `cache-mode: read` on low-trust triggers as defense in
depth; Velnor ignores GitHub cache transport for host-persistent paths entirely.

#### Docker Rust acceleration

The default job image pins Mr Boxington (`mbx`) 1.12.0 and runs
...
The image deliberately uses standalone setup because noninteractive Velnor
execs require the stable absolute Cargo shim path `/opt/mbx/bin/cargo`; the
image bootstraps mise at 2026.9.9, which supports the current locked mbx
backend.
...
admitted scope — the job's trust class, derived from its event, narrowed by
the operator's pool ceiling — and then by numeric GitHub repository ID. A
missing or invalid repository identity fails closed to a job-ephemeral store.
See [Trust scope](/reference/trust-scope) and
...
Native Docker jobs run unbounded: no `--cpus`/`--memory` flag, no slice CPU/RAM
ceiling, and no compiler partition is imposed — builders see the whole
machine. The shared host-wide `max_jobs=N` ledger is a ceiling; Native slot
count caps Native concurrency, and configured worker capacity caps Scale Set
demand. Either lane can stay below `N`. Full policy:
[Storage and resource budgets](/operations/storage-and-resources).
...
The default job image does not ship sccache. The first explicit-sccache step
provisions the pinned, checksum-verified sccache release into the job
container itself (reusing a PATH binary already at the locked version), so
only jobs that request the mode ever download or run it.
...
#### Warm workspaces on the non-mbx paths

The sccache and plain-Cargo opt-out paths check out into a stable workspace
that persists across jobs on the same slot, namespaced by trust scope and
repository (`<slot>/stable-workspaces/<scope>/<repository-id>/workspace`).
Consecutive jobs for the same repository re-check out over it instead of
into a fresh directory, and `clean` keeps the top-level `target/` while
still removing every other untracked file — so a warm same-SHA rebuild
finds Cargo fingerprints intact and reports fresh, exactly like a
long-lived developer checkout. Mtimes stay wall-clock throughout: changed
sources rebuild, unchanged sources do not, and no artifact is ever restored
from a store. The mbx default path is unaffected and keeps the ephemeral
per-job workspace, since mbx manages its own targets.

Each slot's stable tree is capped at 30 GiB (the same allowance as mbx's
managed targets); when a new repository scope arrives over budget, the
least-recently-used idle scopes are evicted. A job without a repository id,
or any stable-allocation failure, falls back to an ephemeral workspace with
a forensics line — warmth is an optimization and never fails a build.

The floor this sets: a warm same-SHA rebuild on a warm slot is a fresh
check (sub-second). Anything that misses the fingerprints — a new SHA, a
cold slot, an evicted scope — pays a content-hit rebuild instead: every
unit re-invoked, each hitting its content cache on the sccache path, or a
full recompile on the plain-Cargo opt-out path.
...
`actions/cache` and `Swatinem/rust-cache` are refused at planning. When an
active job cancellation token and a vsock cancel handle are both available,
Velnor attempts a guest `Cancel` over vsock; otherwise, the jailer process
group is the termination fallback.
...
These slot and wait settings apply to Native broker jobs. Scale Set offers use
durable demand, configured worker capacity, and the shared host-wide permit
ledger; see [Scale Set job flow](/concepts/architecture#scale-set-job-flow).
...
This section describes Native job cancellation through the broker session and
per-job cancellation token. The Scale Set official runner handles its own job
cancellation; Velnor manages the worker lifecycle and cleanup.
...
`always()` and `cancelled()` steps must still find a container to run in. The
step process group and Docker-action sidecars die at `Requested`, matching
upstream's step cancel token.
...
ladder; the guest hook attempts graceful cancellation. Teardown still owns
resource removal.

## Completion durability

This outbox and replay contract applies to Native Run Service completion. The
Scale Set official runner reports its own job completion through its GitHub
session; Velnor tracks worker lifecycle and cleanup.
...
These broker, action-admission, and Native completion limits describe the
Native runner path. Scale Set offers use a separate session and the official
runner handles each acquired job's protocol; Native broker and outbox checks do
not apply to Scale Set work.
...
The Native lane does not establish complete GitHub Actions parity, arbitrary
guest feature parity, crash-resume of an executing job, or guaranteed cleanup
after uncertain isolation teardown. The Scale Set lane runs the official
Actions runner for acquired jobs.

For Native operator diagnosis, distinguish stages: no broker delivery means
scope, labels, registration, or connectivity; delivery without execution means
