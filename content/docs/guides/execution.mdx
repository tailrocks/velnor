...
Velnor mounts the daemon pool's host-persistent Mr Boxington cache and managed
target directories at `/var/cache/mbx`. `VELNOR_TRUST_SCOPE` is configured for
the daemon pool, not independently admitted for each job; it selects the
pool's trust namespace (such as `untrusted`, `trusted`, or `release`) for
persistent compiler stores. Those stores are further scoped by repository
identity when the acquired job payload provides it. Velnor derives no per-job
trust classification: the job message has no `head_repository_id` from which
it could safely derive one. If repository identity is missing or invalid,
Velnor falls back to a job-ephemeral store. This fallback is a persistence
decision, not a trust classification. Trust partitioning is a cache reuse
boundary, not encryption or protection from a compromised host.
...
combined. These limits operate inside Velnor's global disk-capacity admission
and accounting; they do not increase host capacity. Cargo registry and Git
source persistence remain separate stores and are unaffected by Mr Boxington's
GC.

To use the supported alternative, declare
`mozilla-actions/sccache-action` explicitly in the workflow. Velnor then uses
the action's sccache path and sets `MBX_DISABLE=1`; it does not combine the two
accelerators. To disable Mr Boxington without selecting sccache, set
`MBX_DISABLE=1` in the workflow. Kache and the former Velnor compiler-cache
...
cache is disabled for MicroVM. The Docker Mr Boxington integration, its mount,
and explicit sccache alternative are not provided to MicroVM jobs. End-to-end
cancellation is not currently wired for MicroVM.
...
Active-job cancellation arrives through the busy broker session and drives one
cancellation token for the job. Requesting it makes `cancelled()` true and
`success()`/`failure()` false, stops ordinary remaining steps from being
dispatched, and walks the step's own work — its host process tree, Docker action
sidecar and BuildKit builder — down a termination ladder. The job and service
containers deliberately survive that request so `always()` and `cancelled()`
post steps run against a container that still exists, and are terminated on
escalation. The escalation grace comes from the server's cancellation message
when it sends one.

MicroVM cancellation is not end to end: the guest is not yet driven from that
token. Complete
