...
identity when the acquired job payload provides it. A per-job trust class
remains unresolved: the job message has no `head_repository_id` from which
Velnor can safely derive one. If repository identity is missing or invalid,
Velnor falls back to a job-ephemeral store. This fallback is a persistence
decision, not a per-job trust classification. Trust partitioning is a cache
reuse boundary, not encryption or protection from a compromised host.
