...
└─────────────────────────────────────────────────────────────────────────┘
  ...
  Outcomes: exact | prefix | cold      Outcomes: host_warm (persistent paths)
```

Each unit job emits a `VELNOR_CI_REPORT` line and a step summary through
...
mbx also consults its setup outputs and step-summary wording. On the Velnor
lane the same fields report `host_warm` for layers backed by host-persistent
stores (cargo registry/git, mise, mbx local backend, image-baked rustup/mold).
Cold layers log `::warning::` and never fail the job.
