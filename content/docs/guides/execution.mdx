...
Velnor mounts the host-persistent Mr Boxington cache and managed target
directories at `/var/cache/mbx`. The daemon/pool's configured trust scope
selects the store namespace: `untrusted`, `trusted`, or `release`, followed by
the numeric GitHub repository ID. Velnor does not derive or admit a trust class
per job; every job runs within the boundary of the daemon/pool that accepts it.
Missing or invalid repository identity fails closed to a job-ephemeral store.
This prevents compiled state from one repository or trust boundary becoming an
input to another. Trust partitioning is a cache reuse boundary, not encryption
or protection from a compromised host.
