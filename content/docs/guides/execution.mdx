...
The default job image pins Mr Boxington (`mbx`) 1.6.0 and runs
...
directories at `/var/cache/mbx`. The host store is partitioned by the admitted
`untrusted`, `trusted`, or `release` trust class and then by numeric GitHub
repository ID. Missing or invalid repository identity fails closed
to a job-ephemeral store. This prevents compiled state from one repository or
trust boundary becoming an input to another. Trust partitioning is a cache
