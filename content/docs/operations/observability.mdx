...
> Navigation: [← Operator guide](/guides/operator) · [Index](/) · [Troubleshooting](/troubleshooting)
...
outbox entry is retained until it reaches a terminal state, which is one of
three: GitHub acknowledged the completion; the run service reported the job
already terminal; or the entry's durable budget — a permanent refusal, eight
attempts, or six hours — was spent, and the entry was abandoned. An abandoned
entry means **GitHub was never told the job finished** and the run service will
time the job out; the reason is preserved permanently in the journal. Outbox
age counts only pending entries, so an abandoned entry drops out of it.

Inspect outbox age and completion-related events when execution succeeded
locally but GitHub remains pending, and inspect the journal's unresolvable
completions when GitHub timed a job out that Velnor believed it finished.
