---
title: "Security and data contract"
---
...
> Navigation: [← Observability](/operations/observability) · [Index](/) · [Next: Development →](/guides/development)

## Security boundary

- Trust scope is operator configuration, never job payload. It defaults to
  `untrusted` and fails closed: absent, empty, and unrecognized values all
  resolve to `untrusted`, and only the exact value `trusted` grants the host
  Docker socket, privileged container and service options, host port
  publishing, BuildKit secrets, and the right to run a job carrying user
  secrets. One write-once resolution serves both binaries, so a flag and an
  environment variable can no longer disagree and let a fork's job resolve into
  a hardened pool's stores. See [Trust scope](/reference/trust-scope).
  `crates/velnor-runner/src/trust_scope.rs:39-66`,
  `crates/velnor-runner/src/github_adapter.rs:221-235`,
  `crates/velnor-runner/src/runner.rs:6347-6361`
...
3. Durable control state/events retain bounded sanitized records, digests,
   identities, leases, and events; query, log, and storage projections are
   currently transient after service construction. Content stores retain content
   only where their respective paths are used.
...
Firecracker teardown explicitly does not imply secure wipe. Leftover-disk
reclaim removes selected filesystem objects/images; neither source establishes
media sanitization.
...
- **No at-rest confidentiality claim.** The reviewed sources do not establish
  encryption for the control database, caches, workspaces, logs, attestation
  bundles, snapshot files, or writable disks. File modes and
...
- **No immediate-global-retention claim.** Control retention has bounded defaults
  ...
  it does not govern cache, workspace, log, package, or external service
  retention.
