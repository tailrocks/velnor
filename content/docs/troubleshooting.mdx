...
start or checkout activity. Likely causes are an action that is not on the
closed allowlist, an unpinned ref, an action input outside its declared rules,
a trust-policy violation, an oversized or deep action graph, and user secrets
in a non-`trusted` scope. The rejection names the field, the received value,
and the accepted values. See
...
If the build looks under-parallelized, the daemon is not throttling it:
jobs run unbounded and no compiler partition is imposed. Check the tool's
own parallelism settings (`CARGO_BUILD_JOBS` is whatever the workflow or
tool default sets — the daemon no longer overrides it) and the host's
actual load against `VELNOR_MAX_JOBS`.
