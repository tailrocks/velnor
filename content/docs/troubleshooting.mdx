...
Healthy Native path: a delivered job becomes acquired, locally claimed, and
admitted before native checkout, downloads, credentials, or execution.

Inspect `velnorctl get jobs`, `get runs`, `events`, and `telemetry`. For Native
jobs, check the configured backend and capability manifest. Native admission
resolves the complete action closure and rejects unsupported, unpinned,
untrusted, or over-limit work before side effects.

The Scale Set adapter classifies offers from event and owner/repository
metadata, then delegates acquired jobs to the official Actions runner. It does
not resolve the Native action closure or apply Native secret policy. See
[Scale Set job flow](/concepts/architecture#scale-set-job-flow) for the
classifier limits. Unknown Scale Set offers are demoted to `Observed` and are
never admitted or acquired. This includes unparseable event names, missing
owner/repository fields, `pull_request*`, and `workflow_run`; other parseable
events with nonempty owner and repository fields are trusted.

Sections 3–11 below diagnose Native slots, broker sessions, action preparation,
Docker/MicroVM execution, publishing, and the completion outbox. For Scale Set
failures, inspect offer classification, durable demand/permit state, runner and
DinD worker health, and GitHub job evidence. The official runner handles the
per-job protocol; Native broker, backend, and outbox checks do not apply.

Distinguish Native admission rejection from execution failure: there should be
no native backend start or checkout activity. Likely causes are an action not
on the closed allowlist, an unpinned ref, an action input outside its declared
rules, a trust-policy violation, an oversized or deep action graph, or user
secrets in a Native non-`trusted` scope. The rejection names the field, the
received value, and the accepted values. See
[what Velnor does not support](/guides/execution#what-velnor-does-not-support)
and [Trust scope](/reference/trust-scope).

A step that neither ran nor failed but was *skipped* is usually not an
admission problem at all — an `if:` that names no status function is compiled
as `success() && (<CONDITION>)`. See
[Expressions and conditions](/reference/expressions#the-rule-most-people-get-wrong).

## 3. Native authentication and connectivity
...
## 4. Native job delivery and acquisition
...
## 5. Native capacity and reservation
...
## 6. Native action preparation and checkout
...
is workload failure. Cancellation is a separate terminal path. Firecracker
attempts a guest `Cancel` over vsock when both the active cancellation token
and vsock cancel handle are available; otherwise, termination falls back to
the jailer process group. If a job remains stuck, inspect guest-cancel and
jailer cleanup evidence; a cancellation request alone is not proof of
reported completion.

## 8. Native logs, timeline, and live status
...
comparing jobs — a node whose trust scope changed reads a different partition
and looks cold once. Cargo registry and Git source caches are separate, so a
warm registry does not imply a warm Mr Boxington store.

If the build looks under-parallelized, jobs run unbounded and no compiler
partition is imposed, but lane capacity can still be lower than the shared
`VELNOR_MAX_JOBS` ceiling. Check ready lane capacity, the tool's own
parallelism settings (`CARGO_BUILD_JOBS` is whatever the workflow or tool
default sets — the daemon no longer overrides it), and the host's actual load.
...
every status except 4xx other than 408, 409, and 429 — under a bounded budget
of eight attempts and six hours. A typed remote terminal response can mean
GitHub already considers the job terminal.

If GitHub timed the job out while Velnor believed it finished, the entry was
abandoned rather than acknowledged: its budget was spent, and the reason is in
the journal's unresolvable completions. Outbox age will not show it, because
that metric counts only pending entries.
...
## 11. Native cleanup and recovery
...
On native-lane drain, idle slots exit at a poll boundary and native jobs are
allowed to finish subject to service-stop limits. Controller restart is not
itself proof that slot workers stopped. Query/log projections may be empty
after recreation even while durable events remain; use those durable events
plus GitHub state to reconstruct what happened.

## Native quick distinction table
