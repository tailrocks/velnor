...
If the build looks under-parallelized, read `VELNOR_JOB_BUDGET` from the job
environment before assuming a defect: `CARGO_BUILD_JOBS` is the slot's share of
the derived host budget, not the host's core count, and a workflow that sets it
is overridden.
