...
velnorctl telemetry
velnor-tools workflow-monitor --repo OWNER/REPO --run-id RUN_ID
```
...
### Workflow generation and monitoring

`velnor-workflow` is the Velnor-owned static generator and runtime used by
generated GitHub Actions jobs. It writes only its owned files and keeps the
declarative execution contract in `.github/ci/project.toml`:

```sh
velnor-workflow REPOSITORY --runners both
velnor-workflow plan --config .github/ci/project.toml
velnor-workflow run --scope affected
velnor-workflow policy --workflow-root .
velnor-workflow release verify-tag
```

`plan` publishes the CI-selection plan to stdout and `$GITHUB_OUTPUT`, plus a
selection file and an expected-work file when their environment paths are set.
`CURRENT` output fields (`crates/velnor-workflow/src/runtime.rs:1740-1761,1778-1822`;
schema-2 runtime adds JSON `units`, `unit_ids`, `plan_digest`, and `excluded`
at `crates/velnor-workflow/src/s2/runtime.rs:1851-1886`):

| Field | Shape | Meaning |
| --- | --- | --- |
| `scope` | `affected` or `full` | Scope the event resolved to. |
| `base_sha`, `head_sha` | SHAs | Diff range the plan selected for. |
| `units` | CSV unit ids (schema-1) or plan JSON (schema-2) | Verify set that must run: changed units plus dependents. |
| `unit_ids` | CSV unit ids (schema-2 only) | Same verify set as CSV for the selection file. |
| `full_units` | CSV unit ids | The verify set; every scheduled unit runs full scope. |
| `plan_digest` | lowercase 64-hex SHA-256 digest (schema-2 only) | Stable identity over the planned units and exclusions. |
| `excluded` | JSON (schema-2 only) | Units excluded with per-unit provider and reason. |
| `<kind>_matrix` | JSON array of `{unit, label}` | Per-kind caller matrix; empty for unselected kinds. |
| `fallback_reason` | string, on fallback and opaque-narrowed selections | Why a fallback widened the selection: the full set, or the openly-opaque subset an unmatched path conservatively selects; unmatched-path reasons also name the missing narrowing contract. Absent for proven-narrow plans. |
| `planned_no_work` | `true`, only on no-work | Presence-only marker: `true` or absent, never `false`. |
| `no_work_reason` | string, only on no-work | Why zero units were selected. |

`run --scope affected` executes the units the selection file names; a
validated empty selection reports
`no_work_reason=CI selection artifact selects zero workload units; nothing to run`
and exits successfully. Selection semantics, the reason channels, and the
expected-work aggregate live in [change-aware
planning](/concepts/change-aware-planning).

`velnor-tools workflow-monitor` joins authoritative GitHub run state with
read-only local Velnor observations. GitHub remains authoritative for the
terminal result; local readiness, runner, slot, job, event, log, and outbox
observations are evidence only:

```sh
velnor-tools workflow-monitor \
  --repo OWNER/REPO \
  --run-id RUN_ID \
  --instance primary \
  --timeout 30m \
  --poll 2s \
  --evidence-dir .velnor/evidence
```

The monitor never mutates the Velnor control plane and never treats local
completion as proof of GitHub completion. GitHub credentials remain in the
...
`configure` supports repository, organization, and enterprise URLs. It reads the
registration credential from the `GITHUB_TOKEN` environment variable; the CLI
does not accept a command-line credential argument. Relevant options are:

| Option | Meaning |
| --- | --- |
| `--url` | GitHub repository, organization, or enterprise URL. |
| `GITHUB_TOKEN` | Environment-only registration credential. Keep it out of command arguments and shared command history. |
| `--name` | Runner name. |
...
Labels are sorted and deduplicated. macOS labels are rejected; execution is Linux-only. The target MVP label additions are `hetzner-sentry-ci`, `ubuntu-latest`, and `velnor-target-mvp`; `ubuntu-24.04` is reserved for GitHub-hosted workflow selection and rejected on Velnor self-hosted runners.
...
The allowlist is closed. A JavaScript, Docker, or composite action that is not on it is rejected at planning, and there is no generic JavaScript action runtime open to workflows. See [what Velnor does not support](/guides/execution#what-velnor-does-not-support).

## Credential boundaries

The operator PAT is used for registration, runner/group lookup, and runner cleanup. It is not a job credential.

GitHub supplies job-scoped credentials in the acquired job payload. Velnor injects them only after admission, including the job token, Actions runtime token, OIDC material, and cache/results credentials. A job carrying GitHub user secrets is rejected outright unless the pool is `trusted` (`VELNOR_TRUST_SCOPE=trusted`, which is not the default) and the job's own trust class is trusted; see [Trust scope](/reference/trust-scope). MicroVM guests must be credential-free at readiness; credentials are delivered only for the job session.
