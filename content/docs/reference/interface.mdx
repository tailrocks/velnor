...
| `units` | CSV unit ids (schema-1) or plan JSON (schema-2) | Selected units that must run. |
| `unit_ids` | CSV unit ids (schema-2 only) | Same affected set as CSV for the selection file. |
| `full_units` | CSV unit ids | Changed-plus-dependents subset that runs full scope. |
...
| `fallback_reason` | string, on fallback and opaque-narrowed selections | Why a fallback widened the selection: the full set, or the opaque subset an unmatched path conservatively selects; unmatched-path reasons also name the missing narrowing contract. Absent for proven-narrow plans. |
