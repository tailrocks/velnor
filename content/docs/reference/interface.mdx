...
| `fallback_reason` | string, on fallback and opaque-narrowed selections | Why a fallback widened the selection: the full set, or the opaque subset an unmatched path conservatively selects; absent for proven-narrow plans. |
