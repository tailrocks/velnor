...
current step's own work — its host process tree, its Docker-action sidecar, its
BuildKit builder — dies at `Requested`; the job and service containers survive
to `Forced` so `always()` and `cancelled()` post steps still have a container.
Cancellation signals; removal belongs to teardown.

It is not fully equivalent yet. Only host process groups register as
termination targets on the live path, so containers are killed by name rather
than through the ladder, and the Firecracker path sends no vsock cancel to the
guest. Velnor still drains publishers, collects available state, and attempts
terminal completion, but operators must not assume identical graceful step
cancellation or cleanup timing.
