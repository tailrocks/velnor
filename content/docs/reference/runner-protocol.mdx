...
This reference describes the Native runner protocol Velnor currently
implements. It is for engineers investigating registration, work delivery, job
execution, progress, completion, and interruption behavior.
...
The Native path is one JIT V2 session per ephemeral runner slot. A daemon may
operate several slots, but each slot has its own runner identity, broker
session, lease, and job ownership.

The Scale Set adapter classifies incoming offers from event and
owner/repository fields, then delegates acquired jobs to the official Actions
runner. It does not use the Native action-closure admission path. See
...
On the native lane, the acquired payload supplies the workflow/job identity,
steps, actions, variables, masks, services, outputs, permissions, runtime
URLs, and job credentials needed to build the job execution plan (the validated native-job plan, not the CI-selection plan in [change-aware planning](/concepts/change-aware-planning)).
...
## Native admission before side effects

The Native runner parses and validates the complete action closure before it
performs checkout, downloads, credential injection, cache mutation, service
startup, or executor work. Admission checks include:
...
1. record the job's terminal conclusion, before the payload is serialized;
2. write the bounded payload to the job- and generation-specific outbox file
   atomically, then journal the intent and its SHA-256 — payload first, because
   the reverse order left a completing job with no replayable bytes;
3. take the one durable send claim; a second concurrent send is unrepresentable;
4. send the exact payload to the Run Service;
5. record the remote terminal acknowledgement;
6. remove the outbox entry;
7. clear the local in-flight marker after completion handling.

If the process or network fails between these steps, recovery replays the same
payload under the same claim and preserves ownership evidence. It does not
fabricate a new result from partial memory. Missing, non-regular, symlinked,
oversized, or orphaned entries fail closed and are retained or quarantined for
inspection.

The retry budget is bounded. An entry stops being retried when the run service
permanently refuses it (any 4xx except 408, 409, and 429), after eight
attempts, or six hours after the intent was committed — whichever comes first.
Every other status, and any error carrying no HTTP status at all, is transient:
not knowing the remote's answer is exactly the case where retrying is the only
correct move.

An entry whose budget is spent is *abandoned*: the payload is removed and the
slot freed, GitHub is never told the job finished, and the run service will
time the job out. The terminal state is proven from the journal rather than
asserted by a caller, and the reason is preserved permanently as one of "the
run service permanently refused this completion payload", "the durable
completion send budget is spent", or "the completion resolution deadline
passed".
...
| Drain or service stop | Native idle slots exit at a poll boundary; native active jobs drain, then service-stop escalation may terminate them. | GitHub may see cancellation or infrastructure failure if the native stop deadline expires. |
...
| Completion request failure | Retry transient failures under a bounded budget; preserve the outbox until acknowledgement, terminal remote observation, or budget exhaustion. | Inspect outbox age and remote job state before retrying manually. |
| Completion budget exhausted | Abandon the entry, free the slot, record the reason in the journal. | GitHub was never told; it will time the job out. Read the journal's unresolvable completions. |
...
Velnor models the same split: one cancellation token per job with `Requested`
and `Forced` levels, and a `SIGINT → SIGTERM → SIGKILL` ladder per target. The
current step's own work — its host process tree and Docker-action sidecar —
dies at `Requested`; the job and service containers survive to `Forced` so
`always()` and `cancelled()` post steps still have a container. Cancellation
signals; removal belongs to teardown.

The live native path registers host process groups, job and service
containers, and Docker-action sidecars. For Firecracker, an active cancellation
token registers the jailer process group as the termination fallback; a guest
`Cancel` hook is registered when a vsock cancel handle is also available.
Containers and process groups use the signal ladder; the guest hook attempts
graceful cancellation and teardown owns resource removal. Velnor still drains
publishers, collects available state, and attempts terminal completion.
