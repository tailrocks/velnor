...
Velnor's Docker path now follows the same shape: one cancellation token per job,
requested from the broker message, re-evaluating step conditions so `always()`
and `cancelled()` work still runs, and escalating to termination after a grace
taken from the server's message. The Firecracker path does not yet provide
end-to-end equivalence, because the guest is not driven from that token.
