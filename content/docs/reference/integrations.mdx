...
There is no automatic fallback between backends. MicroVM supports a narrower action and runtime surface, named explicitly in [MicroVM is narrower than Docker](/guides/execution#microvm-is-narrower-than-docker), and does not provide end-to-end cancellation parity with Docker.
...
Broker polling errors back off; authentication failures trigger credential/session recovery. A lost runner registration cancels the affected job. Active jobs have a separate broker cancellation poller. Docker cancellation terminates the job's processes and containers through a signal ladder without removing them; MicroVM cancellation is not wired end to end. See [Cancellation](/guides/execution#cancellation).

Completion is the durable boundary: Velnor records the terminal conclusion, writes the payload, journals the intent, takes one send claim, submits it, records remote acknowledgement, and only then removes the outbox entry. If the process fails before acknowledgement, recovery retries the same payload under the same claim. If GitHub has already accepted completion, a terminal remote observation prevents treating it as a new job result. The retry budget is bounded — a permanent refusal, eight attempts, or six hours — and a spent budget abandons the entry without telling GitHub anything, recording the reason in the journal. See [Completion durability](/guides/execution#completion-durability).
