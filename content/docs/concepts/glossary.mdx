...
| Trust scope | The operator-selected boundary controlling whether a job may use the host Docker socket, user secrets, privileged options, and which store partition it shares. Defaults to `untrusted`; only `trusted` grants capability. Never derived from the job payload. |
| Store catalog | The single constructor for every persistent store path, so a store cannot be written at one path and reclaimed at another. |
| Host budget | The machine's real CPU and memory capacity, derived as the tightest of every applicable constraint, and divided by slot count into a per-slot share. |
| Termination ladder | `SIGINT → SIGTERM → SIGKILL`, applied to each registered cancellation target at the level it declares. |
