...
sequenceDiagram
    ...
    A-->>V: Validated job execution plan or rejection
...
| Workflow generation | `velnor-workflow` | Static repository scan, owned workflow/config rendering, affected-unit planning ([change-aware planning](/concepts/change-aware-planning)), policy, and release checks. It does not schedule GitHub work. |
