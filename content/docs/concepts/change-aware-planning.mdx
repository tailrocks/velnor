...
(`crates/velnor-workflow/src/runtime.rs:3026-3042`), and the plan job checks
...
Owned wins over opaque (`crates/velnor-workflow/src/reuse.rs:746-748`): a
...
(`crates/velnor-workflow/src/reuse.rs:717-731`).
...
(`crates/velnor-workflow/src/reuse.rs:1159-1165`):
...
(`crates/velnor-workflow/src/reuse.rs:280-282`). Every selected unit carries an
explanation naming its paths or the edge that pulled it in, and every recorded
input names its consumer
(`crates/velnor-workflow/src/reuse.rs:424-488`).
...
1. A version-bump-only change selects the configured allowlist plus the
   ...
   (`crates/velnor-workflow/src/runtime.rs:2798-2823`).
2. Workspace checks join only when they share a Cargo lockfile root with a
   ...
   affected-only (`crates/velnor-workflow/src/runtime.rs:2907-2932`).
3. Lane admission narrows the selection to units the admitted lanes can run;
   ...
   (`crates/velnor-workflow/src/runtime.rs:1967-2025`).

## Event behavior

`CURRENT`: the event decides the scope before any diff runs
(`crates/velnor-workflow/src/runtime.rs:1911-1942`).
...
(`crates/velnor-workflow/src/reuse.rs:1088-1144`):
...
(`crates/velnor-workflow/src/reuse.rs:830-833`), so a changed script under a
...
unit-wide (mixed claims fail) and contradicts `unresolved`. The claim is
trusted, not proven: generation validates its shape, never its truth, so a
false `complete` silently skips verification for paths the unit actually
reads. Declare `complete` only for mechanically-enumerable read sets — an
exact glob closure the scan plus the contract spells in full, with no
dynamic includes, glob expansion, or runner-resolved inputs outside it.
Every exclusion stays auditable: the verdict names the skipped units in
`closed_excluded`, on the plan log and in the `select` JSON alike. Without
...
would narrow it (`crates/velnor-workflow/src/reuse.rs:774-791`).
...
(`crates/velnor-workflow/src/runtime.rs:1742-1758`) — and publishes the
...
- `velnor-workflow select --config PATH --base BASE --head HEAD [--scope SCOPE]`
  ...
  `prereq_inputs`, `fallback_full`, per-unit `explanations`,
  `fallback_reason` only on fallback, and `closed_excluded` only when a
  closed-world contract excluded units
  (`crates/velnor-workflow/src/runtime.rs:649-663`,
  `crates/velnor-workflow/src/runtime.rs:1348-1353`,
  `crates/velnor-workflow/src/reuse.rs:263-264`). This is the why-run /
  why-skipped oracle: every unit names its matched paths or its pulling edge,
  every recorded input names its consumer, and every closed-world skip
  names its excluded units.
...
- An unproven empty selection fails before any artifact escapes: no marker is
  ...
  (`crates/velnor-workflow/src/runtime.rs:1816-1818`).
- `run --scope affected` against a validated empty selection reports
  ...
  (`crates/velnor-workflow/src/runtime.rs:2443-2450`).
- The aggregate passes explicit no-work only when the marker is set and zero
  ...
  marker, fails closed
  (`crates/velnor-workflow/src/reuse.rs:1920-1932`,
  `crates/velnor-workflow/src/reuse.rs:4456-4487`).
...
- `validate_reuse` reuses a prior successful result only when every rule
  ...
  checks (`crates/velnor-workflow/src/reuse.rs:1650-1719`). Artifact presence
  alone never reuses: `Evidence` has no name field to match
  (`crates/velnor-workflow/src/reuse.rs:1600-1614`).
- The aggregate scores a `reused_from` result against its producing evidence
  ...
  (`crates/velnor-workflow/src/reuse.rs:1879-1890`). Prerequisites outside
  ...
  (`crates/velnor-workflow/src/reuse.rs:2138-2184`).
...
- Trusted events (`push`, `schedule`, `merge_group`) require full scope at
  ...
  (`crates/velnor-workflow/src/runtime.rs:2393-2416`).
- The selection artifact is bound to its plan: version-checked, SHA-bound
  ...
  (`crates/velnor-workflow/src/runtime.rs:2417-2440`).
- The expected-work file carries the plan's `base_sha`/`head_sha`; the
  ...
  (`crates/velnor-workflow/src/reuse.rs:2317-2341`).
- Reuse carries trust: producing evidence has a trust class, and reuse
  requires it to satisfy the request
  (`crates/velnor-workflow/src/reuse.rs:1707-1714`). Units declare their own
...
(`crates/velnor-workflow/src/runtime.rs:2006-2014`). `select` takes no
...
1. Read the plan outputs: `scope`, `units`, `full_units` (the verify set),
   ...
   behind the verify set, and its `closed_excluded=` line names the
   closed-world units an unmatched path skipped
   (`crates/velnor-workflow/src/runtime.rs:1853-1857`).
...
4. A skipped unit under a no-work plan is proven irrelevant: re-run `select`
   ...
   exclude the path. That proof is only as true as the `complete` claim
   behind it — a false claim silently skips verification for paths the
   unit actually reads — so confirm the skip in the plan log's
   `closed_excluded=` line (or the `select` JSON's `closed_excluded`
   field) and re-check the contract against a mechanically-enumerable
   read set before trusting it
   (`crates/velnor-workflow/src/reuse.rs:683-705`).
5. Opacity suspects, in likelihood order: a `node`/`bun` unit running package
   commands (`crates/velnor-workflow/src/reuse.rs:495-497`); an opaque-runner
   token (`bun`, `npm`, `node`, `npx`, `python`, `mise`, `make`, shells, and
   the rest of `crates/velnor-workflow/src/reuse.rs:504-534`); expansion or
   ...
   (`crates/velnor-workflow/src/reuse.rs:834-927`).
...
- Prove the fix on the live `plan` path (`selection_for_diff`), not only on
  ...
  (regression shape:
  `crates/velnor-workflow/src/runtime.rs:6822-6835`).
- Keep the command-vocabulary conformance table green: every emitted command
  ...
  (`crates/velnor-workflow/src/reuse.rs:3871-3971`).
...
reporting; closed-world contracts that exclude provably-unread paths and
name the excluded units;
...
- `node`/`bun` units stay opaque (paths they do not own conservatively select the opaque units)
  ...
  (`crates/velnor-workflow/src/reuse.rs:490-497`); urgent gaps go through
...
- The `select` CLI still parses a non-NUL `git diff --name-status` stream
  (`crates/velnor-workflow/src/runtime.rs:1363-1385`); `-z` parity with the
  plan path (`crates/velnor-workflow/src/runtime.rs:3026-3042`) is a
  follow-up.
