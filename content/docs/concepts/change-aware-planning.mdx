...
(`crates/velnor-workflow/src/reuse.rs:650-706`):
...
(`crates/velnor-workflow/src/reuse.rs:1076-1080`):
...
(`crates/velnor-workflow/src/reuse.rs:1009-1059`):
...
(`crates/velnor-workflow/src/reuse.rs:745-748`).
...
5. Opacity suspects, in likelihood order: a `node`/`bun` unit running package
   ...
   (`crates/velnor-workflow/src/reuse.rs:749-842`).
...
- Prove the fix on the live `plan` path (`selection_for_diff`), not only on
  ...
  (regression shape:
  `crates/velnor-workflow/src/runtime.rs:6659-6672`).
- Keep the command-vocabulary conformance table green: every emitted command
  ...
  (`crates/velnor-workflow/src/reuse.rs:3300-3360`).
