...
(`crates/velnor-workflow/src/runtime.rs:2884-2893`), and the plan job checks
...
(`crates/velnor-workflow/src/s2/runtime.rs:2684-2790`).

## Owned, Unknown, Irrelevant

`CURRENT`: each changed path gets exactly one verdict from `classify_path`
(`crates/velnor-workflow/src/reuse.rs:598-636`):
...
- `UNKNOWN` — no unit owns the path but some unit is opaque (its commands
  resist proof), or the path itself cannot be matched safely. The plan falls
  back to the full set with an explicit reason naming the consulted sources:
  declared reads, watch globs, and command read-globs.

Owned wins over opaque (`crates/velnor-workflow/src/reuse.rs:624-626`): a
...
(`crates/velnor-workflow/src/reuse.rs:599-614`).
...
(`crates/velnor-workflow/src/reuse.rs:1006-1010`):
...
(`crates/velnor-workflow/src/reuse.rs:256-258`). Every selected unit carries an
explanation naming its paths or the edge that pulled it in
(`crates/velnor-workflow/src/reuse.rs:355-405`).
...
1. A version-bump-only change selects the configured allowlist plus the
   ...
   (`crates/velnor-workflow/src/runtime.rs:2647-2669`).
2. Workspace checks join only when they share a Cargo lockfile root with a
   ...
   affected-only (`crates/velnor-workflow/src/runtime.rs:2734-2756`).
...
(`crates/velnor-workflow/src/reuse.rs:939-989`):
...
`reads` table mapping unit ids to `{ paths, reason }` entries
(`crates/velnor-workflow/src/primitives/mod.rs:599-708`):

- Every entry must spell exactly `paths` (an array of valid watch globs) and
  `reason` (a non-empty string auditing the claim). A typo'd key, a missing
  field, an invalid glob, or an empty reason fails generation closed.
...
- The `watch-graph` primitive unions the paths into the unit's watch set, so
  the runtime contract is unchanged: no schema bump, and selection sees owned
  paths through the ordinary watch matcher
  (`crates/velnor-workflow/src/primitives/watch.rs:124-128`).

Declared coverage composes with opacity: a declared path still narrows to its
owners even when other units stay opaque
(`crates/velnor-workflow/src/reuse.rs:675-678`).
...
the reason-bearing fields are `fallback_reason` (present only on fallback),
`planned_no_work=true` (present only on no-work), and `no_work_reason`.
...
- The aggregate passes explicit no-work only when the marker is set and zero
  ...
  marker, fails closed
  (`crates/velnor-workflow/src/reuse.rs:1610-1628`,
  `crates/velnor-workflow/src/reuse.rs:3579-3598`).
...
- `validate_reuse` reuses a prior successful result only when every rule
  ...
  checks (`crates/velnor-workflow/src/reuse.rs:1460-1529`). Artifact presence
  alone never reuses: `Evidence` has no name field to match
  (`crates/velnor-workflow/src/reuse.rs:1410-1424`).
- The aggregate scores a `reused_from` result against its producing evidence
  ...
  (`crates/velnor-workflow/src/reuse.rs:1707-1717`).
...
- The expected-work file carries the plan's `base_sha`/`head_sha`; the
  ...
  (`crates/velnor-workflow/src/reuse.rs:2134-2154`).
- Reuse carries trust: producing evidence has a trust class, and reuse
  requires it to satisfy the request
  (`crates/velnor-workflow/src/reuse.rs:1517-1524`). Units declare their own
...
3. A full run with a `fallback_reason` names the consulted sources and the
   first unprovable input: an unmatched path under an opaque unit, a global
   or unknown `.github/` path, an empty base, an unavailable diff, or an
   unparseable change entry.
...
5. Opacity suspects, in likelihood order: a `node`/`bun` unit running package
   commands (`crates/velnor-workflow/src/reuse.rs:419-421`); an opaque-runner
   token (`bun`, `npm`, `node`, `npx`, `python`, `mise`, `make`, shells, and
   the rest of `crates/velnor-workflow/src/reuse.rs:435-458`); expansion or
   ...
   (`crates/velnor-workflow/src/reuse.rs:679-771`).
...
- Prove the fix on the live `plan` path (`selection_for_diff`), not only on
  ...
  (regression shape:
  `crates/velnor-workflow/src/runtime.rs:6651-6664`).
- Keep the command-vocabulary conformance table green: every emitted command
  ...
  (`crates/velnor-workflow/src/reuse.rs:2998-3058`).
...
`CURRENT` on this branch: the Owned/Unknown/Irrelevant classifier over watch
globs, command read-globs, and declared reads; the `.github/` contract table;
...
- `node`/`bun` units stay opaque (paths they do not own fall back to full)
  until the scan proves whole-root ownership for those units
  (`crates/velnor-workflow/src/reuse.rs:414-421`); urgent gaps go through
  declared `reads` meanwhile.
- The `select` CLI still parses a non-NUL `git diff --name-status` stream
  ...
  plan path (`crates/velnor-workflow/src/runtime.rs:2884-2900`) is a
  follow-up.
