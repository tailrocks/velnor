...
fn open_policy_directory_fd(
    ...
    // Proof: the static string `"/"` contains no NUL byte.
    #[allow(clippy::expect_used, reason = "static path has no NUL")]
...
fn fleet_generate(args: FleetGenerateArgs) -> Result<()> {
    ...
    for (org, (action, bytes)) in &plan {
        ...
        match action {
            PolicyFileAction::Written => {
                // Proof: the planner builds `Written` only for ledger orgs
                // with `Some(bytes)`, and `by_org` covers the same ledger.
                #[allow(clippy::expect_used, reason = "written actions carry bytes and policy")]
                let bytes = bytes
                    .as_deref()
            ...
            PolicyFileAction::Skipped => {
                // Proof: the planner builds `Skipped` only for ledger orgs
                // with `Some(bytes)` whose on-disk bytes match, so the org is
                // in `by_org` and in the same `existing` map read here.
                #[allow(
                    clippy::expect_used,
                    reason = "skipped actions carry policy bytes and file"
                )]
            ...
            PolicyFileAction::Removed => {
                // Proof: the planner builds `Removed` only for orgs read from
                // the same `existing` map, so the file entry is `Some`.
                #[cfg(unix)]
                #[allow(clippy::expect_used, reason = "removed actions have an existing file")]
