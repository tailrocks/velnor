...
fn fixed_goal_classes() -> Result<BTreeMap<String, GeneratedCallerClass>> {
    ...
    for repository in GOAL_FLEET_REPOSITORIES {
        ...
        if map.insert(repository.to_string(), class).is_some() {
            bail!(
                "compiled fixed-goal scope contains duplicate repository {}",
                repository
            );
        }
    }
...
fn generated_caller_sample(
    ...
    if !bytes.starts_with(GENERATED_WORKFLOW_HEADER.as_bytes()) {
        return Ok(None);
    }
...
pub fn audit_ci(args: AuditCiArgs) -> Result<()> {
    ...
    let accepted_auxiliary = if args.estate.is_some() {
        Some(canonical_auxiliary_manifest(&args.repo_path)?)
    } else {
        None
    };
        ...
        match concern.classification {
            ...
            ConcernClassification::Required | ConcernClassification::Applicable => {
                ...
                for implementation in &concern.implementations {
                    ...
                    // Generated callers delegate these concerns to Velnor-owned
                    // local workflows. Their inline job IDs and markers are
                    // intentionally absent; `audit_generated_caller` validates
                    // the closed local delegation contract once for the whole file.
...
fn is_generated_caller(text: &str) -> bool {
    text.starts_with(GENERATED_WORKFLOW_HEADER)
}
...
fn audit_generated_caller(
    ...
    let Some(jobs) = object_get(yaml, "jobs").and_then(Value::as_mapping) else {
        findings.push(Finding::error(
            ...
            file,
            "$.jobs",
            "generated caller must declare the closed local-workflow and aggregator job set",
        ));
        return;
    };
    ...
    for contract in GENERATED_OWNER_LANE_CONTRACTS {
        ...
        let Some(uses) = mapping_get(job, "uses").and_then(Value::as_str) else {
            findings.push(Finding::error(
                ...
                "owner job must call its Velnor-owned local reusable workflow",
            ));
            continue;
        };
        let Some(local_workflow) = uses.strip_prefix(LOCAL_WORKFLOW_PREFIX) else {
            findings.push(Finding::error(
                "generated-caller",
                file,
                format!("$.jobs.{owner}.uses"),
                "owner job must call a local Velnor workflow generated by velnor-workflow",
            ));
            continue;
        };
        let Some(class) = local_workflow
            .strip_prefix("ci-")
            .and_then(|workflow| workflow.strip_suffix(".yml"))
        else {
            findings.push(Finding::error(
                "generated-caller",
                file,
                format!("$.jobs.{owner}.uses"),
                "owner callable must use local ci-<class>.yml generated by velnor-workflow",
            ));
            continue;
        };
    ...
    if classes.len() != 1 {
        findings.push(Finding::error(
            ...
            file,
            "$.jobs",
            "all three local workflow calls must select one identical repository class",
        ));
    }
...
fn audit_lane_selector(file: &str, yaml: &Value, text: &str, findings: &mut Vec<Finding>) {
    ...
    // Generated callers delegate the selected lane to a Velnor-owned local
    // reusable workflow. Their caller YAML intentionally contains no runner
    // labels; `audit_generated_caller` closes the local workflow, class, input,
    // and Velnor-default forwarding contract, while generator/release audits
    // prove the generated workflow's real Velnor+GitHub expansion.
...
jobs:
  jackin-project:
    uses: ./.github/workflows/ci-code.yml
    with:
...
"#;
    ...
    #[test]
    fn generated_caller_uses_closed_velnor_local_workflow_contract() {
        ...
        let tampered = GENERATED_CALLER.replace(
            ...
            "attacker/.github/workflows/ci-code.yml",
        );
...
"#,
    ...
    #[test]
    fn auxiliary_contract_digest_detects_source_drift() {
        let accepted = reviewed_auxiliary_manifest();
