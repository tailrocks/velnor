...
fn audit_generated_caller(file: &str, text: &str, yaml: &Value, findings: &mut Vec<Finding>) {
    ...
    if !text.contains("github.event_name == 'schedule' && 'both'") {
        findings.push(Finding::error(
            "generated-caller",
            file,
            "$.jobs.*.with.lane",
            "weekly schedule must select both independent lanes",
        ));
    }
    if object_get(yaml, "on")
        ...
        .is_none()
    {
        findings.push(Finding::error(
            ...
            "generated caller must expose the closed optional lane selector",
        ));
    }
}
