...
async fn check_runner_reference(root: &Path) -> Result<()> {
    ...
    if reference != latest {
        bail!(
            "actions/runner reference drift: pinned {reference}, latest {latest}\nRefresh docs/reference/runner-protocol.md and re-audit V2 source anchors."
        );
    }
...
fn pinned_reference(root: &Path) -> Result<String> {
    let path = root.join("docs/reference/runner-protocol.md");
    ...
    vec![
        ...
        (
            ...
            vec![
                ...
                ("Docker build action", "docker/build-push-action@"),
        ...
        (
            ...
            vec![
                ...
                (
                    "pages artifact writer expression",
                    "name: ${{ matrix.config.writer",
                ),
        ...
        (
            ...
            vec![
                ("matrix config lanes", "matrix.config"),
                ("matrix runner", "matrix.config.runner"),
                ("renovate action", "renovatebot/github-action@"),
        ...
        (
            ...
            vec![
                ...
                ("cargo-nextest via mise", "nextest-rs/nextest/cargo-nextest"),
            ],
        ),
        (
            "docker-bake.hcl",
