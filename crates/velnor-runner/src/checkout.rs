...
pub(crate) fn checkout_plan(
    ...
    let clone_url = checkout_clone_url(checkout_repository.as_deref(), &self_repository, &server_url)?;
    ...
    R: CommandRunner,
{
    ...
    let (linked, bytes) = link_object_store(
            ...
            match fs::hard_link(&origin, &target) {
                ...
                Err(_) => {
                    fs::copy(&origin, &target).with_context(|| {
                        format!("copy git object {} to {}", origin.display(), target.display())
                    })?;
                }
            }
            linked += 1;
...
fn write_fetch_head(
    ...
    fs::write(&path, format!("{sha}\t\t{kind} '{git_ref}' of {clone_url}\n"))
...
fn checkout_server_url(job: &AgentJobRequestMessage, self_repository: &RepositoryResource) -> String {
    self_clone_url(self_repository)
        .ok()
        .and_then(|url| {
