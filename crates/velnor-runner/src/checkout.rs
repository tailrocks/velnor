    ...
    let mirror = match mirror_store {
        ...
        Some(store) => match crate::git_mirror::ensure_mirror(
            ...
            Err(error) => {
                ...
                return Err(
                    error.context(format!("prepare git mirror for {}", plan.clone_url))
                );
            }
        },
        None => None,
...
mod tests {
    ...
    impl RepoFixture {
        fn new() -> Self {
            ...
            let sha = git(vec!["-C".into(), path_arg(&work), "rev-parse".into(), "HEAD".into()]);
    ...
    fn checkout_hydrates_the_workspace_without_copying_object_bytes() {
        ...
        let head = runner
            .run(
                "git",
                &["-C".into(), path_arg(&workspace), "rev-parse".into(), "HEAD".into()],
            )
            .unwrap();
    ...
    fn checkout_fails_when_the_mirror_cannot_be_prepared() {
        ...
        plan.version = Some("0000000000000000000000000000000000000000".into());
        let mut runner = ProcessCommandRunner;
    ...
    fn concurrent_checkouts_of_one_commit_fetch_the_mirror_once() {
        ...
        // Every leg wants the same commit. The first one to reach the mirror
        // fetches it; the rest find it already there and never hit the origin.
        // Before the short-circuit this was six serialized full fetches.
        let observed = fetches.load(std::sync::atomic::Ordering::SeqCst);
        assert!(
            observed <= 2,
            "expected at most one contended fetch, saw {observed}"
        );
        for index in 0..6 {
