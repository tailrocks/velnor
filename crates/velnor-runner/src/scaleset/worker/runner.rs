...
impl RunnerSpec {
    ...
    pub fn write_env_file(&self) -> Result<PathBuf> {
        ...
        let mut contents = format!("{JIT_CONFIG_ENV}={}\n", self.jit_config);
        if let Ok(token) = std::env::var("GITHUB_TOKEN") {
            contents.push_str(&format!("GITHUB_TOKEN={token}\nGH_TOKEN={token}\n"));
        }
