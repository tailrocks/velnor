...
fn jobs_slice_cpu_quota_is_host_scaled_and_fail_closed() {
    ...
    assert!(postinst.contains(r#"systemctl daemon-reload \"#));
    assert!(postinst
        .contains("|| fail \"systemd refused to reload the host-scaled velnor-jobs.slice quota\""));
    assert!(postinst.contains(r#"systemctl show \"#));
    assert!(postinst.contains("--property=CPUQuotaPerSecUSec --value velnor-jobs.slice"));
    assert!(postinst.contains("no effective CPUQuota after daemon-reload"));
    assert!(postrm.contains("systemctl is-active --quiet velnor-jobs.slice"));
    assert!(postrm.find("systemctl is-active --quiet").unwrap() < postrm.find("rm -f").unwrap());
...
fn jobs_slice_quota_writer_executes_and_rejects_invalid_cpu_detection() {
    ...
    let success_dir = TempDir::new("jobs-slice-success");
    ...
    let success = Command::new("sh")
        .arg("-c")
        .arg(success_script)
        .env("TEST_DROPIN_DIR", success_dir.path())
        .output()
        .unwrap();
    assert!(
    ...
    let failure_dir = TempDir::new("jobs-slice-failure");
