...
use velnor_runner::scaleset::worker::{
    HomogeneousProfile, PinnedImage, ToolContentAttestation, ToolContentExpectation,
    ToolContentHook, WorkerOutput, WorkerRunner,
};
use velnor_runner::scaleset::{
    AcquireBatchStore, CapacityLedger, ClientSession, DaemonDefaults, DaemonWorkerLane,
    DemandState, DemandStore, IdlePolicy, LaneConfig, Listener, LoopConfig, MessageSessionClient,
    Metrics, Processor, ProcessorConfig, ProvisionImages, ProvisionIntentStore, RegistrationPlan,
    RetryPolicy, ScaleSetClient, ScaleSetDaemon, SessionStore, SharedLedger, SystemInfo,
    WorkerRegistry, MAX_ACQUIRE_BATCH,
};
use wiremock::{
...
async fn daemon_serves_one_job_end_to_end() {
    ...
    // runner env; no host socket ever entered an argv.
    let creates = docker.creates();
    assert_eq!(creates.len(), 2, "one dind + one runner: {creates:?}");
    let jit_env: Vec<_> = docker.seen_matching("ACTIONS_RUNNER_INPUT_JITCONFIG=jit-blob-e2e");
    assert_eq!(jit_env.len(), 1);
    assert!(
    ...
    let slug = format!("s{SCALE_SET_ID}-velnor-{SCALE_SET_ID}-{REQUEST_ID}");
    let runner_log = dir
    ...
    assert!(
        !docker.has_container(&format!("velnor-scaleset-runner-{slug}"))
            && !docker.has_container(&format!("velnor-scaleset-dind-{slug}")),
...
fn slug_for(request_id: i64) -> String {
    format!("s{SCALE_SET_ID}-velnor-{SCALE_SET_ID}-{request_id}")
}

fn runner_container_for(request_id: i64) -> String {
    format!("velnor-scaleset-runner-{}", slug_for(request_id))
}

fn dind_container_for(request_id: i64) -> String {
    format!("velnor-scaleset-dind-{}", slug_for(request_id))
}
...
async fn crash_with_dead_workers_fails_explicitly() {
    ...
    wait_for(&defaults.state_db, &ledger_path, "job started", |_, _| {
        WorkerRegistry::open(&defaults.state_db)
            .unwrap()
            .get(&format!(
                "{SCALE_SET_ID}/velnor-{SCALE_SET_ID}-{REQUEST_ID}"
            ))
            .unwrap()
