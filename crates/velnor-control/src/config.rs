...
impl SidecarLock {
    fn acquire(config_path: &std::path::Path) -> Result<Self, ConfigError> {
        ...
        let file = open_lock(&path.parent, &lock_name, true)?.ok_or_else(|| {
    ...
    fn acquire_existing_shared(config_path: &std::path::Path) -> Result<Option<Self>, ConfigError> {
        ...
        let lock_name = lock_file_name(&path.name);
        let Some(file) = open_lock(&path.parent, &lock_name, false)? else {
            return Ok(Some(Self { _file: None, path }));
        };
        file.lock_shared()
            .map_err(|error| ConfigError::Io(error.to_string()))?;
        validate_existing_file(&path.parent, &path.name, "context file")?;
...
mod tests {
    ...
    #[test]
    fn context_store_reads_atomically_when_sidecar_lock_is_absent() {
        ...
        assert_eq!(
            store.list().expect("read without sidecar"),
            vec![test_context()]
        );
    }
