//! Daemon-side control-plane application services.
//!
//! Transport adapters are isolated here by Plan 067; this crate owns control
//! services over the shared model types and physical-action journal, and never
//! depends on Clap or runner internals.

/// Marker service seam; Plan 067 fills in the versioned control API.
pub const CONTROL_SERVICE: &str = "velnor-control";

pub mod application;
pub mod config;
pub mod diagnostics;
pub mod github;
pub mod journal;
pub mod lifecycle;
pub mod logs;
pub mod observation;
pub mod ports;
pub mod query;
pub mod storage;
pub mod store;
pub mod supersession;
pub mod telemetry;

#[cfg(test)]
mod tests {
    #[test]
    fn service_marker_is_stable() {
        assert_eq!(super::CONTROL_SERVICE, "velnor-control");
    }
}
