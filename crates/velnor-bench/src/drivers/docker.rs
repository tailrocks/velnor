...
fn remove_owned_image_tag(
    ...
    let invocation = context
        ...
        .context("docker owned image tag removal")?;
    if invocation.ok()
        || invocation
            .stderr
            .to_ascii_lowercase()
    ...
    bail!(
        "docker owned image tag removal failed with exit code {}: {}",
        invocation.code,
...
impl DockerWorkload {
    ...
    fn cleanup_owned_images(&mut self, context: &mut Context) -> Result<()> {
        ...
        for image in std::mem::take(&mut self.owned_images) {
            if let Err(error) =
                remove_owned_image_tag(context, &image.tag, &image.id, &self.owner_token())
            {
