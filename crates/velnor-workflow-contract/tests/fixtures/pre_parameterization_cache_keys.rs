...
//! (unit, provider) job as the generator rendered them BEFORE the kind reusables
...
//! tree would have rendered). D2 namespaced the bundle, mbx, and docker-seed
//! keys by provider, platform, and trust (`-<provider>-<platform>-<trust>-`
//! after the arch segment; the docker unit is `trusted-only`, the rest
//! `untrusted-ok`), so every entry below carries those segments. D9 of
//! `plans/2026-09-16-ci-workflow-and-cache-plan.md` requires the resolved
//! keys of the parameterized callee to equal these literal forms. D2 also
//! re-captured every mbx/seed compatibility digest: the digest facts gained
//! provider, platform, and trust, so the same units digest new values.
//!
...
pub(crate) struct UnitProviderKeys {
...
pub(crate) const PRE_PARAMETERIZATION_KEYS: &[UnitProviderKeys] = &[
    UnitProviderKeys {
    ...
    UnitProviderKeys {
        ...
        keys: &[
            CacheKey {
                ...
                primary: "velnor-docker-seed-v3-ad4a45385e02-${{ runner.os }}-${{ runner.arch }}-github-hosted-linux-x64-trusted-only-docker-${{ hashFiles('Cargo.lock', 'Dockerfile', 'docker/build-mise.lock', 'docker/build-mise.toml', 'rust-toolchain.toml') }}-${{ hashFiles('Cargo.lock', 'deny.toml') }}",
                restore_keys: &[
                    "velnor-docker-seed-v3-ad4a45385e02-${{ runner.os }}-${{ runner.arch }}-github-hosted-linux-x64-trusted-only-docker-${{ hashFiles('Cargo.lock', 'Dockerfile', 'docker/build-mise.lock', 'docker/build-mise.toml', 'rust-toolchain.toml') }}-",
                    "velnor-docker-seed-v3-ad4a45385e02-${{ runner.os }}-${{ runner.arch }}-github-hosted-linux-x64-trusted-only-docker-",
                ],
            },
        ],
    },
