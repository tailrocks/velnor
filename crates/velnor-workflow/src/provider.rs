...
pub(crate) fn provider_caps(provider: ProviderId) -> ProviderCaps {
    match provider {
        ...
        ProviderId::GithubSelfHosted => ProviderCaps {
        ...
        ProviderId::Velnor => ProviderCaps {
            platforms: BTreeSet::from([Platform::LinuxX64]),
            trusted: true,
            caps: Capabilities {
                docker: true,
...
pub(crate) fn plan_digest(
    ...
    for (unit_id, providers, command_digest) in units {
        ...
        let _ = write!(digest_input, ":{command_digest}\n");
    }
    ...
    let digest = crate::content_digest_bytes(digest_input.as_bytes());
    format!("{digest:016x}")
}
