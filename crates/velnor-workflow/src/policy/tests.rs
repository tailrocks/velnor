...
/// The owner policy job acquires the declared product without a literal
/// `--rev <sha>` install. The variable reference is not a pin literal, so the
/// pin rule still passes on the exported revision and still names it on drift.
#[test]
...
// ---------------------------------------------------------------------------
#[test]
fn generated_tree_report_rejects_candidate_render() {
    let pin = generated_tree_report(PIN_A, Ok(TreeComparison::Pin));
    assert!(pin.passed);
    assert!(pin.reason.contains(PIN_A), "{}", pin.reason);
    // A candidate render is deliberately represented as a non-pin tree
    // mismatch. It must never satisfy the active-tree invariant.
    let candidate = generated_tree_report(
        PIN_A,
        Ok(TreeComparison::Differences(vec![
            "generated output matches candidate renderer".to_owned(),
        ])),
    );
    assert!(!candidate.passed);
    assert!(
        candidate.reason.contains("differs from the render"),
        "{}",
        candidate.reason
    );
    assert_eq!(
        candidate.details,
        vec!["generated output matches candidate renderer".to_owned()]
    );
}
