...
//!   the pinned `--owner`/`--signer-workflow`/`--source-ref` flags, so a
//!   passing run proves the gate pinned the producer and the default-branch
//!   ref.
...
/// The default-branch ref every consumer attestation gate must pin.
const PINNED_SOURCE_REF_FLAG: &str = "--source-ref refs/heads/main";
...
/// producer flags (`$GH_STUB_EXPECT_OWNER`, `$GH_STUB_EXPECT_SIGNER`) and
/// the default-branch ref pin (`refs/heads/main`); anything else fails
/// loudly, so a passing run proves the pin held.
...
/// Fixture "binary": a script answering `--closure`/`--revision` with the
/// canned reports and refusing anything else, so unexpected execution fails
/// loudly.
fn stub_binary_script(reported_closure: &str, reported_revision: &str) -> String {
    format!(
        "#!/bin/sh\nif [ \"$1\" = \"--closure\" ]; then\n  echo \"{reported_closure}\"\n  exit 0\nfi\nif [ \"$1\" = \"--revision\" ]; then\n  echo \"{reported_revision}\"\n  exit 0\nfi\necho \"stub binary: unexpected arguments $*\" >&2\nexit 1\n"
    )
}
...
impl ConsumerFixture {
    ...
    fn serve_asset(&self, reported_closure: &str) -> String {
        let bytes = stub_binary_script(reported_closure, &self.revision);
        must(
...
fn cold_consumer_verifies_everything_with_zero_waivers() {
    ...
    assert!(
        log.contains(PINNED_SOURCE_REF_FLAG),
        "verification pinned the default-branch ref: {log}"
    );
