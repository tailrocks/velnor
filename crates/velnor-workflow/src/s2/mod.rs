...
/// or reuse a revision.
pub const GENERATOR_REVISION: &str = "72";
...
impl crate::config::mise_closure::RustToolchainLike for RustToolchain {
    fn channel(&self) -> &str {
        self.channel()
    }
...
fn apply_check_profiles(
    ...
    for row in rows {
        ...
        let platform = match runner {
            "github" => {
                let labels =
                    provider::runs_on_for(&config.selectors, provider::ProviderId::GithubHosted)
                        .map_err(|error| {
                            GeneratorError::usage(format!("check profile {id}: {error}"))
                        })?;
                crate::config::mise_closure::platform_for_runner_labels(
                    runner,
                    row.platform(),
                    labels.iter().map(String::as_str),
                )
            }
            "macos" => crate::config::mise_closure::platform_for_s2_runner(runner, row.platform())
                .map(str::to_owned),
            "velnor" => {
                let labels = provider::runs_on_for(&config.selectors, provider::ProviderId::Velnor)
                    .map_err(|error| {
                        GeneratorError::usage(format!("check profile {id}: {error}"))
                    })?;
                crate::config::mise_closure::platform_for_runner_labels(
            ...
            _ => crate::config::mise_closure::platform_for_runner(runner, row.platform())
                .map(str::to_owned),
        }
        .map_err(|error| GeneratorError::usage(format!("check profile {id}: {error}")))?;
        let closure = crate::config::mise_closure::resolve_profile_for_platform_with_rust(
            root,
            tasks,
            tools,
            &platform,
            rust_toolchain
                .as_ref()
                .map(|toolchain| toolchain as &dyn crate::config::mise_closure::RustToolchainLike),
        )
        .map_err(|error| GeneratorError::usage(error.to_string()))?;
        closure
            .validate_profile_tools(id, tools)
            .map_err(|error| GeneratorError::usage(error.to_string()))?;
        let install_tools = closure
            .installation_plan(id)
            .and_then(|plan| plan.mise_args(id))
            .map_err(|error| GeneratorError::usage(error.to_string()))?;
