...
mod tests {
    ...
    fn pull_request_owner_renders_independent_candidate_bootstrap() {
        ...
        let workflow = ir.render_nested(WorkflowKind::PullRequest, &[], None);
        let candidate = job_block(&workflow, "candidate-bootstrap");

        assert!(candidate.contains("runs-on: ubuntu-24.04"), "{candidate}");
        assert!(
            !candidate.contains("needs:"),
            "bootstrap does not wait on planning, checks, or Velnor: {candidate}"
        );
        assert!(
            candidate.contains(
                "if: ${{ github.event_name == 'pull_request' && github.event.pull_request.head.repo.full_name == github.repository && github.event.pull_request.head.repo.id == github.repository_id }}"
            ),
            "forks cannot execute the producer: {candidate}"
        );
        assert!(
            candidate.contains("ref: ${{ github.event.pull_request.base.sha }}")
                && candidate.contains("path: policy-setup-action")
                && candidate.contains("sparse-checkout: .github/actions/setup-velnor-workflow")
                && candidate.contains("fetch-depth: 1"),
            "the setup action comes from the base tree: {candidate}"
        );
        assert!(
            candidate.contains("ref: ${{ github.event.pull_request.head.sha }}")
                && candidate.contains("path: candidate-head")
                && candidate.contains("fetch-depth: 0"),
            "the candidate build sees a complete head checkout: {candidate}"
        );
        assert!(
            candidate.contains("uses: ./policy-setup-action/.github/actions/setup-velnor-workflow")
                && !candidate.contains("uses: ./.github/actions/setup-velnor-workflow"),
            "the runtime action is base-owned, never PR-owned: {candidate}"
        );
        assert!(
            candidate.contains("git fetch --no-tags --depth 1")
                && candidate.contains("git show \"$BASE_SHA:.github-gen/velnor-workflow.toml\"")
                && candidate.contains("git show \"$BASE_SHA:.github/workflows/ci-policy.yml\"")
                && candidate.contains("base $BASE_SHA declares no generator pin"),
            "base pin resolution is shallow-history safe and fail closed: {candidate}"
        );
        assert!(
            candidate.contains("velnor-workflow closure --rev=\"$PR_HEAD\" --candidate")
                ...
                && candidate.contains("cargo build --locked --manifest-path crates/velnor-workflow/Cargo.toml"),
            "the immutable base runtime computes the head/base candidate closures before a locked head build: {candidate}"
        );
        assert!(
            candidate.contains("CARGO_HOME: ${{ runner.temp }}/candidate-cargo-home")
                && candidate.contains("RUSTC_WRAPPER: \"\"")
                && candidate.contains("CARGO_BUILD_RUSTC_WRAPPER: \"\"")
                && candidate.contains("GH_TOKEN: \"\"")
                && candidate.contains("GITHUB_TOKEN: \"\""),
            "the PR build has isolated cargo state and no token/wrapper environment: {candidate}"
        );
        assert!(
            candidate.contains("--revision")
                && candidate.contains("reported_revision")
                && candidate.contains("--closure")
                && candidate.contains("reported_closure"),
            "the staged binary proves both source identity and closure: {candidate}"
        );
        assert!(
            candidate.contains("candidate-manifest.json")
                && candidate.contains("--arg features tui")
                && candidate.contains("repository_id")
                && candidate.contains("head_repository_id")
                && candidate.contains("binary_sha256")
                && candidate.contains("name: ${{ steps.candidate.outputs.name }}")
                && candidate.contains("if-no-files-found: error")
                && candidate.contains("retention-days: 1"),
            "the published artifact has the consumer manifest and fail-closed upload: {candidate}"
        );
        assert!(
            !candidate.contains("inputs.candidate_"),
            "the old unit-owned candidate path is gone: {candidate}"
        );
    }

    #[test]
    fn candidate_bootstrap_is_owner_and_pull_request_only() {
        let owner = workflow_setup_action_repository().to_owned();
        let units = vec![rust_unit("rust-generator-crate", "crates/velnor-workflow")];
        let foreign = owner_test_ir("example/foreign", units.clone());
        assert!(
            !foreign
                .render_nested(WorkflowKind::PullRequest, &[], None)
                .contains("\n  candidate-bootstrap:\n"),
            "foreign trees do not produce the owner artifact"
        );

        let owner_ir = owner_test_ir(&owner, units);
        for kind in [WorkflowKind::Main, WorkflowKind::Nightly] {
            assert!(
                !owner_ir
                    .render_nested(kind, &[], None)
                    .contains("\n  candidate-bootstrap:\n"),
                "{kind:?} does not run the pull-request candidate bootstrap"
            );
        }
    }
}
...
impl WorkflowIr {
    ...
    ///
    /// The aggregate owns control-plane composition: on pull requests the
    /// owner-only candidate bootstrap, then the plan job, the advisory policy
    ...
    pub(crate) fn render_nested(
        ...
        if kind == WorkflowKind::PullRequest {
            self.render_candidate_bootstrap(&mut output);
        }
    ...
    /// The owner-only Stage-1 candidate producer. It deliberately has no
    /// `needs:` edge: candidate acquisition must remain live when Planning,
    /// ordinary unit checks, dependency ordering, or Velnor capacity is not.
    /// The policy consumer still decides whether the artifact is needed; this
    /// job only makes the immutable, head-bound product available when the
    /// owner PR changes the generator closure.
    fn render_candidate_bootstrap(&self, output: &mut String) {
        if self.repository != crate::s2::workflow_setup_action_repository() {
            return;
        }
        output.push_str(&candidate_bootstrap_job(
            self.pins.checkout,
...
/// The owner-only Stage-1 candidate producer. The base setup action is checked
/// out from the base SHA before any PR-controlled build code runs, and its
/// immutable runtime product computes the candidate closure. The producer is
/// intentionally a direct aggregate job with no Plan/unit/Velnor dependency:
/// policy must be able to acquire this product while the normal verification
/// graph is unavailable.
#[expect(
    clippy::too_many_lines,
    reason = "the hosted bootstrap is one auditable source-to-artifact contract"
)]
fn candidate_bootstrap_job(checkout_pin: &str, upload_artifact_pin: &str) -> String {
    format!(
        r#"  candidate-bootstrap:
    ...
    permissions:
      ...
      contents: read
    steps:
      # Never resolve a setup action from the PR head before source identity is
      # established. The base tree owns the action used to acquire Stage-0.
      ...
      - name: Check out candidate head
        ...
        with:
          ...
          fetch-depth: 0
          persist-credentials: false
      - name: Resolve base generator pin
        id: base
        working-directory: candidate-head
        env:
          BASE_SHA: ${{{{ github.event.pull_request.base.sha }}}}
        run: |
          set -euo pipefail
          if ! git cat-file -e "$BASE_SHA^{{commit}}" 2>/dev/null; then
            git fetch --no-tags --depth 1 "$GITHUB_SERVER_URL/$GITHUB_REPOSITORY" "$BASE_SHA"
          fi
          base_pin="$(git show "$BASE_SHA:.github-gen/velnor-workflow.toml" 2>/dev/null | sed -n -E 's/^[[:space:]]*revision[[:space:]]*=[[:space:]]*"([0-9a-f]{{40}})".*/\1/p' | head -n 1)"
          test "$base_pin" != '' || base_pin="$(git show "$BASE_SHA:.github/workflows/ci-policy.yml" 2>/dev/null | sed -n -E 's/^.*VELNOR_WORKFLOW_POLICY_REVISION:[[:space:]]*([0-9a-f]{{40}}).*/\1/p' | head -n 1)"
          test "$base_pin" != '' || {{ echo "::error::base $BASE_SHA declares no generator pin" >&2; exit 1; }}
          printf 'sha=%s\n' "$BASE_SHA" >> "$GITHUB_OUTPUT"
          printf 'pin=%s\n' "$base_pin" >> "$GITHUB_OUTPUT"
      - name: Set up base Velnor workflow runtime
        id: runtime
        uses: {policy_setup_action}
        with:
          rev: ${{{{ steps.base.outputs.pin }}}}
          checkout-path: ${{{{ github.workspace }}}}/candidate-head
      - name: Build candidate generator product
        id: candidate
        working-directory: candidate-head
        env:
          CANDIDATE_PR_HEAD_SHA: ${{{{ github.event.pull_request.head.sha }}}}
          CANDIDATE_REPOSITORY: ${{{{ github.repository }}}}
          CANDIDATE_REPOSITORY_ID: ${{{{ github.repository_id }}}}
          CANDIDATE_HEAD_REPOSITORY: ${{{{ github.event.pull_request.head.repo.full_name }}}}
          CANDIDATE_HEAD_REPOSITORY_ID: ${{{{ github.event.pull_request.head.repo.id }}}}
          CANDIDATE_RUN_ID: ${{{{ github.run_id }}}}
          CANDIDATE_JOB_ID: ${{{{ github.job }}}}
          BASE_PIN: ${{{{ steps.base.outputs.pin }}}}
          CARGO_HOME: ${{{{ runner.temp }}}}/candidate-cargo-home
        ...
        run: |
          ...
          cargo build --locked --manifest-path crates/velnor-workflow/Cargo.toml
          binary="target/debug/velnor-workflow"
          test -x "$binary" || {{ echo "::error::candidate build did not produce $binary" >&2; exit 1; }}
          ...
          reported_closure="$("$stage/velnor-workflow" --closure)"
          [[ "$reported_closure" == "$head_closure" ]] || {{ echo "::error::candidate reports closure $reported_closure, head $PR_HEAD declares $head_closure" >&2; exit 1; }}
          reported_revision="$("$stage/velnor-workflow" --revision)"
          [[ "$reported_revision" == "$PR_HEAD" ]] || {{ echo "::error::candidate reports revision $reported_revision, head $PR_HEAD declares $PR_HEAD" >&2; exit 1; }}
          jq -n --arg profile debug --arg features tui --arg platform "${{RUNNER_OS}}-${{RUNNER_ARCH}}" --arg repository "$CANDIDATE_REPOSITORY" --arg repository_id "$CANDIDATE_REPOSITORY_ID" --arg head_repository "$CANDIDATE_HEAD_REPOSITORY" --arg head_repository_id "$CANDIDATE_HEAD_REPOSITORY_ID" --arg run_id "$CANDIDATE_RUN_ID" --arg job_id "$CANDIDATE_JOB_ID" --arg revision "$PR_HEAD" --arg closure "$head_closure" --arg build_revision "$PR_HEAD" --arg binary_sha256 "$digest" '{{profile: $profile, features: $features, platform: $platform, repository: $repository, repository_id: $repository_id, head_repository: $head_repository, head_repository_id: $head_repository_id, run_id: $run_id, job_id: $job_id, revision: $revision, closure: $closure, build_revision: $build_revision, binary_sha256: $binary_sha256}}' > "$stage/candidate-manifest.json"
      ...
      - name: Publish candidate generator product
        if: steps.candidate.outputs.skip != 'true'
        ...
        with:
          ...
          retention-days: 1
"#,
        policy_setup_action = crate::s2::VELNOR_WORKFLOW_POLICY_SETUP_ACTION,
    )
}
