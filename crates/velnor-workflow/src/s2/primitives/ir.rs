...
Run: https://github.com/$GITHUB_REPOSITORY/actions/runs/$GITHUB_RUN_ID""#
    ...
    pub(crate) fn render_kind_unit_workflow(
        ...
        if kind == UnitKind::Homebrew {
            ...
            self.render_homebrew_candidate_jobs(&mut output, &members, &env);
        }
    ...
    fn render_homebrew_candidate_jobs(
        &self,
        output: &mut String,
        members: &[&Unit],
        ...
        let mut shadowed_env = String::new();
        for name in inherited_env.keys() {
            if matches!(
                name.as_str(),
                "HOMEBREW_NO_AUTO_UPDATE" | "HOMEBREW_NO_INSTALL_CLEANUP" | "TAP" | "FORMULA"
            ) {
                continue;
            }
