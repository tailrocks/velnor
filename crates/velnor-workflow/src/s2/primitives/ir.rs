...
use crate::s2::{
    ...
    unit_job_display_name, unit_job_id, workflow_runtime_artifact_upload, workflow_runtime_setup,
...
mod tests {
    ...
    use super::{
        ...
        RequiredCaller, RustNeeds, Unit, UnitKind, WorkflowIr, WorkflowKind, REQUIRED_CHECK,
    };
    use crate::s2::{
    ...
    #[test]
    fn foreign_local_provider_check_implies_policy_runtime() {
        ...
        let owner = "example/consumer".to_owned();
    ...
    #[test]
    fn source_bootstrap_is_a_required_root_before_any_owner_planning(
    ) -> Result<(), Box<dyn std::error::Error>> {
        let ir = owner_test_ir(
            workflow_setup_action_repository(),
            vec![rust_unit("generator", "crates/velnor-workflow")],
        );
        let mut rendered = String::new();
        ir.render_plan(&mut rendered);
        ir.render_policy(&mut rendered);
        let yaml: serde_yaml::Value = serde_yaml::from_str(&format!("jobs:\n{rendered}"))?;
        assert_eq!(
            yaml["jobs"]["runtime"]["permissions"]["contents"].as_str(),
            Some("read")
        );
        assert_eq!(
            yaml["jobs"]["runtime"]["permissions"]
                .as_mapping()
                .map(serde_yaml::Mapping::len),
            Some(1)
        );
        assert_eq!(yaml["jobs"]["plan"]["needs"][0].as_str(), Some("runtime"));
        assert_eq!(yaml["jobs"]["policy"]["needs"][0].as_str(), Some("plan"));
        let plan = yaml["jobs"]["plan"]["steps"]
            .as_sequence()
            .ok_or("missing plan steps")?;
        assert!(!plan
            .iter()
            .any(|step| step["name"].as_str() == Some("Build exact renderer source")));
        assert!(plan
            .iter()
            .any(|step| step["name"].as_str() == Some("Verify Velnor workflow runtime")));
        assert!(
            !rendered.contains("deadline=$((SECONDS + 900))"),
            "current-run Policy must not search for a PR artifact"
        );
        let units = must_render_kind(&ir);
        assert!(!units.contains("Prepare candidate generator product"));
        Ok(())
    }

    #[test]
    fn source_bootstrap_provisions_exact_job_cli_on_every_owner_provider() {
        let ir = owner_test_ir(
            workflow_setup_action_repository(),
            vec![rust_unit("rust-generator", "crates/velnor-workflow")],
        );
        let rendered = must_render_kind(&ir);
        for provider in ["github-hosted", "github-self-hosted", "velnor"] {
            let id = format!("verify-{provider}");
            let block = job_block(&rendered, &id);
            assert!(
                block.contains("Verify Velnor workflow runtime"),
                "{provider}: {block}"
            );
            assert!(
                block.contains("velnor-workflow-runtime-"),
                "{provider}: {block}"
            );
            assert!(
                block.contains("$GITHUB_PATH"),
                "{provider} must use provisioned job CLI"
            );
            assert!(!block.contains("Provision pinned Velnor workflow policy runtime"));
            assert!(!block.contains("Build exact renderer source"));
        }
    }
    ...
    fn foreign_trees_render_no_candidate_surface() {
        ...
        assert!(!content.contains("candidate_publish:"));
    }
}
...
fn rust_provider_for_provider(provider: ProviderId) -> RustProvider {
    if provider.is_local() {
        RustProvider::Velnor
    } else {
        RustProvider::Github
...
Run: https://github.com/$GITHUB_REPOSITORY/actions/runs/$GITHUB_RUN_ID""#
    ...
    fn render_collapsed_provider_verify_job(
        ...
        if provider == ProviderId::GithubHosted
            || super::runtime_bootstrap::owns_runtime(&self.repository)
        {
    ...
    pub(crate) fn unit_provider_facts(
        ...
        ProviderStepFacts {
            ...
            policy_runtime: !super::runtime_bootstrap::owns_runtime(&self.repository)
                && provider.is_local()
                && unit_runs_workflow_plain_check(unit),
            platform: unit.platform.as_str().to_owned(),
            trust: unit.trust.as_str().to_owned(),
    ...
    fn render_collapsed_provider_steps(
        ...
        let rust_phases = if kind == UnitKind::Rust {
            rust_validation_phases_for_members(
                members,
                rust_provider_for_provider(provider),
                &[RustScope::Affected, RustScope::Full],
            )
        } else {
            None
    ...
    pub(crate) fn render_workflow_runtime_download(
        ...
        output.push_str(&crate::s2::workflow_runtime_download_at(
            provider,
            &self.workflow_revision,
            ".",
            true,
    ...
    fn render_unit_runtime(&self, output: &mut String, provider: ProviderId, unit: &Unit) {
        if super::runtime_bootstrap::owns_runtime(&self.repository) {
            output.push_str(&crate::s2::workflow_runtime_download_at(
                ProviderId::GithubHosted,
                &self.workflow_revision,
                ".",
        ...
        // is hosted.
        if unit.kind == UnitKind::Swift && !super::runtime_bootstrap::owns_runtime(&self.repository)
        {
    ...
    pub(crate) fn render_plan(&self, output: &mut String) {
        ...
        let owner = super::runtime_bootstrap::owns_runtime(&self.repository);
        let runtime_setup = if owner {
            let mut platforms = BTreeSet::from([crate::s2::provider::Platform::LinuxX64]);
            if let Some(platform) =
                super::runtime_bootstrap::hosted_control_platform(&self.selectors)
            {
            ...
            crate::s2::workflow_runtime_setup_with_install_rev(
                ProviderId::GithubHosted,
                &self.repository,
                &self.workflow_revision,
        ...
        // consumes.
        if self.providers.contains(&ProviderId::GithubHosted) && !owner {
    ...
    pub(crate) fn render_policy(&self, output: &mut String) {
        output.push_str(&crate::s2::policy_job(&crate::s2::PolicyJobSpec {
            name: "Policy",
            runtime_from_plan: super::runtime_bootstrap::owns_runtime(&self.repository),
