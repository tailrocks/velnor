...
/// A cache hit is only a usable toolchain when rustup can find the expected
/// channel and every declared component/target in that installation. The
/// probe never fails the job itself: a false result sends the leg through the
/// normal provisioning command, preserving a cold-path recovery for a stale
/// or partial archive.
...
/// hit is absent or incomplete, and — when `save_gate` carries the step's
/// `if:` body — save the result for the next run. Image-backed Velnor jobs
/// intentionally bypass this helper because their pinned toolchain is part of
/// the runner image.
