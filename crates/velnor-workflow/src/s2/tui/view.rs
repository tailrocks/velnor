...
use crate::s2::{PlannedAction, Unit, WriteOutcome};
...
fn cli_providers_summary(cli: &crate::s2::Cli) -> String {
    cli.providers.as_ref().map_or_else(
        ...
        |providers| {
            providers
                .iter()
                .map(crate::s2::provider::ProviderId::as_str)
                .collect::<Vec<_>>()
                .join(", ")
        },
    )
...
mod tests {
    ...
    fn scanning_frame_is_calm_and_honest() {
        ...
        app.cli.providers = Some(crate::s2::provider::ProviderId::ALL.into_iter().collect());
    ...
    fn every_workflow_state_has_intentional_copy() {
        ...
        app.outcome = Some(crate::s2::WriteOutcome::DryRun(vec![PathBuf::from(
    ...
    fn review_and_check_drift_show_delete_and_ownership_changes() {
        let mut app = configured_app();
        app.plan = Some(crate::s2::GeneratedWritePlan {
            files: vec![
                crate::s2::PlannedFile {
                    path: PathBuf::from(".github/workflows/old.yml"),
                    action: crate::s2::PlannedAction::Delete,
                    preimage: crate::s2::FilePreimage::Missing,
                },
                crate::s2::PlannedFile {
                    path: PathBuf::from(".github/ci/unchanged.toml"),
                    action: crate::s2::PlannedAction::Same,
                    preimage: crate::s2::FilePreimage::Missing,
                },
                crate::s2::PlannedFile {
        ...
        app.phase = super::super::Phase::Complete;
        app.outcome = Some(crate::s2::WriteOutcome::DryRun(Vec::new()));
