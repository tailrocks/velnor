...
fn workflow_dispatch_inputs(default_scope: &str, extra_inputs: &str) -> String {
    format!(
...
fn aggregate_triggers(
    ...
    match kind {
        WorkflowKind::PullRequest => (
            ...
            format!(
                "on:\n  pull_request:\n{}",
                workflow_dispatch_inputs("affected", "")
            ),
            "true",
        ),
        WorkflowKind::Main => (
...
fn merged_kind_cache(units: &[&Unit]) -> Option<CacheSpec> {
    let mut key_files = BTreeSet::new();
    let mut paths = BTreeSet::new();
    let mut purpose = None;
    let mut justification = None;
    let mut mutable_mount_seed = false;
    for unit in units {
        let Some(cache) = &unit.cache else {
            continue;
        };
        key_files.extend(cache.key_files.iter().cloned());
        paths.extend(cache.paths.iter().cloned());
        purpose.get_or_insert(cache.purpose);
        if justification.is_none() {
            justification.clone_from(&cache.mbx_output_cache_justification);
        }
        mutable_mount_seed |= cache.mutable_mount_seed;
    }
    if key_files.is_empty() && paths.is_empty() {
        return None;
    }
    Some(CacheSpec {
        key_files: key_files.into_iter().collect(),
        paths: paths.into_iter().collect(),
        purpose: purpose.unwrap_or(CachePurpose::Generic),
        mbx_output_cache_justification: justification,
        mutable_mount_seed,
    })
}
...
Run: https://github.com/$GITHUB_REPOSITORY/actions/runs/$GITHUB_RUN_ID""#;
    ...
    pub(crate) fn render_kind_units(&self, kind: UnitKind) -> String {
        let members = self
            .units
            .iter()
    ...
    pub(crate) fn render_lane_job(
        ...
        let gate = self.lane_event_expression(lane);
