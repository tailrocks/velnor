...
mod tests {
    ...
    fn apple_rust_preserves_macos_job_without_linux_candidate_acquire() {
        ...
        apple.platform = crate::platform::PlatformRequirement::apple_xcode();
        let content = must_render_kind(&owner_test_ir(&owner, vec![apple]));
        assert!(content.contains("runs-on: macos-15"), "{content}");
        assert!(
...
Run: https://github.com/$GITHUB_REPOSITORY/actions/runs/$GITHUB_RUN_ID""#
    ...
    fn render_collapsed_lane_steps(
        ...
        if github_lane
            ...
            && members.iter().any(|unit| !unit.platform.requires_apple())
        {
    ...
    pub(crate) fn render_plan(&self, output: &mut String, _runners: RunnerMode, _trusted: bool) {
        ...
        let candidate_plan_path = if runners == RunnerMode::Github
            && self.repository == crate::workflow_setup_action_repository()
        {
    ...
    pub(crate) fn render_verify_lane(
            ...
            .filter(|unit| lane_supports_unit(lane, unit))
        {
            ...
            let _ = writeln!(
                output,
                "      - name: Run {} checks\n        env:\n          CI_SCOPE: ${{{{ needs.plan.outputs.scope }}}}\n          CI_UNIT_ID: {}\n          EVENT_NAME: ${{{{ github.event_name }}}}\n          BASE_SHA: ${{{{ {} }}}}\n          HEAD_SHA: ${{{{ github.sha }}}}\n          VELNOR_SELECTION_FILE: .velnor-ci-selection/velnor-ci-selection{}{token_env}\n        run: |\n          set -o pipefail\n{checks_started_marker}\n          rc=0\n          \"${{VELNOR_WORKFLOW_CANDIDATE_BINARY:-velnor-workflow}}\" run --config .github/ci/project.toml --scope \"$CI_SCOPE\" --unit {} 2>&1 | tee \"$RUNNER_TEMP/velnor-unit-log.txt\" || rc=$?\n{checks_ended_marker}\n          exit $rc",
                verify_name,
                yaml_scalar(&unit.id),
                base_sha,
                checks_env(unit),
