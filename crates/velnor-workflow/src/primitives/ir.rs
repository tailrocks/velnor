...
/// Membership test used by one reusable invocation per unit kind.
...
impl WorkflowIr {
    ...
    /// One reusable-workflow caller per unit kind. The reusable owns one job
    /// per unit and skips unselected jobs from the plan's unit set, avoiding
    /// the N×N jobs produced by a caller-side matrix.
...
Run: https://github.com/$GITHUB_REPOSITORY/actions/runs/$GITHUB_RUN_ID""#;
    ...
    /// The lane jobs a nested unit workflow emits for the configured backends.
    ...
    fn render_unit_runtime(output: &mut String, lane: RunnerMode, _unit: &Unit) {
        // Every hosted job installs the exact pinned runtime it executes.
        // Velnor jobs use the runtime supplied by their trusted runner image.
    ...
    pub(crate) fn tools_for_unit(
        ...
        // Declared tools provision through mise for every unit kind. Versions
        // come from the repository's mise manifest, never an ad hoc install.
