...
use crate::{GeneratorError, RustToolchain, Unit};
...
mod tests {
    ...
    fn toolchain_facts_flatten_the_pin() {
        let pinned = crate::RustToolchain {
    ...
    fn generated_surface(root: &std::path::Path) -> crate::primitives::Surface {
        ...
        let path = root.join(crate::config::GENERATION_CONFIG_PATH);
        let bytes = must(std::fs::read(&path), "read generation config");
        let generation = must(
            crate::config::parse(&path, &bytes),
            "parse generation config",
        );
        must(
            crate::primitives::generate(root, &shape, &config, Some(&generation)),
