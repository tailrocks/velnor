...
/// `XcodeGen` generation, and Swift build/run/test); generated jobs run
...
impl ValidationPhase {
    ...
    /// tests, then doctests, then Swift format/lint, then `XcodeGen`
    /// generation, then Swift builds and executable runs, then tests.
    /// `Check` is prerequisite-only and never renders a validation step.
