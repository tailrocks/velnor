...
/// The generated agent-instruction file every render owns. Its bytes are a
/// fixed contract: no repository name, no scan input, no revision stamp, so
/// the file is identical for every repository and every generator revision
/// until this constant changes.
pub(crate) const GENERATED_GITHUB_AGENTS_MD: &str = "# Generated files\n\
     \n\
...
/// Repository-root-relative path of the generated agent-instruction file.
pub(crate) const GITHUB_AGENTS_MD: &str = ".github/AGENTS.md";
/// Repository-root-relative path of the generated reviewer-assignment file.
...
/// refusal so the fix is one click away.
pub(crate) const CODEOWNERS_DOCS_URL: &str = "https://docs.github.com/en/repositories/managing-your-repositorys-settings-and-features/customizing-your-repository/about-code-owners";

/// Render the generator-owned CODEOWNERS body for reviewer rules in
/// declaration order. GitHub is last-match-wins, so order is semantic and
/// both pipelines share this renderer to keep bytes identical.
pub(crate) fn render_codeowners_contents(rules: &[(&str, &[String])]) -> String {
...
/// Whether `owner` is a valid CODEOWNERS owner: `@user`, `@org/team`, or an
/// email address. Both pipelines share this predicate so validation can
/// never drift between schemas.
pub(crate) fn is_valid_codeowner(owner: &str) -> bool {
    if let Some(handle) = owner.strip_prefix('@') {
        let (user, team) = match handle.split_once('/') {
            Some((user, team)) => (user, Some(team)),
            None => (handle, None),
        };
        if user.is_empty()
            || !user
        ...
        if let Some(team) = team {
            if team.contains('/') || team.is_empty() {
                return false;
            }
            return team
                .bytes()
    ...
    matches!((parts.next(), parts.next(), parts.next()), (Some(user), Some(domain), None) if !user.is_empty() && !domain.is_empty() && !owner.chars().any(char::is_whitespace))
}

/// The ownership digest of a symlink: FNV-1a over the raw link-target bytes,
/// the link analogue of [`content_digest_bytes`]. Both pipelines share it so
/// a recorded state verifies under either renderer.
pub(crate) fn symlink_target_digest(target: &Path) -> u64 {
    content_digest_bytes(target.as_os_str().as_encoded_bytes())
}

/// Create the generator-owned symlink. Unix links files and directories alike;
/// Windows needs the file variant for a file link.
pub(crate) fn create_generator_symlink(target: &Path, link: &Path) -> Result<(), GeneratorError> {
    #[cfg(unix)]
    {
        std::os::unix::fs::symlink(target, link)
            .map_err(|error| GeneratorError::io("create generator symlink", link, &error))
    }
    #[cfg(windows)]
    {
    ...
    #[cfg(not(any(unix, windows)))]
    {
        ...
        Err(GeneratorError::usage(
            "generator symlinks need a unix or windows target".to_owned(),
        ))
    }
}
...
fn generated_files_with_surface(
    ...
    files.insert(
        ...
        GENERATED_GITHUB_AGENTS_MD.to_owned(),
    );
...
impl FilePreimage {
    ...
    fn has_symlink_target(&self, expected: &Path) -> bool {
        self.symlink_target()
...
fn plan_generated_write_with_options(
    ...
    let symlink_preimages = symlinks
        .keys()
        .map(|relative| {
            capture_file_preimage(&root.join(relative), relative)
                .map(|preimage| (relative.clone(), preimage))
    ...
    let files_need_refresh = verify_generated_ownership(
        files,
        ownership.map(|state| &state.outputs),
        &preimages,
        adopt,
    )?;
    let symlinks_need_refresh = verify_symlink_ownership(
        symlinks,
        ownership.map(|state| &state.outputs),
        &symlink_preimages,
    )?;
    ...
    changed.extend(
        symlinks
            ...
            .filter(|(relative, wanted)| {
                symlink_preimages
                    .get(*relative)
                    .is_none_or(|preimage| !preimage.has_symlink_target(wanted))
            })
            .map(|(relative, _)| relative.clone()),
    );
    ...
    let mut planned_files = files
        ...
        .collect::<Vec<_>>();
    planned_files.extend(symlinks.keys().map(|path| {
        let action = if changed.contains(path) {
            if root.join(path).exists() {
                PlannedAction::Update
            } else {
...
fn verify_symlink_ownership(
    ...
    for (relative, wanted) in symlinks {
        ...
        match ownership.and_then(|state| state.get(relative)) {
            Some(digest) if *digest == symlink_target_digest(current) => (),
            Some(_) => {
                if current == wanted {
                    needs_refresh = true;
                    continue;
...
",
    ...
    #[test]
    fn schema_one_rejects_a_dangling_implicit_nextest_dependency_after_closure() {
        let root = nextest_fixture_repository("mise-schema-one-implicit-nextest-dependency");
        let unit = scanned_rust_unit_id(&root);
        // No generation-config row declares tools for this scanned unit: the
        // only install member is the nextest tool the schema-1 IR derives
        // from its commands later in rendering.
        must(
            fs::write(
                root.join("mise.toml"),
                "[settings]\nlockfile = true\n\n[tools]\n\"aqua:nextest-rs/nextest/cargo-nextest\" = { version = \"0.9.0\", depends = [\"example-ghost\"] }\n",
            ),
            "write implicit nextest dependency fixture",
        );
        must(
            fs::write(
                root.join("mise.lock"),
            ...
            "write implicit nextest dependency lock",
        );
        let error = must_fail(
            scan_repository(&root, RunnerMode::Github),
            "schema-1 must reject a dangling implicit nextest dependency",
        );
        let message = error.to_string();
        assert!(message.contains(&unit), "error names the unit: {message}");
        assert!(
            message.contains("aqua:nextest-rs/nextest/cargo-nextest")
                && message.contains("example-ghost"),
            "error names the implicit nextest dependency edge: {message}"
        );
        let _ = fs::remove_dir_all(root);
    }

    #[test]
    fn schema_one_rejects_a_dangling_implicit_cargo_deny_dependency_after_closure() {
        let root = configured_repository("mise-schema-one-implicit-cargo-deny-dependency", None);
        must(
            fs::write(root.join("deny.toml"), "[advisories]\n"),
            "write cargo-deny policy fixture",
        );
        // The policy unit has an empty declared mise_tools vector. Its
        // cargo-deny install is introduced only by the final Velnor IR.
        must(
            fs::write(
                root.join("mise.toml"),
                "[settings]\nlockfile = true\n\n[tools]\n\"aqua:EmbarkStudios/cargo-deny\" = { version = \"0.18.3\", depends = [\"example-ghost\"] }\n",
            ),
            "write implicit cargo-deny dependency fixture",
        );
        must(
            fs::write(
                root.join("mise.lock"),
            ...
            "write implicit cargo-deny dependency lock",
        );
        let error = must_fail(
            scan_repository(&root, RunnerMode::Github),
            "schema-1 must reject a dangling implicit cargo-deny dependency",
        );
        let message = error.to_string();
        assert!(
            message.contains("rust-policy"),
            "error names the policy unit: {message}"
        );
        assert!(
            message.contains("aqua:EmbarkStudios/cargo-deny") && message.contains("example-ghost"),
            "error names the implicit cargo-deny dependency edge: {message}"
        );
        let _ = fs::remove_dir_all(root);
    }

    #[test]
    fn schema_one_renders_the_closed_implicit_nextest_subset() {
        let root = nextest_fixture_repository("mise-schema-one-rendered-nextest-closure");
        must(
            fs::write(
                root.join("mise.toml"),
                "[settings]\nlockfile = true\n\n[tools]\n\"aqua:nextest-rs/nextest/cargo-nextest\" = { version = \"0.9.0\", depends = [\"cargo-binstall\"] }\ncargo-binstall = \"1.0.0\"\n",
            ),
            "write rendered nextest dependency fixture",
        );
        must(
            fs::write(
                root.join("mise.lock"),
            ...
            "write rendered nextest dependency lock",
        );
        let config = must(
            scan_repository(&root, RunnerMode::Github),
            "scan rendered nextest dependency fixture",
        );
        let unit = must_some(
            config.units.iter().find(|unit| unit.kind == UnitKind::Rust),
            "Rust unit",
        ...
        let workflow =
            WorkflowIr::from_config(&config).render_nested_unit(unit, WorkflowKind::PullRequest);
        assert!(
            workflow.contains("install_args: aqua:nextest-rs/nextest/cargo-nextest cargo-binstall"),
            "rendered nextest install args must carry its closed dependency: {workflow}"
        );
        let _ = fs::remove_dir_all(root);
    }

    #[test]
    fn schema_one_renders_the_closed_implicit_cargo_deny_subset() {
        let root = configured_repository("mise-schema-one-rendered-cargo-deny-closure", None);
        must(
            fs::write(root.join("deny.toml"), "[advisories]\n"),
            "write rendered cargo-deny policy fixture",
        );
        must(
            fs::write(
                root.join("mise.toml"),
            ...
            "write rendered cargo-deny dependency fixture",
        );
        must(
            fs::write(
                root.join("mise.lock"),
            ...
            "write rendered cargo-deny dependency lock",
        );
        let mut config = must(
            scan_repository(&root, RunnerMode::Github),
            "scan rendered cargo-deny dependency fixture",
        );
        // Scan validation derives both lane subsets. Select the Velnor lane
        // only for this direct IR assertion so the fixture needs no runner
        // label declaration unrelated to the closure contract.
        config.runners = RunnerMode::Velnor;
        let unit = must_some(
        ...
        let install_line = workflow
            .lines()
            .find(|line| line.contains("mise --yes install"))
            .unwrap_or_default();
        assert!(
    ...
    #[test]
    fn schema_one_rejects_an_unlisted_qualified_mise_dependency_alias() {
        let root = configured_repository("mise-schema-one-unlisted-alias", None);
        let unit = scanned_rust_unit_id(&root);
        write_generation_config(&root, &unit, "\"cargo:boltffi_cli\"");
        must(
            fs::write(
                root.join("mise.toml"),
                "[tools]\n\"cargo:boltffi_cli\" = { version = \"0.30.1\", depends = [\"sccache\"] }\n",
            ),
            "write unlisted alias fixture",
        );
        must(
            fs::write(
                root.join("mise.lock"),
                "[[tools.\"cargo:boltffi_cli\"]]\nversion = \"0.30.1\"\nbackend = \"cargo:boltffi_cli\"\n\n[[tools.\"github:sccache\"]]\nversion = \"0.10.0\"\nbackend = \"github:sccache\"\n",
            ),
            "write unlisted alias lock",
        );
        let error = must_fail(
            scan_repository(&root, RunnerMode::Github),
            "schema-1 must reject an unlisted qualified alias",
        );
        let message = error.to_string();
        assert!(message.contains(&unit), "error names the unit: {message}");
        assert!(
...
"#,
    ...
    fn generate_removes_absent_stale_entry_from_ownership_state() {
        ...
        assert_eq!(
            ...
            ownership_state_content(&current, &BTreeMap::new(), &GenerationInputs::parts(0, 0))
        );
        let _ = fs::remove_dir_all(root);
    }
    #[test]
    ...
    fn reviewers_render_in_declared_order_with_header() {
        ...
        let root = declared_surface_repository("v1-reviewers-order", &config, &[], &[]);
        let scanned = must(
            scan_target(&root, RunnerMode::Both, "main"),
        ...
        let codeowners = must_some(
            files.get(&PathBuf::from(CODEOWNERS_PATH)),
    ...
    fn codeowners_static_file_row_is_refused() {
        let root = temporary_repository("v1-codeowners-reserved");
        must(
            fs::write(
                root.join("Cargo.toml"),
                "[package]\nname = \"fixture\"\nversion = \"0.1.0\"\n",
        ...
        assert!(
            error.to_string().contains(CODEOWNERS_PATH),
    ...
    fn zero_reviewer_rows_emit_no_codeowners() {
        let root = temporary_repository("v1-codeowners-absent");
        must(
            fs::write(
                root.join("Cargo.toml"),
                "[package]\nname = \"fixture\"\nversion = \"0.1.0\"\n",
        ...
        let files = must(generated_files(&config), "generate");
        assert!(
            !files.contains_key(&PathBuf::from(CODEOWNERS_PATH)),
    ...
    #[test]
    fn codeowner_validity_covers_handles_teams_and_email() {
        for valid in [
            "@tailrocks",
            "@org/team",
            "@org/team-name.x_y",
            ...
            assert!(is_valid_codeowner(valid), "{valid} must be valid");
        }
        for invalid in [
            "",
            "@",
            ...
            assert!(!is_valid_codeowner(invalid), "{invalid} must be refused");
        }
    }
