...
/// or reuse a revision.
pub const GENERATOR_REVISION: &str = "25";
...
const VELNOR_POLICY_WORKFLOW_REV: &str = "f6c84657a1440e12592cbf43af75c8e5f1483a4b";
...
// `workflow_dispatch`; the reachable integrated ancestor above also carries
// the secure same-repository Velnor PR-gate parser. Bump after publishing a
// Velnor commit that changes the workflow runtime contract.
const VELNOR_WORKFLOW_SOURCE_REV: &str = "f6c84657a1440e12592cbf43af75c8e5f1483a4b";
...
impl ProjectConfig {
    ...
    fn toml(&self) -> String {
        ...
        // `automatic` controls generation only. It is intentionally absent
        // from the runtime project contract consumed by pinned older binaries.
        output.push('\n');
...
fn inferred_automatic(runners: RunnerMode) -> RunnerMode {
    match runners {
        RunnerMode::Both => RunnerMode::Velnor,
        other => other,
    }
}
...
mod tests {
    ...
    #[test]
    fn hosted_jobs_install_the_pinned_runtime_without_a_runtime_artifact() {
        let config = must(
            scan_repository_with_default_branch(&fixture_root(), RunnerMode::Github, "main"),
            "scan fixture for hosted runtime setup",
        );
        ...
        assert!(!workflow.contains("Prepare Velnor workflow runtime"));
...
"#,
        ...
        assert!(
            surface.contains("name: Set up Velnor workflow runtime"),
            "Linux hosted jobs install the pinned runtime directly: {surface}"
        );
        assert!(
            !surface.contains("name: Download Velnor workflow runtime"),
            "Linux jobs must not consume a planning runtime artifact: {surface}"
        );
        let _ = fs::remove_dir_all(root);
    }
    ...
    fn docker_mutable_mount_seed_renders_restore_seed_and_trusted_export() {
        ...
        let contract = UnitContract {
            lanes: WorkflowIr::default_lane_jobs(config.runners, true),
    ...
    fn explicit_runner_modes_select_exact_trusted_ci_lanes() {
        ...
        assert!(velnor.contains("github.event_name == 'push' || github.event_name == 'schedule'"));
        assert!(velnor.contains("github.event.inputs.runner == ''"));
    ...
    #[test]
    fn omitted_automatic_prefers_velnor_when_both_lanes_are_available() {
        assert_eq!(inferred_automatic(RunnerMode::Both), RunnerMode::Velnor);
        assert_eq!(inferred_automatic(RunnerMode::Github), RunnerMode::Github);
        assert_eq!(inferred_automatic(RunnerMode::Velnor), RunnerMode::Velnor);
    }
    ...
    fn both_with_automatic_both_runs_velnor_on_trusted_push() {
        ...
        assert!(main.contains("github.event_name == 'push' || github.event_name == 'schedule'"));
        assert!(main.contains("github.event.inputs.runner == ''"));
        let pr = WorkflowIr::from_config(&config).render(WorkflowKind::PullRequest);
        assert!(
