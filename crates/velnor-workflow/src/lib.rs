...
/// or reuse a revision.
pub const GENERATOR_REVISION: &str = "53";
...
pub struct Unit {
    ...
    /// Shape-derived Apple host contract. It remains in the scan shape so a
    /// changed SDK/toolchain/architecture obligation changes the generation
    /// digest, but is omitted from the pinned runtime TOML (the renderer
    /// consumes it while generating hosted jobs).
    pub(crate) apple_native: Option<native_contract::AppleNativeContract>,
...
fn scan_target(
    ...
    platform::validate_native_host_contract(&mut config)?;
        ...
        let apple_native = if let Some(native) = explicit_native {
            if !platform.requires_apple() {
                if row.os().is_some() || row.capabilities().is_some() {
                    return Err(GeneratorError::usage(format!(
                        "[[units]] {id}.native requires an Apple platform; remove the portable os/capabilities override"
                    )));
                }
...
fn generation_reasons(config: &ProjectConfig) -> Vec<String> {
    ...
    if config
        .units
        .iter()
        .any(|unit| unit.kind == UnitKind::Swift && !platform::unit_requires_native(unit))
    {
        reasons.push(
