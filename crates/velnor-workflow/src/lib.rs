...
/// or reuse a revision.
pub const GENERATOR_REVISION: &str = "45";
...
/// After a cache hit, prove every requested cargo-bin tool is on PATH. The
/// save step used to run before install on cold runs, so older cache entries
/// can contain only the workflow runtime binary and skip deny/nextest installs.
