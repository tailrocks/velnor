...
/// or reuse a revision.
pub const GENERATOR_REVISION: &str = "37";
...
const VELNOR_POLICY_WORKFLOW_REV: &str = "d3e441fb36dc0fee7502cc08fdc7bebf8c8bf754";
...
const VELNOR_WORKFLOW_SOURCE_REV: &str = "d3e441fb36dc0fee7502cc08fdc7bebf8c8bf754";
...
"#,
    ...
    #[test]
    fn inline_policy_install_step_reuses_rev_keyed_cached_runtime() {
        let policy = inline_policy_job_for_lane("Policy", "abc123", "ubuntu-24.04", "github", None);
        let cached = "$HOME/.cargo/bin/velnor-workflow-policy-abc123";
        assert!(policy.contains(&format!("CACHED_POLICY_RUNTIME=\"{cached}\"")));
        assert!(policy.contains("if [ -x \"$CACHED_POLICY_RUNTIME\" ]; then"));
        assert!(policy.contains(
            "ln -sf \"velnor-workflow-policy-abc123\" \"$HOME/.cargo/bin/velnor-workflow\""
        ));
    ...
    fn velnor_workspace_check_barrier_is_root_scoped_and_hosted_independent() {
        ...
        assert_eq!(
            job_needs("velnor-rust-workspace").as_deref(),
            Some("[velnor-prepare-cargo-sources]")
        );
        assert_eq!(
            job_needs("velnor-rust-model").as_deref(),
            Some("[velnor-prepare-cargo-sources, velnor-rust-workspace]")
        );
        assert_eq!(
            job_needs("velnor-rust-child").as_deref(),
            Some("[velnor-prepare-cargo-sources, velnor-rust-workspace, velnor-rust-model]")
        );
        assert_eq!(
            job_needs("velnor-rust-contract").as_deref(),
            Some("[velnor-prepare-cargo-sources]")
        );
