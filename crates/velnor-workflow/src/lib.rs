...
/// or reuse a revision.
pub const GENERATOR_REVISION: &str = "67";
...
/// Names a PR candidate binary used only to diagnose whether generated files
/// match the audited head. It never satisfies the declared pin lookup.
pub const VELNOR_WORKFLOW_CANDIDATE_BINARY_ENV: &str = "VELNOR_WORKFLOW_CANDIDATE_BINARY";
/// Names the manifest binding the diagnostic candidate binary: the
/// `candidate-manifest.json` the policy job downloads beside that binary.
/// The candidate is executed only when this manifest's closure equals the
...
pub struct Unit {
    ...
    /// Versioned, phase-explicit Rust runtime contract. Non-Rust units omit
    /// this field; each Rust scanner/config path constructs it from explicit
    /// phase facts instead of classifying command text.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub(crate) rust_validation: Option<crate::rust_validation::RustValidationContract>,
    ...
    /// Generation-time only: the rendered `runs-on` carries the decision.
    pub(crate) platform: platform::PlatformRequirement,
...
impl config::mise_closure::RustToolchainLike for RustToolchain {
    fn channel(&self) -> &str {
        self.channel()
    }
...
pub(crate) struct RenovateSpec {
    ...
    /// Writer execution lanes, independent of CI unit lanes.
    pub(crate) lanes: RunnerMode,
...
pub(crate) struct CheckProfileSpec {
    ...
    /// Typed Rust provisioning required by this profile's selected Mise
    /// closure. `None` keeps Rust out of profiles that do not use it.
    pub(crate) rust_toolchain: Option<RustToolchain>,
...
pub struct ProjectConfig {
    ...
    /// Repository-scoped Velnor runner concurrency group for PR recovery;
...
/// A Swift unit the scan cannot prove Apple-bound renders on the lane's
/// default executor, where no Swift toolchain is provisioned by the
/// generator. Name those units in the contract notes so an Apple-bound
/// package (Xcode schemes, `XCFramework` binary targets, Apple-only SDKs)
/// the scan missed gets its `capabilities` declaration instead of a
/// toolchain failure three jobs into CI.
fn refresh_swift_executor_note(config: &mut ProjectConfig) {
    const PREFIX: &str = "Swift units render on the default executor: ";
    ...
    let defaulted: Vec<String> = config
        ...
        .filter(|unit| {
            unit.kind == UnitKind::Swift
                && platform::github_runner_for_unit(
    ...
    let runner = config.github_runner.clone();
    config.notes.push(format!(
        "{PREFIX}{} (no Apple requirement detected; if Apple-bound, declare `capabilities = [\"xcode\"]` or `[\"xcframework\"]` on a [[units]] row, otherwise ensure a Swift toolchain provisions on {runner}).",
        defaulted.join(", "),
    ));
}
...
fn apply_renovate(
    ...
    let lanes = match renovate.lanes().unwrap_or("velnor") {
        "github" => RunnerMode::Github,
        "velnor" => RunnerMode::Velnor,
        "both" => RunnerMode::Both,
        other => {
            return Err(GeneratorError::usage(format!(
                "[renovate] lanes must be one of: velnor, github, both; found `{other}`"
            )));
        }
    };
...
fn apply_check_profiles(
    ...
    let rust_toolchain = scan::rust::parse_rust_toolchain_from_dir(root)?;
    ...
    for row in rows {
        ...
        let closure = config::mise_closure::resolve_profile_for_platform_with_rust(
            root,
            tasks,
            tools,
            &platform,
            rust_toolchain
                .as_ref()
                .map(|toolchain| toolchain as &dyn config::mise_closure::RustToolchainLike),
        )?;
        closure.validate_profile_tools(id, tools)?;
        let install_tools = closure.installation_plan(id)?.mise_args(id)?;
        let profile_rust_toolchain = closure
            .uses_rust_toolchain
            .then(|| rust_toolchain.clone())
            .flatten();
        profiles.push(CheckProfileSpec {
            ...
            rust_toolchain: profile_rust_toolchain,
            tools: install_tools,
            tasks: tasks.to_vec(),
...
fn apply_unit_row(
    ...
    let Some(unit) = existing else {
        ...
        let platform = row.platform_requirement(id, &platform::PlatformRequirement::portable())?;
    ...
    if row.os().is_some() || row.arch().is_some() || row.capabilities().is_some() {
        unit.platform = row.platform_requirement(id, &unit.platform.clone())?;
    }
...
/// Return the host identity reported by the selected Rust toolchain.
///
/// Component listings use bare names for target-independent components and
/// `<component>-<host>` for host-qualified components. The host must come
/// from the toolchain being checked; accepting an arbitrary suffix would let
/// a different target satisfy the preinstalled-toolchain contract.
pub(crate) fn rustup_host_command(channel: &str) -> String {
    format!(
        "rustup run {} rustc -vV | awk '$1 == \"host:\" {{ print $2; exit }}'",
        shell_quote(channel)
    )
}

/// Return an exact Rustup component-presence check for the selected host.
///
/// Rustup prints target-independent entries such as `rust-src` and
/// host-qualified entries such as `rustfmt-aarch64-apple-darwin`. Matching the
/// first field exactly avoids accepting a lookalike prefix such as
/// `rust-src-preview-aarch64-apple-darwin`.
pub(crate) fn rustup_component_check_command(channel: &str, component: &str) -> String {
    format!(
        "rustup component list --toolchain {} --installed | awk -v want={} -v host=\"$rustup_host\" '$1 == want || $1 == want \"-\" host {{ found=1 }} END {{ exit !found }}'",
        shell_quote(channel),
...
/// Owner policy step acquiring a same-repository PR candidate only as a
/// diagnostic. The trusted declared-pin renderer gets first chance to
/// reproduce the tree, so source-only and pin-adoption PRs need no candidate
/// artifact. A candidate can explain a stale generated tree, but never
/// authorizes it; the policy still requires the published pin. Mainline and
/// dispatch events skip this step entirely. The candidate manifest binds the
/// artifact to the audited head's candidate closure and binary digest. The
/// `--closure` probe runs with both tokens emptied, and the policy command
...
fn policy_candidate_step() -> String {
    format!(
        ...
        run: |
          ...
          renderer="$(command -v velnor-workflow)"
          pinned_binary="${{{VELNOR_WORKFLOW_PINNED_BINARY_ENV}:-}}"
          if [[ -n "$pinned_binary" ]]; then
            renderer="$pinned_binary"
          fi
          if "$renderer" --plain --check; then
            echo "the declared published renderer reproduces the tree; no candidate product is needed"
            exit 0
          fi
          echo "the declared published renderer differs; checking the PR candidate for diagnosis"
          ...
          head_candidate="$("$renderer" closure --rev="$HEAD_SHA" --candidate)"
...
fn policy_renderer_steps(repository: &str, revision: &str) -> String {
    let setup_uses = workflow_setup_action_uses(repository, revision);
    policy_renderer_steps_with_setup(&setup_uses, revision)
}

/// Read the audited pin and acquire its verified product through a trusted
/// setup action. The owner passes the action checked out from the PR base;
/// consumers pass their published action pin. The product is skipped when
/// the already-running Stage-0 validator has the same declared revision.
fn policy_renderer_steps_with_setup(setup_uses: &str, revision: &str) -> String {
    format!(
...
pub(crate) fn render_policy_entrypoint(config: &ProjectConfig) -> String {
    let velnor = config.runners == RunnerMode::Velnor;
    let gate = velnor.then(|| control_plane_trusted_gate(&config.default_branch));
...
pub(crate) fn lane_supports_unit(lane: RunnerMode, unit: &Unit) -> bool {
    lane_supports_unit_kind(lane, unit.kind)
        && platform::lane_supports_platform(lane, &unit.platform)
}
...
fn validate_both_lane_unit_coverage(
    ...
    for unit in &config.units {
        let github_ok = lane_supports_unit(RunnerMode::Github, unit);
        let velnor_ok = lane_supports_unit(RunnerMode::Velnor, unit);
        ...
        let unsupported = if lane_supports_unit_kind(
            if velnor_ok {
                RunnerMode::Github
            } else {
                RunnerMode::Velnor
            ...
            "its platform requirement the lane cannot serve"
        } else {
            "this kind"
        };
        return Err(GeneratorError::usage(format!(
            "runners=both cannot generate unit `{}` ({}); the {missing} lane cannot run {unsupported}. Declare `jobs = [\"github\"]` (or `[\"velnor\"]`) as an explicit opt-out, or remove the unit",
            unit.id,
            unit.kind.label(),
        )));
    }
...
/// Hosted runtime setup for jobs that also run the policy check: a single
/// setup of the declared pin, which is both the runtime on PATH and the
/// policy binary. No second revision is ever provisioned next to it.
fn workflow_runtime_setup(lane: RunnerMode, repository: &str, revision: &str) -> String {
...
///
/// The runtime product is identified by the generator revision the tree
...
/// one directory per revision, holding the product layout
/// (`bin/velnor-workflow`) and the product `manifest.json`. The directory is
/// the action's cache path and the landing place of a downloaded product, so
/// a caller that needs a specific revision addresses it by path instead of by
...
pub(crate) fn hosted_mold_setup(default_branch: &str, cache_save: bool) -> String {
    ...
    let mut output = format!(
        ...
        ActionPin::CacheRestore.reference(),
        curl_flags = crate::runtime::CURL_DOWNLOAD_FLAGS,
    );
    if cache_save {
...
#[cfg(test)]
mod hosted_mold_setup_tests {
    use super::hosted_mold_setup;

    /// The mold download carries the canonical bounded-download flags:
    /// retries and time bounds travel together, never one without the other.
    #[test]
    fn mold_download_uses_the_canonical_bounded_flags() {
        let setup = hosted_mold_setup("main", true);
        assert!(
            setup.contains(crate::runtime::CURL_DOWNLOAD_FLAGS),
            "mold must download with the canonical flags: {setup}"
        );
    }
}

/// Restore the shared `~/.cargo/bin` cache for hosted jobs that provision
/// policy and test tools through `taiki-e/install-action` instead of mise.
pub(crate) fn hosted_cargo_bin_toolchain_restore() -> String {
    format!(
        "      - name: Restore cargo bin toolchain\n        id: cargo-bin-toolchain\n        uses: {}\n        with:\n          path: ~/.cargo/bin\n          key: velnor-cargo-bin-v2-${{{{ runner.os }}}}-${{{{ runner.arch }}}}-${{{{ hashFiles('mise.lock', 'mise.toml') }}}}\n",
        ActionPin::CacheRestore.reference()
    )
}

/// After a cache hit, prove every requested cargo-bin tool is on PATH and
/// registers as a working cargo subcommand. Stale v1 cache entries and mise
...
/// Persist `~/.cargo/bin` only after install so cached snapshots include the
/// requested tools, not just the workflow runtime added earlier in the job.
pub(crate) fn hosted_cargo_bin_toolchain_save(default_branch: &str) -> String {
    let trusted = primitives::trusted_cache_save_expression(default_branch);
    format!(
        "      - name: Save cargo bin toolchain\n        if: ({trusted}) && (steps.cargo-bin-toolchain.outputs.cache-hit != 'true' || steps.cargo-bin-verify.outputs.missing == 'true')\n        uses: {}\n        with:\n          path: ~/.cargo/bin\n          key: velnor-cargo-bin-v2-${{{{ runner.os }}}}-${{{{ runner.arch }}}}-${{{{ hashFiles('mise.lock', 'mise.toml') }}}}\n",
        ActionPin::CacheSave.reference()
    )
}

#[cfg(test)]
mod hosted_cargo_bin_toolchain_tests {
    use super::{
        hosted_cargo_bin_toolchain_restore, hosted_cargo_bin_toolchain_save,
        hosted_cargo_bin_toolchain_verify,
    };

    #[test]
    fn save_follows_install_and_repairs_stale_cache_hits() {
        let block = format!(
            "{}{}{}\n{}",
            hosted_cargo_bin_toolchain_restore(),
            hosted_cargo_bin_toolchain_verify("${{ inputs.cargo_bin_tools }}"),
...
/// The runtime-product producer is owner-only infrastructure: the repository
/// that ships the setup action owns `ci-runtime-products.yml` without
/// declaring it, so the publisher can never fall out of the owner's surface
/// through a config edit. Consumers never carry the file; the renderer
/// refuses them, and an adopted surface keeps its reviewed templates.
fn add_owner_runtime_products_file(config: &mut ProjectConfig) {
    if !config.adopted_workflow_surface
...
fn generated_release_package_signer(config: &ProjectConfig) -> String {
    primitives::release::release_signer_content(&config.workflow_revision)
}
...
fn render_actionlint_config(config: &ProjectConfig) -> String {
    ...
    let macos = (config
        .units
        .iter()
        .any(|unit| unit.platform.requires_apple())
        || apple_release)
...
fn report_unit_runners(config: &ProjectConfig, unit: &Unit) -> String {
    let github =
        platform::github_runner_for_unit(&config.github_runner, &config.macos_runner, unit);
    let velnor_supported = lane_supports_unit(RunnerMode::Velnor, unit);
    match config.runners {
        RunnerMode::Github => format!("github: {github}"),
        RunnerMode::Velnor => {
            if velnor_supported {
                format!(
                    "velnor: {}",
            ...
            } else if unit.kind == UnitKind::Swift {
                "velnor: skipped (Apple unit requires macOS)".to_owned()
            } else {
                "velnor: skipped (unit requires macOS)".to_owned()
            }
        }
        RunnerMode::Both => {
            if velnor_supported {
                format!(
                    "github: {github}; velnor: {}",
                    velnor_runner(&config.velnor_labels, velnor_runner_group(config))
                )
            } else {
                format!(
                    "github: {github}; velnor: unsupported (declare jobs = [\"github\"] to opt out)"
                )
            }
        }
    }
...
fn generation_reasons(config: &ProjectConfig) -> Vec<String> {
    ...
    if config
        .units
        .iter()
        .any(|unit| unit.kind == UnitKind::Swift && !unit.platform.requires_apple())
    {
        reasons.push(
...
mod tests {
    ...
    fn hosted_runtime_setup_uses_the_versioned_setup_action() {
        ...
        assert!(
            plan.contains(&format!("rev: {FIXTURE_REVISION}")),
    ...
    fn install_rev_is_always_the_declared_pin() {
        assert_eq!(
            ...
            FIXTURE_REVISION,
            "the owner installs the declared pin: an event SHA (the synthetic merge commit on pull_request) can never name a trusted product"
        );
        assert_eq!(
            workflow_setup_install_rev("example/consumer", FIXTURE_REVISION),
            FIXTURE_REVISION,
            "a consumer installs the published pin"
        );
        ...
        assert!(
            maintenance.contains(&format!("rev: {FIXTURE_REVISION}")),
            "maintenance installs the declared pin: {maintenance}"
        );
        ...
        assert!(
            ...
            "foreign Planning must not install a foreign github.sha: {planning}"
        );
        let mut home = config.clone();
        home.repository = workflow_setup_action_repository().to_owned();
        let home_workflow = WorkflowIr::from_config(&home).render(WorkflowKind::Main);
    ...
    fn runtime_lane_consumes_platform_qualified_verified_products() {
        let mut output = String::new();
        WorkflowIr::from_config(&scanned_fixture(RunnerMode::Github))
            .render_workflow_runtime_download(&mut output, RunnerMode::Github);
    ...
    fn hosted_runtime_artifact_carries_a_proven_policy_binary() {
        ...
        WorkflowIr::from_config(&scanned_fixture(RunnerMode::Github))
            .render_workflow_runtime_download(&mut download, RunnerMode::Github);
        assert!(
    ...
    fn planning_provisions_a_single_pinned_runtime() {
        ...
        let foreign =
            workflow_runtime_setup(RunnerMode::Github, "example/consumer", FIXTURE_REVISION);
        assert!(
    ...
    fn this_binary_reports_its_own_source_closure() {
        assert!(
            crate::closure::is_full_closure(SOURCE_CLOSURE) || SOURCE_CLOSURE == "unknown",
    ...
    fn swift_units_on_the_default_executor_are_named_in_contract_notes() {
        ...
        let mut config = ProjectConfig {
            units: vec![
                unit(
                    "swift-portable",
                    crate::platform::PlatformRequirement::swift_package(),
                ),
                unit(
                    "swift-apple",
                    crate::platform::PlatformRequirement::apple_xcframework(),
                ),
            ],
        ...
        assert!(
            config.notes[0].contains("capabilities"),
            "{}",
            config.notes[0]
        );
    ...
    fn scanner_extracts_package_json_manager_scripts_and_package_dependencies() {
        ...
        must(
            ...
            "create root package asset directory",
        );
        must(
            fs::write(root.join(".oxlintrc.jsonc"), "{\"rules\":{}}\n"),
            "write root package config",
        );
        must(
            fs::write(root.join("assets/logo.svg"), "<svg/>\n"),
            "write root package asset",
        );
        must(
            fs::write(
                app.join("src/schema.graphql"),
                "type Query { ok: Boolean }\n",
            ),
            "write nested package input",
        );
        must(
            fs::write(app.join("src/fixture.snapshot"), "fixture\n"),
            "write nested package unknown extension",
        );
        let config = must(
        ...
        assert!(
            root_unit.watch.contains(&"**".to_owned()),
            "root package must cover arbitrary script/config inputs: {:?}",
            root_unit.watch
        );
        assert!(
            app_unit.watch.contains(&"packages/app/**".to_owned()),
            "nested package must stay scoped to its own root: {:?}",
            app_unit.watch
        );
        assert!(config.analysis.limitations.iter().any(|reason| {
            reason.contains("complete package root") && reason.contains("packages/app")
        }));
        assert!(config
            .analysis
            .detected
    ...
    fn scanner_marks_xcframework_binary_target_packages_apple_bound() {
        ...
        must(
            fs::create_dir_all(root.join("Sources/Plain")),
            "create plain sources",
        );
        must(
            fs::create_dir_all(root.join("bridge/Sources/Bridge")),
        ...
        must(
            fs::write(
                root.join("bridge/Package.swift"),
    ...
    fn macos_runner_label_overrides_the_apple_lane() {
        ...
        // Ordinary SwiftPM support implies no Apple placement: it verifies on
        // the lane's default executor. Only the Xcode scheme needs macOS.
        let package_surface =
...
"##;
    ...
    #[expect(
        clippy::too_many_lines,
        reason = "the test pins one explicit unit per lane-command combination"
    )]
...
"#,
    ...
    fn scanned_nextest_commands_tolerate_test_less_crates() {
        ...
        assert!(
            clippy_index < nextest_index,
            "clippy must report lint failures before nextest runs: {}",
            rust.pr_commands.join(" | ")
        );
        assert!(
...
"#,
        ...
        let surface = ir.render_nested_unit(xcode, WorkflowKind::PullRequest);
        assert!(
        ...
        // A portable SwiftPM unit runs on the default executor, so it
        // consumes the Linux plan artifact like every other Linux job.
        let package = must_some(
            config
                .units
                .iter()
        ...
        let surface = ir.render_nested_unit(package, WorkflowKind::PullRequest);
        assert!(
            surface.contains("name: Download Velnor workflow runtime"),
            "SwiftPM jobs on Linux consume the plan artifact: {surface}"
        );
        let rust = must_some(
            config.units.iter().find(|unit| unit.kind == UnitKind::Rust),
            "Rust unit",
    ...
    fn assert_phase_report_workflow(workflow: &str, report_uses: &str) {
        ...
        assert!(!workflow.contains("- name: Mark cleanup start"));
        assert!(!workflow.contains("- name: Mark cleanup end"));
        for marker in [
            "JOB_STARTED",
            "RUNNER_SETUP_ENDED",
            "SELECTION_ENDED",
            ...
            "CACHE_SEED_SAVE_STARTED",
            "CACHE_SEED_SAVE_ENDED",
            "CACHE_BUNDLE_SAVE_STARTED",
            "CACHE_BUNDLE_SAVE_ENDED",
        ] {
    ...
    fn run_report_action_case_at(
        ...
        run_report_action_case_with_env_and_log_at(
            name,
            markers,
            run_started_at,
            queue_time,
    ...
    fn run_report_action_case_with_env_and_log_at(
        name: &str,
        markers: &[(&str, &str)],
        run_started_at: Option<&str>,
        queue_time: Option<&str>,
    ...
    fn assert_malformed_cache_saves(now: u64, job_started: u64) {
        let malformed_save = run_report_action_case(
            "malformed-cache-save",
            &[
                ("JOB_STARTED", &job_started.to_string()),
                ("CACHE_BUNDLE_SAVE_STARTED", &(now - 6).to_string()),
                ("CACHE_BUNDLE_SAVE_ENDED", "not-an-epoch"),
            ],
            None,
            None,
        );
        assert!(malformed_save["cache_save_seconds"].is_null());
        assert_eq!(
            malformed_save["cache_save_observation"],
            serde_json::json!("explicit_cache_save_step_incomplete")
        );

        let reversed_save = run_report_action_case(
            "reversed-cache-save",
            &[
                ("JOB_STARTED", &job_started.to_string()),
                ("CACHE_BUNDLE_SAVE_STARTED", &(now - 4).to_string()),
                ("CACHE_BUNDLE_SAVE_ENDED", &(now - 5).to_string()),
            ],
            None,
            None,
        );
        assert!(reversed_save["cache_save_seconds"].is_null());
        assert_eq!(
            reversed_save["cache_save_observation"],
            serde_json::json!("explicit_cache_save_step_incomplete")
        );

        let reversed_phases = run_report_action_case(
            "reversed-cache-phase-order",
            &[
                ("JOB_STARTED", &job_started.to_string()),
                ("CHECKS_STARTED", &(now - 12).to_string()),
                ("CHECKS_ENDED", &(now - 10).to_string()),
                ("CACHE_BUNDLE_SAVE_STARTED", &(now - 9).to_string()),
                ("CACHE_BUNDLE_SAVE_ENDED", &(now - 8).to_string()),
                ("CACHE_SEED_SAVE_STARTED", &(now - 7).to_string()),
                ("CACHE_SEED_SAVE_ENDED", &(now - 6).to_string()),
            ],
            None,
            None,
        );
        assert!(reversed_phases["cache_save_seconds"].is_null());
        assert_eq!(
            reversed_phases["cache_save_observation"],
            serde_json::json!("explicit_cache_save_phase_invalid")
        );
        assert_eq!(
            reversed_phases["cache_save_observed_phases"],
            serde_json::json!([])
        );
    }

    fn assert_combined_cache_saves(
        now: u64,
        job_started: u64,
        checks_started: u64,
        checks_ended: u64,
    ) {
        let combined = run_report_action_case(
            "combined-cache-saves",
            &[
                ("JOB_STARTED", &job_started.to_string()),
                ("CHECKS_STARTED", &checks_started.to_string()),
                ("CHECKS_ENDED", &checks_ended.to_string()),
                ("CACHE_SEED_SAVE_STARTED", &(now - 10).to_string()),
                ("CACHE_SEED_SAVE_ENDED", &(now - 8).to_string()),
                ("CACHE_BUNDLE_SAVE_STARTED", &(now - 7).to_string()),
                ("CACHE_BUNDLE_SAVE_ENDED", &(now - 4).to_string()),
                ("CLEANUP_STARTED", &(now - 3).to_string()),
                ("CLEANUP_ENDED", &(now - 2).to_string()),
            ],
            None,
            None,
        );
        assert_eq!(combined["cache_seed_save_seconds"], serde_json::json!(2));
        assert_eq!(combined["cache_bundle_save_seconds"], serde_json::json!(3));
        assert_eq!(combined["cache_save_seconds"], serde_json::json!(5));
        assert_eq!(
            combined["cache_save_observation"],
            serde_json::json!("explicit_cache_save_step")
        );
        assert_eq!(
            combined["cache_save_observed_phases"],
            serde_json::json!(["docker_seed", "unit_bundle"])
        );
    }
    ...
    fn assert_incomplete_and_skipped(
        ...
        let incomplete = run_report_action_case(
            "incomplete-candidate-cache",
            &[
                ("JOB_STARTED", &job_started.to_string()),
                ("CHECKS_STARTED", &checks_started.to_string()),
                ("CHECKS_ENDED", &checks_ended.to_string()),
                ("CANDIDATE_STARTED", &(now - 12).to_string()),
                ("CANDIDATE_PREPARATION_STARTED", &(now - 11).to_string()),
                ("CANDIDATE_PUBLICATION_STARTED", &(now - 9).to_string()),
                ("CACHE_BUNDLE_SAVE_STARTED", &(now - 6).to_string()),
                ("CLEANUP_STARTED", &(now - 4).to_string()),
                ("CLEANUP_ENDED", &(now - 3).to_string()),
            ],
            None,
            None,
        );
        assert!(incomplete["candidate_seconds"].is_null());
        assert!(incomplete["candidate_preparation_seconds"].is_null());
        assert!(incomplete["candidate_publication_seconds"].is_null());
        assert!(incomplete["cache_save_seconds"].is_null());
        assert_eq!(
            incomplete["cache_save_observation"],
            serde_json::json!("explicit_cache_save_step_incomplete")
        );
        assert_eq!(
            incomplete["cache_save_observed_phases"],
            serde_json::json!([])
        );
        assert!(incomplete["cache_post_job_observation"].is_null());
        assert!(incomplete["cleanup_seconds"].is_null());
        assert_eq!(
            incomplete["cleanup_observation"],
            serde_json::json!("unobserved_no_renderer_boundary")
        );

        let skipped = run_report_action_case(
            "skipped-candidate-cache",
            &[
                ("JOB_STARTED", &job_started.to_string()),
                ("CHECKS_STARTED", &checks_started.to_string()),
                ("CHECKS_ENDED", &checks_ended.to_string()),
                ("CANDIDATE_STARTED", &(now - 8).to_string()),
                ("CANDIDATE_PREPARATION_STARTED", &(now - 7).to_string()),
                ("CANDIDATE_PREPARATION_ENDED", &(now - 6).to_string()),
                ("CLEANUP_STARTED", &(now - 4).to_string()),
                ("CLEANUP_ENDED", &(now - 3).to_string()),
            ],
            None,
        ...
        assert!(skipped["candidate_seconds"].is_null());
        assert_eq!(
            skipped["candidate_preparation_seconds"],
            serde_json::json!(1)
        );
        assert!(skipped["candidate_publication_seconds"].is_null());
        assert_eq!(
            skipped["candidate_observation"],
            serde_json::json!("candidate_phase_incomplete")
        );
        assert!(skipped["cache_save_seconds"].is_null());
        assert!(skipped["cache_save_observation"].is_null());
        assert_eq!(skipped["cache_save_observed_phases"], serde_json::json!([]));
        assert!(skipped["cache_post_job_observation"].is_null());
        assert!(skipped["cleanup_seconds"].is_null());
        assert_eq!(
            skipped["cleanup_observation"],
            serde_json::json!("unobserved_no_renderer_boundary")
        );
    }

    fn assert_invalid_cross_phase_graph(now: u64, job_started: u64) {
        let invalid_graph = run_report_action_case(
            "cross-phase-overlap",
            &[
                ("JOB_STARTED", &job_started.to_string()),
                ("CHECKS_STARTED", &(now - 10).to_string()),
                ("CHECKS_ENDED", &(now - 5).to_string()),
                ("CANDIDATE_STARTED", &(now - 4).to_string()),
                ("CANDIDATE_PREPARATION_STARTED", &(now - 20).to_string()),
                ("CANDIDATE_PREPARATION_ENDED", &(now - 19).to_string()),
                ("CANDIDATE_PUBLICATION_STARTED", &(now - 3).to_string()),
                ("CANDIDATE_PUBLICATION_ENDED", &(now - 2).to_string()),
                ("CANDIDATE_ENDED", &(now - 1).to_string()),
                ("CACHE_SEED_SAVE_STARTED", &(now - 20).to_string()),
                ("CACHE_SEED_SAVE_ENDED", &(now - 19).to_string()),
                ("CACHE_BUNDLE_SAVE_STARTED", &(now - 18).to_string()),
                ("CACHE_BUNDLE_SAVE_ENDED", &(now - 17).to_string()),
            ],
            None,
            None,
        );
        assert!(invalid_graph["candidate_seconds"].is_null());
        assert_eq!(
            invalid_graph["candidate_observation"],
            serde_json::json!("candidate_phase_order_invalid")
        );
        assert!(invalid_graph["cache_save_seconds"].is_null());
        assert_eq!(
            invalid_graph["cache_save_observation"],
            serde_json::json!("explicit_cache_save_phase_invalid")
        );
        assert_eq!(
            invalid_graph["cache_save_observed_phases"],
            serde_json::json!([])
        );
    }

    fn assert_phase_report_intervals(
        ...
        assert_eq!(report["candidate_seconds"], serde_json::json!(5));
        assert_eq!(
            report["candidate_preparation_seconds"],
            serde_json::json!(1)
        );
        assert_eq!(
            report["candidate_publication_seconds"],
            serde_json::json!(1)
        );
        assert_eq!(
            report["candidate_observation"],
            serde_json::json!("candidate_phase_complete")
        );
        assert_eq!(report["cache_seed_save_seconds"], serde_json::Value::Null);
        assert_eq!(report["cache_bundle_save_seconds"], serde_json::json!(1));
        assert_eq!(report["cache_save_seconds"], serde_json::json!(1));
        assert_eq!(
            report["cache_save_observation"],
            serde_json::json!("explicit_cache_save_step")
        );
        assert_eq!(
            report["cache_save_observed_phases"],
            serde_json::json!(["unit_bundle"])
        );
        assert_eq!(
            report["cache_save_unobserved_phases"],
            serde_json::json!(["rustup", "mold", "cargo_bin", "mbx_post_job"])
        );
        assert_eq!(
            report["cache_post_job_observation"],
            serde_json::json!("mbx_post_job_unobserved")
        );
        assert!(report["cleanup_seconds"].is_null());
        assert_eq!(
    ...
    fn assert_invalid_candidate_graph(report: &serde_json::Value) {
        for field in [
            "candidate_seconds",
            "candidate_preparation_seconds",
            "candidate_publication_seconds",
            ...
            assert!(
                ...
                "{field} must be censored for invalid graph"
            );
        }
        assert_eq!(
            report["candidate_observation"],
    ...
    fn assert_candidate_graph_validation(
        now: u64,
        job_started: u64,
        checks_started: u64,
        checks_ended: u64,
    ) {
        let missing_end_invalid = run_report_action_case(
            "candidate-child-before-open-parent",
            &[
                ("JOB_STARTED", &job_started.to_string()),
                ("CHECKS_STARTED", &checks_started.to_string()),
                ("CHECKS_ENDED", &checks_ended.to_string()),
        ...
        assert_invalid_candidate_graph(&missing_end_invalid);

        let missing_middle_order = run_report_action_case(
            "candidate-observed-order-with-missing-preparation-end",
            &[
                ("JOB_STARTED", &job_started.to_string()),
                ("CHECKS_STARTED", &checks_started.to_string()),
                ("CHECKS_ENDED", &checks_ended.to_string()),
                ("CANDIDATE_STARTED", &(now - 30).to_string()),
                ("CANDIDATE_PREPARATION_STARTED", &(now - 20).to_string()),
                ("CANDIDATE_PUBLICATION_STARTED", &(now - 25).to_string()),
                ("CANDIDATE_PUBLICATION_ENDED", &(now - 24).to_string()),
            ],
            None,
            None,
        );
        assert_invalid_candidate_graph(&missing_middle_order);

        let child_before_checks_end = run_report_action_case(
            "candidate-child-before-checks-end",
            &[
                ("JOB_STARTED", &(now - 30).to_string()),
                ("CHECKS_STARTED", &(now - 20).to_string()),
                ("CHECKS_ENDED", &(now - 10).to_string()),
                ("CANDIDATE_PREPARATION_STARTED", &(now - 15).to_string()),
                ("CANDIDATE_PREPARATION_ENDED", &(now - 14).to_string()),
                ("CANDIDATE_PUBLICATION_STARTED", &(now - 13).to_string()),
                ("CANDIDATE_PUBLICATION_ENDED", &(now - 12).to_string()),
            ],
            None,
            None,
        );
        assert_invalid_candidate_graph(&child_before_checks_end);

        let candidate_before_job_start = run_report_action_case(
            "candidate-before-job-start",
            &[
                ("JOB_STARTED", &(now - 10).to_string()),
                ("CHECKS_STARTED", &(now - 8).to_string()),
                ("CHECKS_ENDED", &(now - 7).to_string()),
                ("CANDIDATE_STARTED", &(now - 20).to_string()),
                ("CANDIDATE_PREPARATION_STARTED", &(now - 19).to_string()),
                ("CANDIDATE_PREPARATION_ENDED", &(now - 18).to_string()),
                ("CANDIDATE_PUBLICATION_STARTED", &(now - 17).to_string()),
                ("CANDIDATE_PUBLICATION_ENDED", &(now - 16).to_string()),
                ("CANDIDATE_ENDED", &(now - 15).to_string()),
            ],
            None,
            None,
        );
        assert_invalid_candidate_graph(&candidate_before_job_start);

        let missing_start = run_report_action_case(
            "candidate-end-without-start",
            &[
                ("JOB_STARTED", &job_started.to_string()),
                ("CHECKS_STARTED", &checks_started.to_string()),
                ("CHECKS_ENDED", &checks_ended.to_string()),
        ...
        assert_invalid_candidate_graph(&missing_start);
    }

    fn assert_candidate_marker_bounds(
        now: u64,
        job_started: u64,
        checks_started: u64,
        checks_ended: u64,
    ) {
        let future = run_report_action_case_at(
            now,
            "future-candidate-markers",
            &[
                ("JOB_STARTED", &job_started.to_string()),
                ("CHECKS_STARTED", &checks_started.to_string()),
                ("CHECKS_ENDED", &checks_ended.to_string()),
                ("CANDIDATE_STARTED", &(now + 1).to_string()),
                ("CANDIDATE_PREPARATION_STARTED", &(now + 2).to_string()),
                ("CANDIDATE_PREPARATION_ENDED", &(now + 3).to_string()),
                ("CANDIDATE_PUBLICATION_STARTED", &(now + 4).to_string()),
                ("CANDIDATE_PUBLICATION_ENDED", &(now + 5).to_string()),
                ("CANDIDATE_ENDED", &(now + 6).to_string()),
            ],
            None,
            None,
        );
        assert_invalid_candidate_graph(&future);

        let stale = now.saturating_sub(8 * 24 * 60 * 60);
        let stale_candidate = run_report_action_case_at(
            now,
            "stale-candidate-markers",
            &[
                ("JOB_STARTED", &job_started.to_string()),
                ("CHECKS_STARTED", &checks_started.to_string()),
                ("CHECKS_ENDED", &checks_ended.to_string()),
                ("CANDIDATE_STARTED", &stale.to_string()),
                ("CANDIDATE_PREPARATION_STARTED", &(stale + 1).to_string()),
                ("CANDIDATE_PREPARATION_ENDED", &(stale + 2).to_string()),
                ("CANDIDATE_PUBLICATION_STARTED", &(stale + 3).to_string()),
                ("CANDIDATE_PUBLICATION_ENDED", &(stale + 4).to_string()),
                ("CANDIDATE_ENDED", &(stale + 5).to_string()),
            ],
            None,
            None,
        );
        assert_invalid_candidate_graph(&stale_candidate);
    }

    fn assert_report_edge_cases(
        now: u64,
        job_started: u64,
        checks_started: u64,
        checks_ended: u64,
        hosted_run_started: &str,
    ) {
        ...
        assert!(invalid["total_seconds"].is_null() && invalid["runner_setup_seconds"].is_null());
        ...
        assert!(future["total_seconds"].is_null() && future["checks_wall_seconds"].is_null());
        ...
        assert!(stale["total_seconds"].is_null() && stale["checks_wall_seconds"].is_null());
        ...
        assert_invalid_cross_phase_graph(now, job_started);
        assert_candidate_marker_bounds(now, job_started, checks_started, checks_ended);
        ...
        let env_override = run_report_action_case_with_env(
            ...
            None,
            &[
                ("VELNOR_JOB_STARTED_EPOCH", &(now - 20).to_string()),
                ("VELNOR_CHECKS_STARTED_EPOCH", &(now - 15).to_string()),
                ("VELNOR_CHECKS_ENDED_EPOCH", &(now - 5).to_string()),
            ],
        );
    ...
    fn report_action_prefers_fractional_queue_time_and_bounds_markers() {
        ...
        let report = run_report_action_case_with_env(
            ...
            &[
                ...
                ("CANDIDATE_PREPARATION_STARTED", &(now - 7).to_string()),
                ("CANDIDATE_PREPARATION_ENDED", &(now - 6).to_string()),
                ("CANDIDATE_PUBLICATION_STARTED", &(now - 5).to_string()),
                ("CANDIDATE_PUBLICATION_ENDED", &(now - 4).to_string()),
                ("CANDIDATE_ENDED", &(now - 3).to_string()),
                ("CACHE_BUNDLE_SAVE_STARTED", &(now - 2).to_string()),
                ("CACHE_BUNDLE_SAVE_ENDED", &(now - 1).to_string()),
                ("CLEANUP_STARTED", &(now - 4).to_string()),
                ("CLEANUP_ENDED", &(now - 3).to_string()),
            ],
            Some(&hosted_run_started),
            ...
            &[
                ...
                ("VELNOR_CACHE_MBX_PRIMARY", "mbx-key"),
            ],
        );
        ...
        assert_phase_report_intervals(&report, now, job_started, checks_started, checks_ended);

        assert_report_edge_cases(
            now,
            job_started,
            checks_started,
            checks_ended,
    ...
    fn every_runtime_install_owns_a_revision_addressed_root() {
        ...
        assert!(
            action.contains("steps.closure.outputs.value"),
            "the cache key is closure-addressed: {action}"
        );
        assert!(
            action.contains("default: velnor-workflow-v3"),
    ...
    fn assert_collapsed_seed_and_bundle_markers(config: &ProjectConfig, index: usize) {
        let mut collapsed_config = config.clone();
        let mut bundle_unit = collapsed_config.units[index].clone();
        bundle_unit.id = "docker-bundle".to_owned();
        bundle_unit.label = "Docker bundle".to_owned();
        must_some(bundle_unit.cache.as_mut(), "seed fixture has a cache").mutable_mount_seed =
            false;
        collapsed_config.units.push(bundle_unit);
        let collapsed_ir = WorkflowIr::from_config(&collapsed_config);
        let collapsed = must_some(
            must(
                collapsed_ir.render_kind_unit_workflow(UnitKind::Docker, None),
                "render docker kind reusable",
            ),
            "docker kind has members",
        )
        .1;
        assert_eq!(collapsed.matches("Mark SEED cache-save start").count(), 1);
        assert_eq!(collapsed.matches("Mark SEED cache-save end").count(), 1);
        assert_eq!(collapsed.matches("Mark BUNDLE cache-save start").count(), 1);
        assert_eq!(collapsed.matches("Mark BUNDLE cache-save end").count(), 1);
        assert!(
            collapsed.find("Mark SEED cache-save start")
                < collapsed.find("Mark SEED cache-save end")
        );
        assert!(
            collapsed.find("Mark SEED cache-save end")
                < collapsed.find("Mark BUNDLE cache-save start")
        );
        assert!(
            collapsed.find("Mark BUNDLE cache-save start")
                < collapsed.find("Mark BUNDLE cache-save end")
        );
    }
    ...
    fn docker_mutable_mount_seed_renders_restore_seed_and_trusted_export() {
        ...
        assert_eq!(rendered.matches("Mark SEED cache-save start").count(), 1);
        assert_eq!(rendered.matches("Mark SEED cache-save end").count(), 1);
        assert!(rendered.contains(
            "Mark SEED cache-save end\n        if: always() && (always() && ((github.event_name == 'push' && github.ref == 'refs/heads/main')"
        ));
        assert!(rendered.contains("steps.docker_seed_cache_save.outcome == 'success'"));

        assert_collapsed_seed_and_bundle_markers(&config, index);
    ...
    fn workflow_snapshot_enforces_security_shape() {
        ...
        assert!(
            workflow.contains(&format!("rev: {FIXTURE_REVISION}")),
            "the embedded policy job acquires the validator at the declared pin"
        );
        assert!(workflow.contains(&format!("{VELNOR_POLICY_REVISION_ENV}: {FIXTURE_REVISION}")));
        assert!(
            !workflow.contains(&format!("--rev {FIXTURE_REVISION}")),
            "no inline revision flag carries the pin"
        );
        assert!(!workflow.contains("cargo install"), "nothing compiles");
    ...
    fn generated_output_uses_binary_runtime_without_shell_helpers() {
        ...
        let policy_pin = format!("rev: {FIXTURE_REVISION}");
        ...
        assert!(
            policy_workflow.contains(&format!("{VELNOR_POLICY_REVISION_ENV}: {FIXTURE_REVISION}"))
        );
        assert!(policy_workflow.contains("Set up Velnor workflow runtime"));
        assert!(!policy_workflow.contains("cargo install"));
        assert!(!policy_workflow.contains("cargo build"));
        assert!(!policy_workflow.contains(&format!("--rev {FIXTURE_REVISION}")));
    ...
    fn generated_maintenance_splits_prune_and_cache_when_runners_are_velnor() {
        ...
        assert!(
            cache_budget.contains(
                "gh run list --repo \"$GITHUB_REPOSITORY\" --workflow \"$workflow\" --status in_progress --limit 1 --json databaseId --jq 'length'"
            ),
            "cache retention gate must query in-progress producers with --repo and --json: {cache_budget}"
        );
        assert!(
    ...
    fn generated_planning_uses_configured_velnor_lane_without_runtime_artifact() {
        ...
        assert_eq!(
            all_runs_on
                .iter()
                .filter(|runner| **runner == "runs-on: ubuntu-24.04")
                .count(),
            2,
            "only the two verdict jobs use hosted control"
        );
        for job in ["ci-required", "required"] {
            assert!(yaml_job(workflow, job).contains("runs-on: ubuntu-24.04"));
        }
        assert!(all_runs_on
            .iter()
            .all(|runner| { *runner == velnor.as_str() || *runner == "runs-on: ubuntu-24.04" }));
    ...
    fn hosted_policy_job(revision: &str) -> String {
        hosted_policy_job_for_repository(revision, regen_repository_marker())
    }
    ...
    /// would load the tree's `mise.toml` `[env]` (including `_.path` prepends)
    /// before executing PATH-resolved helpers: fork RCE plus finding
    /// suppression. `MISE_NO_CONFIG=1` on the step is what neutralizes fork
    /// config; the explicit `actionlint@<version>` on the command line still
    ...
    /// The policy job checks out the base repository's full history and then
    /// fetches the audited head into it, so pin ancestry against the base
    ...
    fn no_policy_job_builds_pull_request_code() {
        ...
        assert!(
            hosted.contains("--ruleset-contexts \"$RULESET_CONTEXTS\""),
            "{hosted}"
        );
        assert!(
            hosted.contains("gh api \"repos/$GITHUB_REPOSITORY/rulesets?includes_parents=true\""),
            "{hosted}"
        );
        assert!(
    ...
    fn policy_job_acquires_validator_product_without_compiling() {
        ...
        assert!(
            policy.contains("      - name: Set up Velnor workflow runtime\n"),
            "{policy}"
        );
    ...
    fn no_generated_file_passes_pin_build() {
        ...
        // compile inside CI instead of failing closed.
        for repository in [workflow_setup_action_repository(), "example/consumer"] {
            let mut config = scanned_fixture(RunnerMode::Both);
            config.repository = repository.to_owned();
            let files = must(generated_files(&config), "generate");
            assert!(!files.is_empty(), "the surface renders");
            for (path, content) in &files {
                assert!(
                    !content.contains("--pin-build"),
                    "{} renders --pin-build for {repository}",
                    path.display()
                );
            }
        }
    ...
    fn both_runner_policy_entrypoint_stays_on_hosted_runner() {
        ...
        assert!(
            policy.contains("      - name: Set up Velnor workflow runtime\n"),
            "the entrypoint acquires the base validator as a product: {policy}"
        );
        assert!(
            policy.contains(&format!("rev: {FIXTURE_REVISION}")),
            "the validator install uses the declared pin: {policy}"
        );
        assert!(
        ...
        for runners in [RunnerMode::Github, RunnerMode::Both] {
            ...
            for surface in [pr.as_str(), nested.as_str()] {
                ...
                assert_eq!(
                    ...
                    "    if: ${{ always() }}",
                    "ci-required evaluates every terminal dependency, including cancellation: {surface}"
                );
            }
        }
    ...
    fn explicit_runner_modes_select_exact_trusted_ci_lanes() {
        ...
        assert_eq!(velnor.matches("runs-on: ubuntu-24.04").count(), 2);
        ...
        assert_eq!(velnor_pr.matches("runs-on: ubuntu-24.04").count(), 2);
    ...
    fn detected_cargo_source_caches_render_alongside_mr_boxington() {
        ...
        assert_eq!(workflow.matches("Mark BUNDLE cache-save start").count(), 1);
        assert_eq!(workflow.matches("Mark BUNDLE cache-save end").count(), 1);
        assert!(workflow.contains("id: unit_cache_save"));
        assert!(workflow.contains("steps.unit_cache_save.outcome == 'success'"));
    ...
    #[expect(
        clippy::too_many_lines,
        reason = "the test pins the fetch-then-verify contract across units and lanes"
    )]
    ...
    fn ci_main_and_ci_pr_concurrency_groups_differ() {
        ...
        assert!(
            main.contains("group: example-${{ github.repository }}-main-${{ github.run_id }}\n"),
            "ci-main must use a per-run derived -main concurrency group: {main}"
        );
        assert!(
            !main.contains("group: example-${{ github.repository }}-main\n"),
            "ci-main must not serialize unrelated runs by ref: {main}"
        );
        assert!(
            main.contains("cancel-in-progress: false"),
            "ci-main must not cancel a valid running pipeline: {main}"
        );
        assert!(
    ...
    #[test]
    fn default_aggregate_concurrency_keeps_main_runs_independent() {
        let config = header_fixture_config();
        ...
        let main = ir.render(WorkflowKind::Main);
        let nightly = ir.render(WorkflowKind::Nightly);
        let default_ref_group =
            "group: ci-${{ github.workflow }}-${{ github.event.pull_request.number || github.ref }}\n";
        assert!(
            pr.contains(default_ref_group),
            "unset velnor_concurrency_group must keep PR grouping by workflow+ref: {pr}"
        );
        assert!(
            nightly.contains(default_ref_group),
            "unset velnor_concurrency_group must keep nightly grouping by workflow+ref: {nightly}"
        );
        assert!(
            main.contains("group: ci-${{ github.workflow }}-${{ github.run_id }}\n"),
            "unset velnor_concurrency_group must use a unique group per main run: {main}"
        );
        assert!(
            !main.contains("github.event.pull_request.number || github.ref"),
            "main concurrency must not fall back to a shared ref group: {main}"
        );
        assert!(
            main.contains("cancel-in-progress: false"),
            "main concurrency must preserve non-cancellation: {main}"
        );
    }
