    ...
    fn generated_maintenance_splits_prune_and_cache_when_runners_are_velnor() {
        ...
        assert!(
            cache_budget.contains(
                "gh run list --workflow \"$workflow\" --status in_progress --limit 1 --json databaseId --jq 'length'"
            ),
            "cache retention gate must query in-progress producers with --json: {cache_budget}"
        );
        assert!(
