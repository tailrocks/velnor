...
/// or reuse a revision.
pub const GENERATOR_REVISION: &str = "7";
...
fn refresh_mr_boxington_note(config: &mut ProjectConfig) {
    ...
    let note = match config.runners {
        RunnerMode::Github => format!(
            "Rust verification uses Mr. Boxington {MR_BOXINGTON_VERSION} with its GitHub cache backend and stable per-unit keys (exact hits reuse the cache; a new entry is saved only when dependency inputs change)."
        ),
