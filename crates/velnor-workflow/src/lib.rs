...
/// Hosted runtime setup. `uses:` stays on the published action pin; `rev:` is
/// the `cargo install` / artifact revision the action resolves. CI Planning
/// pins `rev` to [`VELNOR_WORKFLOW_SOURCE_REV`]. Maintenance installs
/// `${{ github.sha }}` so a default-branch `workflow_dispatch` cargo-installs
/// HEAD.
