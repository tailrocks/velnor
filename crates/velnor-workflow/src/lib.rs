...
struct RawCli {
    ...
    /// Supported execution backends. Automatic events always run GitHub.
    /// Velnor runs only on explicit dispatch (`velnor` or `both`).
    #[arg(long, value_enum, default_value_t = RunnerMode::Github, value_name = "MODE")]
    runners: RunnerMode,
...
mod tests {
    ...
    fn velnor_runtime_does_not_publish_an_orphan_artifact() {
        ...
        let pr_plan = yaml_job(&pr, "plan");
        assert!(
            pr_plan.contains("name: Prepare Velnor workflow runtime"),
            "pull-request Planning publishes the GitHub runtime: {pr_plan}"
        );
        assert!(pr_plan.contains("name: Publish Velnor workflow runtime"));
    }
...
"##;
    ...
    fn generated_planning_uses_configured_velnor_lane_without_runtime_artifact() {
        ...
        let pr_plan = yaml_job(pr, "plan");
        assert!(
            pr_plan.contains("runs-on: ubuntu-24.04"),
            "pull-request Planning stays GitHub-hosted: {pr_plan}"
        );
        assert!(
            !pr_plan.contains(&velnor),
            "untrusted Planning must not use the Velnor lane: {pr_plan}"
        );
        for (path, content) in &files {
            ...
            if name.starts_with("ci-unit-") {
                assert!(
                    ...
                    "{name} unit jobs must keep the configured Velnor lane"
                );
                assert!(
                    content.contains("runs-on: ubuntu-24.04"),
                    "{name} unit jobs must keep the GitHub lane for pull requests"
                );
                assert!(
                    content.contains(
                    ...
                    "Velnor-first unit jobs run on trusted default-branch events: {name}"
                );
            }
        }
    }
        ...
        for runners in [RunnerMode::Github, RunnerMode::Velnor, RunnerMode::Both] {
            ...
            match runners {
                ...
                RunnerMode::Velnor => {
                    ...
                    assert!(workflow.contains("  github-"));
                    assert!(workflow.contains("  velnor-"));
                    assert!(workflow.contains("github.event_name == 'workflow_dispatch' && (github.event.inputs.runner == 'velnor' || github.event.inputs.runner == 'both')"));
                }
                RunnerMode::Both => {
                    assert!(workflow.contains("runs-on: ubuntu-24.04"));
                    assert!(workflow.contains(&fixture_lane_selector()));
