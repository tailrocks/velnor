...
use crate::scan::file_walk::{has_extension, is_test_support_path, repository_files};
...
const LEGACY_ACTIONLINT_CONFIG: &str = "self-hosted-runner:\n  labels:\n    - velnor-target-mvp\n";

/// The default self-hosted runner labels a scanned surface selects. This
/// mirrors the estate catalog's runner labels, so the two must stay in sync.
const DEFAULT_VELNOR_RUNNER_LABELS: &[&str] = &["self-hosted", "velnor-target-mvp"];

/// The default self-hosted runner labels, as the config stores them.
pub(crate) fn default_velnor_runner_labels() -> Vec<String> {
    DEFAULT_VELNOR_RUNNER_LABELS
        .iter()
...
struct CacheSpec {
    ...
    mbx_output_cache_justification: Option<String>,
}
...
impl CacheSpec {
    ...
    fn justified_output_alongside_mr_boxington(&self) -> bool {
...
fn scan_target(
    ...
    if let Some(generation) = &generation {
        ...
        generation.validate(
            &unit_ids,
            &apt_package_update_owner_blocks(APT_PACKAGE_UPDATE_WORKFLOW_TEMPLATE),
        )?;
    }
...
fn estate_workflow_files(profile: &EstateProfile) -> Vec<String> {
    ...
    if profile.repository == "tailrocks/velnor" {
        // The velnor estate's rolling preview lane. Hosted-only by contract: the
        // debs are attested by the shared package signer and consumers verify
        // them with --deny-self-hosted-runners.
        files.push("preview.yml".to_owned());
...
/// Rule file emitted inside every estate's workflow surface: the same
/// generation rule the top-level `.github/AGENTS.md` states, scoped to this
/// directory.
const ESTATE_WORKFLOWS_AGENTS_FILE: OwnedStaticFile = OwnedStaticFile {
    path: ".github/workflows/AGENTS.md",
    content: "# Generated by velnor-workflow. Regenerate; do not hand-edit.\n\n\
              # Rules\n\n\
...
const VELNOR_OWNED_STATIC_FILES: &[OwnedStaticFile] = &[
    ESTATE_WORKFLOWS_AGENTS_FILE,
    OwnedStaticFile {
        ...
        .any(|unit| unit.id == "rust-production-topology")
    {
        config.units.push(Unit {
            ...
            pr_commands: vec![
                ...
                "mise run test-release-boundary-script".to_owned(),
            ],
            full_commands: vec![
                "cargo check --workspace --all-targets --locked".to_owned(),
                "mise run test-release-feature-boundary".to_owned(),
...
fn catalog_config_with_default_branch(
    ...
    let mut config = ProjectConfig {
        ...
        velnor_labels: default_velnor_runner_labels(),
...
/// block.
fn apt_package_update_owner_blocks(template: &str) -> Vec<&str> {
...
fn package_update_channel_grant(config: &ProjectConfig, block: &str) -> Vec<String> {
...
fn is_known_legacy_actionlint_config(relative: &Path, current: &[u8]) -> bool {
    relative == Path::new(".github/actionlint.yaml")
        && current == LEGACY_ACTIONLINT_CONFIG.as_bytes()
}
...
mod tests {
    ...
    /// Scan a repository and finalize its unit contracts through the declared
    /// surface: a repository whose config owns the watch graph and the
    /// regeneration gate carries those contracts only after the registry ran.
    fn scanned_with_surface(root: &Path, runners: RunnerMode) -> ProjectConfig {
        let scanned = must(scan_target(root, runners, "main"), "scan repository");
        let surface = must(
            primitives::generate(
                root,
                &scanned.shape,
            ...
            "render the declared surface",
        );
        let mut config = scanned.config;
        config.units = surface.units;
        config
    }
    ...
    fn test_inputs() -> GenerationInputs {
        GenerationInputs::parts(0, 0)
    }
    ...
    fn plan_generated_write(
        ...
        super::plan_generated_write(root, files, &test_inputs())
    }
    ...
    fn apply_generated_write_plan(
        ...
        super::apply_generated_write_plan(root, files, &test_inputs(), dry_run, check, force, plan)
    }
    ...
    fn hosted_runtime_setup_uses_the_versioned_setup_action() {
        ...
        assert!(policy.contains(&format!("--rev {VELNOR_POLICY_WORKFLOW_REV}")));
    ...
    fn velnor_workflow_regeneration_gate_compiles_checked_out_generator() {
        let config = scanned_with_surface(&repository_root(), RunnerMode::Github);
...
"##;
    ...
    fn velnor_rust_lanes_use_mbx_and_release_native_builds() {
        ...
        // The Cargo-source cache rides both lanes; the object transport never
        // turns into an actions cache write.
        assert!(velnor_lane.contains("actions/cache/restore@"));
        assert!(velnor_lane.contains("~/.cargo/registry"));
        assert!(!velnor_lane.contains("actions/cache/save@"));
        ...
        let release_workflow = crate::primitives::release::render_release(&config, release);
        assert_eq!(
        ...
        let parent = WorkflowIr::from_config(&config).render_nested(
            ...
            for runners in [RunnerMode::Github, RunnerMode::Velnor, RunnerMode::Both] {
                ...
                for (path, content) in files {
                    ...
                    if matches!(
                        ...
                        && config.repository == "tailrocks/velnor")
                    {
                        // The velnor rolling preview lane is hosted-only by
                        // contract: consumers admit its debs with
                        // --deny-self-hosted-runners, so no lane may pin them to
    ...
    fn velnor_only_lane_uses_local_mbx_backend() {
        ...
        for kind in [
            ...
            // The declared Cargo-source cache is restored on the local lane
            // too; only the hosted lane ever saves.
            assert!(workflow.contains("actions/cache/restore@"));
    ...
    fn velnor_generator_owns_every_current_workflow() {
        ...
        let config = scanned_with_surface(&root, RunnerMode::Both);
        ...
        .collect::<BTreeSet<_>>();
        let files = must(generated_files(&config), "generate");
        // The workflow surface carries one non-YAML rule file (AGENTS.md); the
        // set equality here is about the emitted workflow files.
        let generated = files
            .keys()
            .filter_map(|path| {
        ...
        assert!(!files.contains_key(&PathBuf::from(
    ...
    fn owned_static_files_flow_through_ownership_and_stale_deletion() {
        ...
        let ownership = schema_two_state(ownership);
        assert_eq!(
            ownership.outputs.get(&action).copied(),
