...
const VELNOR_POLICY_WORKFLOW_REV: &str = "3975207f4e840b728c8bd7b804033855f8ed203e";
...
const VELNOR_WORKFLOW_SOURCE_REV: &str = "3975207f4e840b728c8bd7b804033855f8ed203e";
...
mod tests {
    ...
    #[test]
    fn velnor_default_keeps_planning_on_the_packaged_runtime_lane() {
        let config = must(
            scan_repository_with_default_branch(&fixture_root(), RunnerMode::Velnor, "main"),
            "scan fixture for Velnor planning",
        );
        let workflow = WorkflowIr::from_config(&config).render(WorkflowKind::Main);
        let plan = workflow
            .split_once("  plan:")
            .and_then(|(_, rest)| rest.split_once("  policy:").map(|(plan, _)| plan))
            .unwrap_or_default();

        assert!(
            !plan.is_empty(),
            "generated workflow has plan and policy jobs"
        );
        assert!(plan.contains("name: Planning"));
        assert!(plan.contains("runs-on: [self-hosted, example-runner-label]"));
        assert!(plan.contains("velnor-workflow plan --config .github/ci/project.toml"));
        assert!(!plan.contains("setup-velnor-workflow"));
