...
/// or reuse a revision.
pub const GENERATOR_REVISION: &str = "40";
...
"#,
    ...
    fn preview_static_surface_saves_guest_seed_on_trusted_exact_miss() {
        ...
        for unit in [
            ...
            assert!(
                ci_units.contains(&format!("  github-{unit}:\n    name: {unit}")),
                "native verification unit lost its GitHub lane: {unit}: {ci_units}"
            );
            assert!(
                ci_units.contains(&format!("  velnor-{unit}:\n    name: {unit}")),
    ...
    fn velnor_workspace_check_barrier_is_root_scoped_and_hosted_independent() {
        ...
        assert_eq!(
            ...
            Some("[velnor-rust-workspace]")
        );
        assert_eq!(
            job_needs("velnor-rust-child").as_deref(),
            Some("[velnor-rust-workspace, velnor-rust-model]")
        );
