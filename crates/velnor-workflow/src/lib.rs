...
"#,
    ...
    fn precondition_reaches_every_provider_override_on_unphased_units() {
        ...
        must(
            unit.prepend_precondition_commands(std::slice::from_ref(&gate)),
            "gate reaches every lane",
        );
        must(
            unit.prepend_precondition_commands(std::slice::from_ref(&gate)),
            "gate insertion is idempotent",
        );
        ...
        for (lane, commands) in [
            ...
            (
                "GitHub PR",
                must_some(unit.github_pr_commands.as_ref(), "GitHub PR lane"),
            ),
            (
                "GitHub full",
                must_some(unit.github_full_commands.as_ref(), "GitHub full lane"),
            ),
            (
                "Velnor PR",
                must_some(unit.velnor_pr_commands.as_ref(), "Velnor PR lane"),
            ),
            (
                "Velnor full",
                must_some(unit.velnor_full_commands.as_ref(), "Velnor full lane"),
            ),
        ] {
    ...
    fn phased_precondition_rejects_gate_present_in_only_one_scope() {
        ...
        let error = must_fail(
            unit.prepend_precondition_commands(&["cargo run -- --check".to_owned()]),
            "partial precondition state must fail closed",
        );
        assert!(
