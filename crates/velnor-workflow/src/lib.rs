...
mod tests {
    ...
    fn velnor_provisioner_executes_product_repo_resolution() {
        ...
        // The product asset is itself a script reporting the closure, so the
        // installed slot passes the self-report gate like a real binary.
        ...
        must(
            fs::write(log.join("asset-manifest.json"), format!(
                "{{\"closure\":\"{closure}\",\"profile\":\"release\",\"features\":\"\",\"products\":{{\"Linux-X64\":{{\"binary\":\"{asset_digest}\",\"asset\":\"{asset_name}\"}}}}}}"
            )),
            "write stub manifest",
        );
        must(
