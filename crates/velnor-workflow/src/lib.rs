...
const VELNOR_POLICY_WORKFLOW_REV: &str = "8c84b39d1f39e3fd7b3b450edc6e5dafa50383f8";
...
const VELNOR_WORKFLOW_SOURCE_REV: &str = "8c84b39d1f39e3fd7b3b450edc6e5dafa50383f8";
...
/// Hosted runtime install. `rev:` is always `install_rev`. `uses:` stays on
/// [`VELNOR_WORKFLOW_SOURCE_REV`] unless `install_rev` is `${{ github.sha }}`,
/// which must also resolve the action yaml at HEAD so dispatch bootstrap is
/// the current gate. Planning calls [`workflow_runtime_setup`] and keeps both
/// pins on `SOURCE_REV`.
fn workflow_runtime_setup_with_install_rev(lane: RunnerMode, install_rev: &str) -> String {
    if lane != RunnerMode::Github {
        return String::new();
    }
    let action_rev = if install_rev == github_expression("github.sha") {
        install_rev
    } else {
        VELNOR_WORKFLOW_SOURCE_REV
    };
    format!(
        "      - name: Set up Velnor workflow runtime\n        if: ${{{{ runner.environment == 'github-hosted' }}}}\n        uses: {VELNOR_WORKFLOW_SETUP_ACTION}@{action_rev}\n        with:\n          rev: {install_rev}\n      - name: Set trusted workflow policy revision\n        run: echo \"{VELNOR_POLICY_REVISION_ENV}={VELNOR_POLICY_WORKFLOW_REV}\" >> \"$GITHUB_ENV\"\n"
    )
}
...
mod tests {
    ...
    #[test]
    fn github_sha_install_rev_resolves_setup_action_at_head() {
        ...
        assert!(
            maintenance.contains(&format!("uses: {VELNOR_WORKFLOW_SETUP_ACTION}@{head_rev}")),
            "maintenance must resolve setup at HEAD: {maintenance}"
        );
        assert!(
        ...
        assert!(
            !maintenance.contains(&format!(
                "uses: {VELNOR_WORKFLOW_SETUP_ACTION}@{VELNOR_WORKFLOW_SOURCE_REV}"
            )),
            "maintenance must not pin setup to SOURCE_REV: {maintenance}"
        );
...
"#,
    ...
    fn generated_maintenance_stays_github_hosted_when_runners_are_velnor() {
        ...
        assert_maintenance_setup_resolves_at_head(workflow);
    ...
    fn assert_maintenance_setup_resolves_at_head(workflow: &str) {
        let head_rev = github_expression("github.sha");
        assert!(
            workflow.contains(&format!("uses: {VELNOR_WORKFLOW_SETUP_ACTION}@{head_rev}")),
            "maintenance must resolve setup at HEAD: {workflow}"
        );
        assert!(
            workflow.contains(&format!("rev: {head_rev}")),
            "maintenance must install HEAD runtime: {workflow}"
        );
        assert!(
            !workflow.contains(&format!(
                "uses: {VELNOR_WORKFLOW_SETUP_ACTION}@{VELNOR_WORKFLOW_SOURCE_REV}"
