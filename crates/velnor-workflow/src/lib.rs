...
/// or reuse a revision.
pub const GENERATOR_REVISION: &str = "23";
...
struct RawCli {
    ...
    #[arg(long, value_enum, default_value_t = RunnerMode::Velnor, value_name = "MODE")]
    runners: RunnerMode,
...
fn scan_target(
    ...
    let scan_runners = generation
        ...
        .transpose()?
        .unwrap_or(runners);
    let shape = scan::scan_shape(root, scan_runners, default_branch, exclude)?;
...
fn load_workflow_templates(
    ...
    for entry in entries {
        ...
        // The fleet caller keeps the render-consumer header the pinned fleet
        // release hash-pins; accept it alongside the generator header so a
        // tree stamped either way loads, while every other template must
        // carry the generator header.
        let mut body = content.strip_prefix(GENERATED_HEADER);
        if body.is_none() && name == FLEET_CALLER_WORKFLOW {
            body = content.strip_prefix(FLEET_CALLER_HEADER);
        }
...
mod tests {
    ...
    #[test]
    fn cli_defaults_to_velnor_and_accepts_documented_target_forms() {
    ...
    fn velnor_runtime_does_not_publish_an_orphan_artifact() {
        ...
        assert!(!workflow.contains("name: Set up Velnor workflow runtime"));
    ...
    #[test]
    fn template_loader_accepts_fleet_caller_header_for_ci_only() {
        let root = temporary_repository("fleet-caller-header");
        let templates = root.join(WORKFLOW_TEMPLATE_DIR);
        must(fs::create_dir_all(&templates), "create template directory");
        must(
            fs::write(
                templates.join("ci.yml"),
                format!("{FLEET_CALLER_HEADER}name: CI\n"),
            ),
            "write fleet caller template",
        );
        let config = must(
            load_workflow_templates(&root, header_fixture_config(), None),
            "load fleet caller template",
        );
        assert_eq!(
        ...
        must(
            fs::write(
                templates.join("docs.yml"),
                format!("{FLEET_CALLER_HEADER}name: Docs\n"),
            ),
            "write foreign-header template",
        );
        assert!(
            load_workflow_templates(&root, header_fixture_config(), None).is_err(),
            "a non-caller template must carry the generator header"
        );
...
"#,
    ...
    fn docker_mutable_mount_seed_renders_restore_seed_and_trusted_export() {
        ...
        let contract = UnitContract {
            lanes: WorkflowIr::default_lane_jobs(true),
    ...
    fn assert_maintenance_setup_uses_literal_source_rev(workflow: &str) {
        ...
        assert!(
            ...
            "maintenance cache-plan still installs HEAD: {workflow}"
        );
    ...
    fn both_mode_plan_consumes_the_admitted_lanes() {
        ...
        for aggregate in [
            ...
            let consumer = "VELNOR_LANES: ${{ needs.lane-admission.outputs.lanes }}";
            assert!(aggregate.contains(consumer), "{aggregate}");
            // The consumer sits in the plan step, after the admission job the
            // plan job needs.
            let admission = must_some(aggregate.find("  lane-admission:"), "lane-admission job");
            let plan = must_some(aggregate.find("  plan:"), "plan job");
            let lanes_env = must_some(aggregate.find(consumer), "plan lanes env");
            assert!(admission < plan && plan < lanes_env, "{aggregate}");
        }
    }
    ...
    fn adoption_snapshots_existing_workflows_and_reloads_them_from_templates() {
        ...
        // The fleet caller is stamped with the render-consumer header its
        // pinned fleet release hash-pins, not the generator header.
        assert!(rendered.starts_with(FLEET_CALLER_HEADER));
