...
/// or reuse a revision.
pub const GENERATOR_REVISION: &str = "1";
...
// Rolling preview lane for the velnor estate: emitted as
// `.github/workflows/preview.yml` for tailrocks/velnor only. Other estates keep
// the generated `render_preview` surface; the velnor estate's preview builds
// release-grade debs, so it is a code-owned static template instead.
...
fn scan_target(
    ...
    // The repo-owned config is validated against the scanned shape before it
    // can influence anything: a declared unit that the scan did not find is a
    // hard error, never a silent no-op.
    let generation = config::discover(root)?;
    if let Some(generation) = &generation {
        generation.validate(&shape)?;
    }
    let inputs = GenerationInputs::current(generation.as_ref(), &shape)?;
    let config = apply_local_velnor_profile(root, ProjectConfig::from(shape))?;
    let config = load_workflow_templates(root, config)?;
    Ok(ScannedTarget {
        config: apply_local_estate_runner_profile(root, config),
        generation,
        inputs,
    })
}
    ...
    for workflow_file in &config.workflow_files {
        let content = config
            ...
            .or_else(|| match workflow_file.as_str() {
                ...
                // The velnor estate owns a static preview template: its rolling
                // preview publishes debs, not the generic tarball surface.
                "preview.yml" if config.repository == "tailrocks/velnor" => {
                    Some(generated_velnor_preview())
                }
...
fn generated_ci_pr(workflow: &WorkflowIr) -> String {
    workflow.render_nested(WorkflowKind::PullRequest)
}
...
fn generated_release_package_signer() -> String {
    render_static_template(VELNOR_RELEASE_PACKAGE_SIGNER_TEMPLATE)
}

fn generated_velnor_workflow_policy(config: &ProjectConfig) -> String {
    render_policy_provider(config)
}

fn generated_ci_main(workflow: &WorkflowIr) -> String {
    workflow.render_nested(WorkflowKind::Main)
}

fn generated_nightly(workflow: &WorkflowIr) -> String {
    workflow.render_nested(WorkflowKind::Nightly)
}

fn generated_maintenance(config: &ProjectConfig) -> String {
    format!("{GENERATED_HEADER}{}", render_maintenance(config))
}

fn generated_velnor_preview() -> String {
    render_static_template(VELNOR_PREVIEW_WORKFLOW_TEMPLATE)
}

fn generated_preview(config: &ProjectConfig) -> String {
    render_preview(config)
}
...
"##;
    ...
    fn apt_package_updater_splits_lanes_into_static_runner_jobs() {
        ...
        let normalized_files = generated_files(&apt);
    ...
    fn generate_repository(root: &Path, force: bool) -> WriteOutcome {
        ...
        let files = generated_files(&scanned.config);
        must(
