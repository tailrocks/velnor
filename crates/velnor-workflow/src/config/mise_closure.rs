...
struct TaskSpec {
    ...
    run_windows_tasks: Option<Vec<TaskRef>>,
    aliases: Vec<String>,
}
...
struct Resolver<'a> {
    manifest: Manifest,
    closure: TaskClosure,
    env_overrides: Vec<TaskEnvOverride>,
...
#[derive(Clone, Debug)]
struct TaskEnvOverride {
    task: String,
    source: Option<String>,
    env: BTreeMap<String, String>,
}

impl TaskEnvOverride {
    fn context(&self) -> String {
        self.source.as_ref().map_or_else(
            || format!("Mise task {} env", self.task),
            |source| format!("Mise task {} {source} env", self.task),
        )
    }
}
...
impl Resolver<'_> {
    ...
    fn visit_task_body(
        ...
        if !spec.env.is_empty() {
            self.env_overrides.push(TaskEnvOverride {
                task: task.to_owned(),
                source: None,
                env: spec.env.clone(),
            });
        ...
        let is_windows = self.platform.starts_with("windows");
        let (run_relation, run_tasks) = if is_windows {
            match spec.run_windows_tasks {
                Some(run_tasks) => ("run_windows", run_tasks),
                None => ("run", spec.run_tasks),
            }
        } else {
            ("run", spec.run_tasks)
        };
        for run in run_tasks {
            self.visit_ref(&run, run_relation, TaskPhase::Run)?;
        }
    ...
    fn visit_ref(
        ...
        for name in names {
            if !reference.env.is_empty() {
                ...
                self.env_overrides.push(TaskEnvOverride {
                    task: name.clone(),
                    source: Some(format!("{relation} reference from task {caller}")),
                    env: reference.env.clone(),
                });
            }
...
fn parse_task_spec(table: &Table, name: &str) -> Result<TaskSpec, GeneratorError> {
    ...
    Ok(TaskSpec {
        ...
        run_windows_tasks: table
            .get("run_windows")
            .map(|value| parse_run_tasks(Some(value), &context))
            .transpose()?,
...
mod tests {
    ...
    fn pinned_rust() -> crate::RustToolchain {
        crate::RustToolchain {
    ...
    #[test]
    fn rejects_task_env_that_overrides_generated_auto_install_policy() {
        for key in [
            "MISE_AUTO_INSTALL",
            "MISE_EXEC_AUTO_INSTALL",
            "MISE_NOT_FOUND_AUTO_INSTALL",
            ...
            let mise =
                format!("[tasks.check]\nenv = {{ {key} = \"true\" }}\nrun = \"echo check\"\n");
            let root = fixture(&mise, "");
            let error = must_fail(
                resolve_profile_for_platform(&root, &["check".to_owned()], &[], "linux-x64"),
                "task env must not override generated Mise auto-install policy",
            );
            assert!(error.to_string().contains("Mise task check env"), "{error}");
    ...
    #[test]
    fn rejects_task_env_that_shadows_pinned_rustup_selection() {
        let root = fixture(
        ...
        let error = must_fail(
            ...
            "task env must not override the generated Rustup pin",
        );
        assert!(
            error
                .to_string()
                .contains("Mise task check env `RUSTUP_TOOLCHAIN`"),
            "{error}"
        );
        remove_fixture(&root);
    }
    ...
    #[test]
    fn rejects_structured_run_env_after_task_visit_deduplication() {
        let root = fixture(
            "[tasks.check]\ntools = { rust = \"1.90.0\" }\nrun = [{ task = \"child\" }, { task = \"child\", env = { RUSTUP_TOOLCHAIN = \"nightly\" } }]\n\n[tasks.child]\nrun = \"echo child\"\n",
            "[[tools.rust]]\nversion = \"1.90.0\"\nbackend = \"core:rust\"\n",
        );
        let error = must_fail(
            resolve_profile_for_platform_with_rust(
            ...
            "structured run env must not override the generated Rustup pin",
        );
        assert!(
            error
                .to_string()
                .contains("Mise task child run reference from task check env `RUSTUP_TOOLCHAIN`"),
            "{error}"
        );
        remove_fixture(&root);
    }
    ...
    #[test]
    fn permits_rustup_env_on_stages_without_typed_rust_activation() {
        let root = fixture(
            "[tasks.check]\nenv = { RUSTUP_TOOLCHAIN = \"nightly\" }\nrun = [{ task = \"child\", env = { PATH = \"/custom/bin\" } }]\n\n[tasks.child]\nrun = \"echo child\"\n",
            "",
        );
        let closure = must(
            resolve_profile_for_platform(&root, &["check".to_owned()], &[], "linux-x64"),
            "non-Rust task env remains stage-scoped",
        );
    ...
    #[test]
    fn permits_unrelated_task_and_reference_env_with_typed_rust() {
        let root = fixture(
            "[tasks.check]\ntools = { rust = \"1.90.0\" }\nenv = { CI = \"true\" }\nrun = [{ task = \"child\", env = { RUST_LOG = \"debug\" } }]\n\n[tasks.child]\nrun = \"echo child\"\n",
            "[[tools.rust]]\nversion = \"1.90.0\"\nbackend = \"core:rust\"\n",
        );
        let closure = must(
            resolve_profile_for_platform_with_rust(
            ...
            "unrelated task env remains supported",
        );
        assert!(closure.uses_rust_toolchain);
        assert!(closure.invocations.iter().any(|invocation| {
            invocation.task == "child"
                && invocation.env.get("RUST_LOG").map(String::as_str) == Some("debug")
        }));
        remove_fixture(&root);
    }

    #[test]
    fn resolves_run_windows_references_only_for_windows_profiles() {
        let root = fixture(
            "[tasks.check]\nrun = [{ task = \"unix-check\" }]\nrun_windows = [{ task = \"windows-check\" }]\n\n[tasks.unix-check]\nrun = \"echo unix\"\n\n[tasks.windows-check]\ntools = { rust = \"1.90.0\" }\n\n[tasks.common]\nrun = [{ task = \"shared\" }]\n\n[tasks.shared]\nrun = \"echo shared\"\n",
            "[[tools.rust]]\nversion = \"1.90.0\"\nbackend = \"core:rust\"\n",
        );
        let boundary = pinned_rust();
        let unix = must(
            resolve_profile_for_platform_with_rust(
                &root,
                &["check".to_owned(), "common".to_owned()],
                &[],
                "linux-x64",
                Some(&boundary),
            ),
            "Unix profile selects run",
        );
        assert!(unix.tasks.iter().any(|task| task == "unix-check"));
        assert!(!unix.tasks.iter().any(|task| task == "windows-check"));
        assert!(unix.tasks.iter().any(|task| task == "shared"));
        assert!(!unix.uses_rust_toolchain);
        ...
        let windows = must(
            ...
            "Windows profile selects run_windows",
        );
        assert!(windows.tasks.iter().any(|task| task == "windows-check"));
        assert!(!windows.tasks.iter().any(|task| task == "unix-check"));
        assert!(windows.tasks.iter().any(|task| task == "shared"));
        assert!(windows.uses_rust_toolchain);
        assert!(windows.tools.iter().all(|tool| tool.key != "rust"));
        remove_fixture(&root);
    }
