...
impl TaskClosure {
    ...
    pub(crate) fn installation_plan(
        ...
        Ok(ToolInstallPlan { requirements })
    }
...
impl ToolInstallPlan {
    ...
    pub(crate) fn mise_args(&self, profile_id: &str) -> Result<Vec<String>, GeneratorError> {
        ...
        for requirement in &self.requirements {
            if matches!(&requirement.scope, ToolScope::Root) && seen.insert(requirement.key.clone())
            {
...
impl Manifest {
    fn load(root: &Path) -> Result<Self, GeneratorError> {
        ...
        if let Some(rust) = manifest.tools.get("rust")
            && !rust.options.is_empty()
        {
            return Err(rustup_boundary_error(
                "root mise.toml [tools.rust] options",
                rust.options.keys().map(String::as_str),
            ));
        }
...
fn add_idiomatic_rust_tool(
    ...
    if !options.is_empty() {
        return Err(rustup_boundary_error(
            "rust-toolchain idiomatic discovery",
