...
pub(crate) struct RenovateSection {
    ...
    cache: Option<bool>,
    /// Writer execution lanes, independent of CI unit lanes: `velnor`
    /// (default), `github`, or `both` (Velnor default with a manual dispatch
    /// override to GitHub). A scheduled writer must not force CI units onto
    /// runners the repository deliberately avoids.
    lanes: Option<String>,
...
/// The row names the job, the repository tasks it runs, the sibling jobs it
/// waits on, the lane it runs on, and the release modes that run it. Product
/// build, sign, and publish logic stays in the named tasks; the renderer
/// owns triggers, ordering, mode gates, and the lane.
#[derive(Clone, Debug, Default, Eq, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
...
pub(crate) struct ReleaseSection {
    ...
    /// Named-task jobs, one `[[release.job]]` row per job the `tasks`
    /// publisher renders. Any other kind rejects the rows: publishers with
    /// their own job graph take no sibling jobs.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
...
impl RenovateSection {
    ...
    pub(crate) fn lanes(&self) -> Option<&str> {
        self.lanes.as_deref()
    }
...
fn validate_workflow(workflow: &WorkflowSection) -> Result<(), GeneratorError> {
    ...
    if let Some(branch) = workflow.default_branch.as_deref()
        && !crate::runtime::valid_branch(branch)
    {
        return Err(GeneratorError::usage(format!(
...
impl RepoGenerationConfig {
    fn validate_renovate(&self) -> Result<(), GeneratorError> {
        ...
        let lanes = renovate.lanes.as_deref().unwrap_or("velnor");
        if !matches!(lanes, "velnor" | "github" | "both") {
            return Err(GeneratorError::usage(format!(
                "[renovate] lanes must be one of: velnor, github, both; found `{lanes}`"
            )));
        }
        if lanes == "both" && self.workflow.velnor_runner_group.is_some() {
            return Err(GeneratorError::usage(
                "[renovate] lanes = \"both\" cannot select a runner group dynamically; declare lanes = \"velnor\" or drop [workflow] velnor_runner_group",
            ));
        }
        // Writer execution is declared here, not inherited from CI unit lanes:
        // a `runners = "github"` repository may still run its scheduled writer
        // on Velnor runners without doubling its CI surface.
        if lanes != "github" {
            if self.workflow.velnor_trusted_label.is_none() {
                return Err(GeneratorError::usage(
...
fn validate_check_profile_row(
    ...
    if row.runner.as_deref().unwrap_or("github") == "velnor" {
        if config.workflow.runners.as_deref() == Some("github") {
            return Err(GeneratorError::usage(format!(
                "[[check_profile]] {id} runs on velnor, but [workflow] runners = \"github\" has no Velnor lane"
            )));
        }
        if config
            .workflow
        ...
        {
            return Err(GeneratorError::usage(format!(
                "[[check_profile]] {id} runs on velnor, but [workflow] velnor_labels names no runner labels"
            )));
        }
    }
...
fn validate_release_naming(release: &ReleaseSection) -> Result<(), GeneratorError> {
    if let Some(package) = release.package.as_deref()
        && !crate::runtime::valid_package(package)
    {
        return Err(GeneratorError::usage(format!(
    ...
    if let Some(binary) = release.binary.as_deref()
        && !crate::runtime::valid_binary(binary)
    {
        return Err(GeneratorError::usage(format!(
    ...
    for target in &release.targets {
        if !crate::runtime::valid_target(target) {
            return Err(GeneratorError::usage(format!(
    ...
    if let Some(pattern) = release.tag_pattern.as_deref()
        && !crate::runtime::valid_tag_pattern(pattern)
    {
        return Err(GeneratorError::usage(format!(
...
fn validate_release_binding_kind(release: &ReleaseSection) -> Result<(), GeneratorError> {
    ...
    if kind == "tasks" {
        // The tasks publisher renders dispatch modes (the drill input its
        // jobs gate on) but no producer, archive, or credential bindings:
        // it orchestrates repository tasks, not artifacts.
        ...
        if bound {
            return Err(GeneratorError::usage(format!(
                "[release] producer bindings, archive contracts, and credential pairings render only for kind `rust-binary` or `native`, not `{kind}`"
            )));
        }
        return Ok(());
    }
...
fn validate_release_bindings(release: &ReleaseSection) -> Result<(), GeneratorError> {
    ...
    for credential in &release.credential {
        ...
        if !crate::runtime::valid_package(name) {
            return Err(GeneratorError::usage(format!(
...
/// The `[[release.job]]` rows a `tasks` release renders: job identity,
/// named tasks, sibling dependencies, lane, mode gates, and timeout. Any
/// other kind rejects the rows next to its own publisher contract.
...
/// One job row against the repository around it: display name, task
/// references, dependencies, lane, mode gates, and timeout.
fn validate_release_job_row(
    ...
    if row.runner.as_deref().unwrap_or("github") == "velnor" {
        if workflow.runners.as_deref() == Some("github") {
            return Err(GeneratorError::usage(format!(
                "[[release.job]] {id} runs on velnor, but [workflow] runners = \"github\" has no Velnor lane"
            )));
        }
        if workflow.velnor_labels.as_ref().is_none_or(Vec::is_empty) {
            return Err(GeneratorError::usage(format!(
                "[[release.job]] {id} runs on velnor, but [workflow] velnor_labels names no runner labels"
            )));
        }
    }
...
mod tests {
    ...
    #[test]
    fn tasks_release_accepts_named_task_jobs() {
        let config = config_for(
            "schema = 1\n\n[generator]\nrepository = \"example/fixture\"\n\n[release]\nenabled = true\nkind = \"tasks\"\nmodes = [\"validate\"]\n\n[[release.job]]\nid = \"build\"\ntasks = [\"build-release\", \"verify-release\"]\n\n[[release.job]]\nid = \"sign\"\nname = \"Sign release\"\ntasks = [\"sign-release\"]\nneeds = [\"build\"]\nrunner = \"macos\"\nmodes = [\"publish\"]\ntimeout_minutes = 45\n",
        );
        must(
            config.validate(&[], &[], &BTreeSet::new()),
            "validate tasks release with two jobs",
        );
        let jobs = config.release().jobs();
        assert_eq!(jobs.len(), 2);
        assert_eq!(jobs[0].id(), Some("build"));
        assert_eq!(
            jobs[0].tasks(),
            Some(&["build-release".to_owned(), "verify-release".to_owned()][..])
        );
        assert_eq!(jobs[1].needs(), Some(&["build".to_owned()][..]));
    ...
    #[test]
    fn tasks_release_requires_jobs_and_rejects_artifact_bindings() {
        let missing = must_fail(
            config_for(
                "schema = 1\n\n[generator]\nrepository = \"example/fixture\"\n\n[release]\nenabled = true\nkind = \"tasks\"\n",
            )
            .validate(&[], &[], &BTreeSet::new()),
            "enabled tasks release without jobs must fail",
        );
        assert!(missing.to_string().contains("tasks"), "{missing}");
        for (name, binding) in [
            ("members", "archive_members = [\"app.tar.gz\"]\n"),
            (
                "credential",
            ...
            let error = must_fail(
                ...
                .validate(&[], &[], &BTreeSet::new()),
                "tasks release with artifact bindings must fail",
            );
            assert!(
    ...
    #[test]
    fn release_jobs_reject_other_kinds_and_bad_rows() {
        let other_kind = must_fail(
            config_for(
                "schema = 1\n\n[generator]\nrepository = \"example/fixture\"\n\n[release]\nenabled = true\nkind = \"pages\"\nartifact_path = \"dist\"\n\n[[release.job]]\nid = \"build\"\ntasks = [\"build-release\"]\n",
            )
            .validate(&[], &[], &BTreeSet::new()),
            "jobs on a pages release must fail",
        );
        assert!(
            other_kind.to_string().contains("only for kind `tasks`"),
            "{other_kind}"
        );
        for (name, row, fragment) in [
            (
                "missing-id",
                "tasks = [\"build-release\"]\n",
                "missing `id`",
            ...
            (
                "velnor-without-labels",
                "id = \"build\"\ntasks = [\"build-release\"]\nrunner = \"velnor\"\n",
                "velnor_labels names no runner labels",
            ),
        ] {
            let error = must_fail(
                config_for(&format!(
    ...
    #[test]
    fn release_jobs_reject_duplicate_ids() {
        let error = must_fail(
            config_for(
    ...
    #[test]
    fn renovate_lanes_rejects_unknown_values() {
        let error = must_fail(
            config_for(
                "schema = 1\n\n[generator]\nrepository = \"example/fixture\"\n\n\
                 [[declare]]\nprimitive = \"renovate\"\nfile = \"renovate.yml\"\n\n\
            ...
            .validate(&[], &[], &BTreeSet::new()),
            "unknown renovate lanes must fail",
        );
        assert!(
            error
                .to_string()
                .contains("[renovate] lanes must be one of"),
            "{error}"
        );
    }

    #[test]
    fn renovate_lanes_both_rejects_runner_group() {
        let error = must_fail(
            config_for(
                "schema = 1\n\n[generator]\nrepository = \"example/fixture\"\n\n\
            ...
            .validate(&[], &[], &BTreeSet::new()),
            "lanes=both with a runner group must fail",
        );
        assert!(
            error
                .to_string()
                .contains("cannot select a runner group dynamically"),
            "{error}"
        );
    }

    #[test]
    fn renovate_writer_lanes_ignore_ci_runners() {
        must(
            config_for(
                "schema = 1\n\n[generator]\nrepository = \"example/fixture\"\n\n\
                 [workflow]\nrunners = \"github\"\n\
            ...
            .validate(&[], &[], &BTreeSet::new()),
            "github-only CI must accept a velnor renovate writer",
        );
    }

    #[test]
    fn renovate_lanes_github_needs_no_velnor_declarations() {
        must(
            config_for(
                "schema = 1\n\n[generator]\nrepository = \"example/fixture\"\n\n\
                 [workflow]\nrunners = \"github\"\n\n\
            ...
            .validate(&[], &[], &BTreeSet::new()),
            "github-lane writer must not require velnor declarations",
        );
    }
    ...
    #[test]
    fn check_profile_velnor_runner_needs_a_lane_and_labels() {
        ...
        assert!(error.to_string().contains("has no Velnor lane"), "{error}");

        let unlabeled = config_for(&check_profile_config(
            "[[check_profile]]\nid = \"fleet\"\nschedule = \"23 2 * * *\"\nrunner = \"velnor\"\ntasks = [\"check-fleet\"]\n",
        ));
        let error = must_fail(
            unlabeled.validate(&[], &[], &BTreeSet::new()),
