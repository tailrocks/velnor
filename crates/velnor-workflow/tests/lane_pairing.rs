...
fn automatic_both_gates_pair_except_fork_pr_admission() {
    ...
    // boundary into this schema-1 surface. `pull_request_on_velnor`
    // requires the approved runner contract, and the crate must never
    // spell the estate's labels itself (see generic_surface_literals), so
    // the values flow from the estate module at test time.
    let approved =
    ...
    let mut lines: Vec<String> = current
        .lines()
        .filter(|line| !line.starts_with("velnor_labels ="))
        .map(str::to_owned)
        .collect();
