...
fn assert_names_are_control_or_lane(jobs: &BTreeMap<String, Value>) {
    for (id, job) in jobs {
        let name = job_name(job);
        assert!(!name.is_empty(), "job `{id}` is missing a display name");
        let token = lane_token(&name);
        assert!(
            matches!(token, "GitHub" | "Velnor" | "Control"),
            "job `{id}` name `{name}` must start with GitHub /, Velnor /, or Control /"
        );
    }
}
...
fn both_emits_one_github_and_one_velnor_job_per_unit() {
    ...
    assert_names_are_control_or_lane(&jobs);
    ...
    assert_eq!(job_name(prep), "Control / Rust / prepare-cargo");
    assert!(
    ...
    for unit in &github {
        ...
        assert_eq!(github_name, format!("GitHub / Rust / {unit}"));
        assert_eq!(velnor_name, format!("Velnor / Rust / {unit}"));
        assert_eq!(lane_token(&github_name), "GitHub");
        assert_eq!(lane_token(&velnor_name), "Velnor");
        assert!(
...
fn automatic_both_gates_pair_except_fork_pr_admission() {
    ...
    assert_eq!(lane_token(&job_name(&pr["group-rust"])), "Control");
}
...
"#
    ...
    assert_eq!(
        job_name(&swift[&github_job]),
        format!("GitHub / Swift / {swift_id}")
    );
}
