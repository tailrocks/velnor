...
2. Linux and macOS on-demand Docker-backed Velnor execution works, including real native macOS velnorctl operation.
3. Repository-wide and PR-targeted operation have tested, truthful scheduling semantics and safe cleanup.
4. PRs #809 and #812 are merged with their intended changes preserved and no protection bypass.
5. Full main verification and the applicable release processes pass for the final integrated code; released artifacts install and function.
6. Sentry runs the final verified release persistently and successfully executes real repository jobs after deployment.
7. Independent recovery, simultaneous extra capacity, and return to Sentry-only operation are demonstrated.
8. No missing evidence, failed required check, unfinished migration, or unavailable validation is disguised as success.
