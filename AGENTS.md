...
- Use the latest available versions of all tools, dependencies, runtimes, and platforms; recheck official sources and refresh immutable pins—never silently downgrade. Keep action and image references immutable: latest means a verified current SHA/digest, never a floating reference.
