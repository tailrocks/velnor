...
`docs/mission.md`, `docs/vision.md`, and `docs/roadmap.md` are the normative
direction sources. `docs/comparison.md` is timestamped non-normative evidence.
`docs/prompt.md`, `plans/README.md`, and the rest of the repository defer to
them and the marked contract. There is exactly one active execution prompt.
...
- 2026-08-24: **Plural `lanes` selector restored as canonical** (operator
  ...
  `lanes` choice input (`velnor | github | both`, Velnor default), superseding
  the sole-`lane` entries recorded earlier this day. Authority: generator
  `tailrocks/velnor-actions` origin/main commits `87b3c31`, `3f03ffe`,
  `84a3d2c` (mirrored release `2026.8.31`) and fixture PRs #85–#88. Callable
  reusable workflows keep their singular `lane` input; callers derive it from
  `inputs.lanes`. Org-derived defaults are unchanged.
...
- 2026-08-09: The marked unified-CI contract supersedes every earlier
  ...
  merge-context entry regardless of date. Conflicting older entries below are
...
- 2026-07-22: **SUPERSEDED estate/default/selector record.** Historical
  program evidence remains in Git history; the marked 2026-08-09 contract is
  the only executable estate and lane direction. Its still-compatible
  latest-version, mise, nextest, cache-performance, and gated-merge rules
  remain active.
