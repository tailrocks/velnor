...
- Never cancel `tailrocks/velnor` `Release`, `tailrocks/velnor-apt` `Package update` or `Publish apt repo`, or release `workflow_dispatch` runs in either repository; immutable-tag/SHA staleness is not grounds; report suspected stale runs in `COORDINATION.md`.
