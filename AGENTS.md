...
2026-08-29 — cargo target snapshots are automatic and compatibility-classed (generation `workspace-v5-compat`): trusted pools restore/publish compatible generations with no opt-in, `VELNOR_CARGO_TARGET_PERSIST` is removed, identity excludes commit/job/workflow, `[acceleration] target_persistence` `auto|on|off` governs with recorded degradation when off.
