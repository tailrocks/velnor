[Skip to content](https://reui.io/docs/components/base/data-grid#main-content)
...
[Sign in](https://reui.io/login?redirect=%2Fdocs%2Fcomponents%2Fbase%2Fdata-grid) [Get ProGet All-Access](https://reui.io/pricing)
...
Copy Markdown [Previous](https://reui.io/docs/components/base/code-block) [Next](https://reui.io/docs/components/base/date-selector)
...
[Base UI](https://reui.io/docs/components/base/data-grid) [Radix UI](https://reui.io/docs/components/radix/data-grid)

[API Reference](https://tanstack.com/table/latest)
...
It covers both cases: a session with the [ReUI MCP](https://reui.io/docs/mcp) connected, and a plain
...
Cell range selection, clipboard copy / cut / paste with Excel and Google Sheets
round-trip, a fill handle, inline editors, and a bulk edit bar over row
selection, all in one grid alongside sorting, pagination, resizable and pinned
columns.
...
The end-to-end pattern for a grid backed by a large server-side dataset: the
table holds exactly one page, and pagination, sorting, and a debounced search
are sent to the server, which returns the page plus the total after filtering.
The demo simulates the server with an in-file async function over 487 records;
replace its body with a fetch to your own API and keep the return shape.
...
aggregates, infinite scroll, server side pagination, row pinning, and
spreadsheet editing) browse the
[Data Grid components](https://reui.io/components/data-grid).
...
| `onCellsChange` | `(details: DataGridCellsChangeDetails<TData>) => void` | - | Receives every spreadsheet write batch (paste, cut, clear, fill, edit). See `DataGridCellSelection`. |
| `onCellEditRequest` | `(request: DataGridCellEditRequest<TData>) => void` | - | Opens your editor from the keyboard: Enter, F2, or typing on the focused cell. See `DataGridCellSelection`. |
...
| `i18n` | `DataGridI18nOverrides` | - | Overrides for every built-in string; see Internationalization below. |
...
| `moreLimit` | `number` | `5` | Page buttons in the truncated middle, counting the first and last page. An ellipsis stands in for each hidden stretch, and the row holds a steady width as you page. Rounded down to an odd count, minimum 3. |
...
| `apiRef` | `RefObject<DataGridCellSelectionApi | null>` | - | Receives the controller's imperative API once wired; null while `cellSelection` is off. |
...
- Utility columns (checkboxes, expand toggles, action buttons) opt out with `enableCellSelection: false` on the column definition; ranges, keyboard stepping and select-all skip them.
- In-cell controls follow the editor's two-step gesture: the first click on an UNFOCUSED cell only focuses it (the control does not activate), the second click activates it, so moving the focus around can never fire actions. Checkboxes keep their one-click toggle (the row-select idiom), modifier presses are selection gestures, drags select as usual, and `cellEditMode: "click"` makes everything single-click. Keys inside an editable element never move the selection; Escape hands focus back to the grid unless the control consumed it with `preventDefault`. For custom widgets built from other elements, put `data-cell-interactive` on the widget's root to opt out entirely, or `data-cell-control` on a widget whose active surface is bigger than its buttons (a combobox opened by its whole chips strip) to make a press anywhere on it two-step.
- Editing grids should pass `autoResetPageIndex: false` to `useTable`; every commit replaces `data`, and the TanStack default would snap a paginated grid back to page 1 on each write.
- With `cellSelection` on, the grid sets `autoResetCellSelection: false` on your table, overriding the TanStack default: every write batch replaces `data`, and the default would wipe the selection after each commit.
- A page change clears the cell selection. Sorting and filtering keep it: ranges anchor to row ids, and rows that leave the view drop out of the selection bounds until they return.
- With `rowsPinnable`, keyboard navigation and every batch walk rendered rows, so pinned rows (a bottom-pinned create draft) select, edit and fill like any other row; only the selection bounds are stored in data order.
- Undo is intentionally left to the consumer: every change carries `previousValue`, so an undo stack is a list of batches replayed through the same state update with `value` and `previousValue` swapped.

* * *

### DataGridSelectionBar

A thin bulk-edit shell over row selection: selected count, a slot for your controls, and a clear action. Hidden while nothing is selected, and presented as a floating toolbar the Google Sheets way: auto width, centered, elevated, sticky to the viewport bottom with breathing room. What the controls do stays yours; apply bulk updates with your own state update over `table.getSelectedRowModel().rows`.
...
[Browse all 34 Shadcn Data Grid components](https://reui.io/components/data-grid) for copy-ready layouts, dashboards, and forms built with Tailwind CSS in the ReUI library.
...
[Preview all 37 Shadcn Data Grid Pro blocks](https://reui.io/blocks/data-grid) in the ReUI blocks gallery.

[Code Block](https://reui.io/docs/components/base/code-block) [Date Selector](https://reui.io/docs/components/base/date-selector)

On This Page

[Installation](https://reui.io/docs/components/base/data-grid#installation) [TanStack Table v9](https://reui.io/docs/components/base/data-grid#tanstack-table-v9) [Prompt for TanStack Table v9 migration](https://reui.io/docs/components/base/data-grid#prompt-for-tanstack-table-v9-migration) [React Compiler](https://reui.io/docs/components/base/data-grid#react-compiler) [Usage](https://reui.io/docs/components/base/data-grid#usage) [Examples](https://reui.io/docs/components/base/data-grid#examples) [Cell Border](https://reui.io/docs/components/base/data-grid#cell-border) [Dense Table](https://reui.io/docs/components/base/data-grid#dense-table) [Light Table](https://reui.io/docs/components/base/data-grid#light-table) [Striped Table](https://reui.io/docs/components/base/data-grid#striped-table) [Auto Width](https://reui.io/docs/components/base/data-grid#auto-width) [Row Selection](https://reui.io/docs/components/base/data-grid#row-selection) [Tree Rows](https://reui.io/docs/components/base/data-grid#tree-rows) [Spreadsheet Editing](https://reui.io/docs/components/base/data-grid#spreadsheet-editing) [Column Virtualization](https://reui.io/docs/components/base/data-grid#column-virtualization) [Localized Labels](https://reui.io/docs/components/base/data-grid#localized-labels) [Server Side Pagination](https://reui.io/docs/components/base/data-grid#server-side-pagination) [API Reference](https://reui.io/docs/components/base/data-grid#api-reference) [DataGrid](https://reui.io/docs/components/base/data-grid#datagrid) [DataGridTableClassNames](https://reui.io/docs/components/base/data-grid#datagridtableclassnames) [DataGridContainer](https://reui.io/docs/components/base/data-grid#datagridcontainer) [DataGridScrollArea](https://reui.io/docs/components/base/data-grid#datagridscrollarea) [DataGridTable](https://reui.io/docs/components/base/data-grid#datagridtable) [DataGridPagination](https://reui.io/docs/components/base/data-grid#datagridpagination) [DataGridTableDnd](https://reui.io/docs/components/base/data-grid#datagridtablednd) [DataGridTableDndRows](https://reui.io/docs/components/base/data-grid#datagridtabledndrows) [Internationalization](https://reui.io/docs/components/base/data-grid#internationalization) [DataGridTableVirtual](https://reui.io/docs/components/base/data-grid#datagridtablevirtual) [DataGridCellSelection](https://reui.io/docs/components/base/data-grid#datagridcellselection) [DataGridSelectionBar](https://reui.io/docs/components/base/data-grid#datagridselectionbar) [DataGridTableRowPin](https://reui.io/docs/components/base/data-grid#datagridtablerowpin) [DataGridTableRowExpand](https://reui.io/docs/components/base/data-grid#datagridtablerowexpand) [DataGridTableFoot](https://reui.io/docs/components/base/data-grid#datagridtablefoot) [DataGridTableFootRow](https://reui.io/docs/components/base/data-grid#datagridtablefootrow) [DataGridTableFootRowCell](https://reui.io/docs/components/base/data-grid#datagridtablefootrowcell) [DOM Attributes](https://reui.io/docs/components/base/data-grid#dom-attributes)
