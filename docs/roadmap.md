...
**Top-level unified-CI direction and sequencing live in the coordination graph
named by the marked contract.** `master-plan.md` is historical evidence. This
...
**Estate uniformity follows the marked contract's four exhaustive generated
classes.** Applicability is declared as class data and never implemented as a
repository-local omission or workflow fork.
