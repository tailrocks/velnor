...
`mise` is the task entrypoint. Install the pinned Rust toolchain selected by
`rust-toolchain.toml`, then install the tools declared in `mise.toml`: mise,
`actionlint`, `cargo-deny`, `cargo-nextest`, `cargo-zigbuild`, `cargo-deb`, and
Zig. The lockfile is mandatory, and CI/jobs use locked artifacts. Run these
current gates:
...
- `actionlint` checks workflow syntax; `deny` runs advisory checks. `check` is the
  aggregate gate and also runs the offline CI/fleet policy tasks. It is not
  wholly read-only: its `fleet-generate` dependency rewrites generated files
  under `fleet/policies/`. Review that diff after running it.
...
The current tagged release build uses:

```sh
cargo zigbuild -p velnor-runner -p velnorctl -p velnor-tools \
  --release --locked --features release-build --target "$TGT"
cargo deb -p velnor-runner --target "$TGT" --no-build --no-strip \
  --deb-version "$VERSION"
```

The guest-image and identity stages run separately in release CI. The runner
manifest and release workflow define this cross-build/package sequence.
[`crates/velnor-runner/Cargo.toml:25-34`](../crates/velnor-runner/Cargo.toml)
[`release.yml:653-654`](../.github/workflows/release.yml)
[`release.yml:746-749`](../.github/workflows/release.yml)
...
- Renderer goldens cover 12 resource nouns through every approved output format,
  ...
  `UPDATE_GOLDENS=1 cargo nextest run -p velnor-render --locked`, followed by
  human diff review. [`crates/velnor-render/tests/goldens.rs:1-6`](../crates/velnor-render/tests/goldens.rs)
...
- Never cancel the named release/package workflows or release dispatches. The
  current tree ships no `COORDINATION.md`; coordinate suspected stale runs with
  the release owner before any external action. This is a research project and
...
- The `velnor-runner` library dependency in `velnorctl` is interim. Plan 079 is
  ...
  (`docs/future-direction.md:28-39`,
