...
| `logs RESOURCE` | `--source`, `--cursor`, `--step`, `--failed`, `--tail`. | `CURRENT` |
...
| `run list` | Resource query flags. | `CURRENT`; reads `runs`. |
| `run view RUN_ID` | Numeric run id. | `CURRENT`; reads `runs`. |
| `run watch RUN_ID` | Watch run events. | `CURRENT` |
...
| `config` | `view`, `validate`, `diff`, `sources`. | `CURRENT`; local built-in/context resolution. |
...
| `workflow check` | Required `--repo`, `--reference`, `--workflow`. | `CURRENT`; static client-side compatibility check. |
...
| `POST /v1/instances/{instance}/{operation}` | JSON mutation body below | Would return operation id/phase; current server returns HTTP 501 `lifecycle reconciler is not installed`. | `RESERVED` |
...
`operation` must match the path. Reason and idempotency key are non-empty and max 512 bytes; scale is 1–4096. `crates/velnorctl/src/http.rs:858-946`, `crates/velnor-control/src/lifecycle.rs:289-347`.
...
`ApplicationServices` wires query, event/watch, logs, lifecycle, storage, and telemetry services. Durable construction uses the store; test construction is in-memory. `crates/velnor-control/src/application.rs:19-102`.
...
| `FeedStreamClient` | Connect, send log lines, ping; legacy per-call append remains available but connect/send is preferred. |
