...
Use the signed apt repository and an exact package version. The current setup
procedure is:
...
For a named daemon, the implicit daemon config base is:
...
```sh
velnorctl get hosts
velnorctl get instances --selector ready
...
```sh
GITHUB_TOKEN=... velnorctl configure \
...
These are **RESERVED**. They select the admin socket and carry reason,
idempotency, version, and scale fields, but the current HTTP mutation handler
returns `operation.unsupported` because the lifecycle ledger is not yet an
actuator (`crates/velnorctl/src/lib.rs:834-880`,
`crates/velnor-client/src/http.rs:244-291`,
`crates/velnorctl/src/http.rs:868-907`). `reconcile` additionally returns
`reconcile.plan_unavailable` because no reviewed plan is installed
(`crates/velnorctl/src/lib.rs:912-928`). No host or fleet state changes.
