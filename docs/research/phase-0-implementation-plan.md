...
Current code also treats enabled `actions/checkout` as a native host-side checkout before starting the Docker job container. It uses the self repository resource clone URL, job version/ref, and system access token by default; explicit `repository`, `path`, `ref`, `token`, `fetch-depth`, `persist-credentials`, and `clean` inputs are supported for target shapes such as Jackin's Homebrew tap checkout. Checkout defaults to GitHub's clean behavior with `git reset --hard HEAD` and `git clean -ffdx` after checkout. When credential persistence is enabled, Velnor writes a local git extraheader so later `git push` commands in the same checkout can authenticate. Velnor also writes per-job mounted-home `safe.directory` entries for checked-out worktrees so Git commands inside Docker accept host-created workspaces. Submodules, sparse checkout, LFS, and full credential cleanup remain later compatibility work.
