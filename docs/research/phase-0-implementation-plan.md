...
- env/context expansion in scripts and JavaScript action env: basic `steps.*.outputs.*`, `github.*`, `runner.*`, `env.*`, generic job `ContextData`, `toJSON(...)`, and workspace-backed `hashFiles(...)` interpolation is implemented
