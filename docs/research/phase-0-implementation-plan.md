...
Basic service containers from GitHub container resources are started on the same per-job Docker network before the job container, using the GitHub alias as Docker network alias. Velnor passes resource port mappings to Docker, waits for Docker health/running status before starting the job container, then removes services during cleanup.
