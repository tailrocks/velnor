...
| `actions/upload-artifact` | `UploadArtifact` | `name`, `path` including target glob patterns, `if-no-files-found`, `include-hidden-files` defaulting to false like the latest action, `overwrite` defaulting to false with duplicate-name failure, exact estate-approved `compression-level: 0` (ZIP Stored), `retention-days`, target outputs, deterministic per-run/per-name artifact id, run-scoped workdir storage, Results Service upload required for cross-host handoff |
