...
| `actions/cache` | `Cache` | restore/save paths, key, restore keys with newest prefix match, `hashFiles(...)` keys, latest `cache-hit` output semantics (`true` exact hit, `false` prefix hit, empty miss), `fail-on-cache-miss`, `lookup-only` without restoring paths, shared workdir cache storage; per-key locks and a post-copy completion marker prevent readers from observing partial generations, and restored trees are verified before execution |
...
| `actions/upload-pages-artifact` | `UploadPagesArtifact` | package the approved `path` as one uncompressed `artifact.tar`, dereference links, exclude hidden entries, upload that single file through Results Service, and expose the artifact handoff id; never upload the site files directly |
