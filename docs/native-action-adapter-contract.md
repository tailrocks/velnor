...
| `actions/upload-artifact` | `UploadArtifact` | `name`, `path`, `if-no-files-found`, `retention-days`, target outputs, deterministic per-run/per-name artifact id, run-scoped workdir storage |
