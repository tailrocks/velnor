...
- stdout/stderr from executed script and JavaScript steps is captured and uploaded to GitHub timeline task records before completion, with GitHub/job/add-mask masks applied
...
- basic workflow command parsing: state-changing stdout commands are parsed; `error`, `warning`, and `notice` commands are counted on timeline records, preserved as structured run-service step annotations, and title/location plus group/debug markers are preserved in uploaded step logs; native GitHub UI group folding is not yet reported
