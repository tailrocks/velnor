...
- runner-side expression contexts: `env.*`, selected `github.*`/`runner.*`, `steps.*.outputs.*`, and generic job `ContextData` lookup such as `matrix.*`, `needs.*`, `inputs.*`, and `vars.*` are supported for script/action env and basic step conditions; `contains()`, `toJSON()`, and workspace-backed `hashFiles(...)` cover target-shaped cases
