...
- target expression coverage now checks both workflow YAML from the two target repositories and cached marketplace/local action metadata under `/tmp/velnor-actions`, including multiline composite expressions and multi-argument `hashFiles(...)`
