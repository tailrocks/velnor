...
- broader `actions/checkout` compatibility: `repository`, `path`, `ref`, `token`, `fetch-depth`, `persist-credentials`, and `clean` inputs are implemented for target shapes; checked-out worktrees are added to the mounted-home `safe.directory` config for Docker-side Git commands; sparse checkout, submodules, LFS, and credentials cleanup remain open
