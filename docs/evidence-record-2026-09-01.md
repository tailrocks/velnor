...
The current documentation index makes the same boundary explicit: source and
tests define current behavior, while dated evidence is historical unless
revalidated (`docs/index.md:1-59`, `AGENTS.md:1-16`). This record therefore does
not promote an old plan, green local test, release, or live sample into a
current implementation claim. Several source paths cited below belonged to
the historical documentation set and are retained only as provenance in this
archive; they are not active links or procedures.
