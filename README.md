...
<!-- MAP:BEGIN — generated by `mise run fleet:map`; edit `repolint.toml` instead. -->
> Drift-checked in `mise run fleet:check`; details live in each component's README.

| Path | Purpose |
| --- | --- |
| [`crates/`](crates/) | Workspace members; `velnorctl` binary legal in `crates/` per §3 |
| [`config/`](config/) | Repository and fleet configuration |
| [`docker/`](docker/) | Job-container build context and pinned mise inputs |
| [`docs/`](docs/) | Product, operator, and architecture documentation |
| [`fleet/`](fleet/) | CI-producer product surface (§3 producer exception) |
| [`microvm/`](microvm/) | MicroVM guest image and runtime assets |
| [`plans/`](plans/) | Velnor migration and operations plans |
| [`schemas/`](schemas/) | Versioned action and protocol schemas |
| [`scripts/`](scripts/) | Fixture, smoke, and repository verification tooling |
| [`tools/`](tools/) | Workspace tooling and unit-collector utility |
<!-- MAP:END -->
